      SUBROUTINE PZTREECOMB( ICTXT, SCOPE, N, MINE, RDEST0, CDEST0,
     $                       SUBPTR )
*
...
*  =======
*
*  PZTREECOMB does a 1-tree parallel combine operation on scalars,
...
*
*  MINE    (local input/global output) COMPLEX*16 array of
...
*
            ...
            END IF
*
            CALL ZGESD2D( ICTXT, N, 1, MINE, N, RMSSG, CMSSG )
*
            GO TO 20
*
         ELSE
...
*
               CALL ZGERV2D( ICTXT, N, 1, HIS, N, RMSSG, CMSSG )
...
*
      IF( BCAST ) THEN
         IF( MYDIST2.EQ.0 ) THEN
            CALL ZGEBS2D( ICTXT, SCOPE, ' ', N, 1, MINE, N )
         ELSE
            CALL ZGEBR2D( ICTXT, SCOPE, ' ', N, 1, MINE, N,
     $                    TRDEST, TCDEST )
         END IF
      END IF
*
...
*  =======
*
*  ZCOMBAMAX finds the element having max. absolute value as well
