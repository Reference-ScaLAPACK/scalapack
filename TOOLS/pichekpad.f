...
*  =======
*
*  PICHEKPAD checks that the padding around a local array has not
*  been overwritten since the call to PIFILLPAD.  3 types of errors
*  are reported:
*
...
*
*  A       (local input) @(typec) array of dimension (LDA,N).
...
*
   ...
   10    CONTINUE
      ELSE
         WRITE( *, FMT = * ) 'WARNING no pre-guardzone in PICHEKPAD'
      END IF
*
...
*
   ...
   20    CONTINUE
      ELSE
         WRITE( *, FMT = * )
     $          'WARNING no post-guardzone buffer in PICHEKPAD'
      END IF
*
...
*
 ...
 9998 FORMAT( '{', I5, ',', I5, '}:  ', A, ' memory overwrite in ',
     $        A4, '-guardzone: loc(', I3, ') = ', I8 )
 9997 FORMAT( '{', I5, ',', I5, '}: ', A, ' memory overwrite in ',
     $        'lda-m gap: loc(', I3, ',', I3, ') = ', I8 )
*
      RETURN
*
*     End of PICHEKPAD
