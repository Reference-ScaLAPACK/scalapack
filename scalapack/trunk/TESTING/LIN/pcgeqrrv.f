...
*  =======
*
*  PCGEQRRV computes sub( A ) = A(IA:IA+M-1,JA:JA+N-1) from Q, R
*  computed by PCGEQRF.
*
*  Notes
*  =====
...
*          by PCGEQRF. On exit, the original matrix is restored.
*
...
*          TAU of the elementary reflectors computed by PCGEQRF. TAU
...
*
         CALL PCLARFT( 'Forward', 'Columnwise', M-I+IA, JB, A, I, J,
     $                 DESCA, TAU, WORK( IPT ), WORK( IPW ) )
*
*        Copy Householder vectors into workspace
*
         CALL PCLACPY( 'Lower', M-I+IA, JB, A, I, J, DESCA, WORK( IPV ),
     $                 IV, 1, DESCV )
         CALL PCLASET( 'Upper', M-I+IA, JB, ZERO, ONE, WORK( IPV ), IV,
     $                 1, DESCV )
*
*        Zeroes the strict lower triangular part of sub( A ) to get
*        block column of R
*
         CALL PCLASET( 'Lower', M-I+IA-1, JB, ZERO, ZERO, A, I+1, J,
     $                 DESCA )
*
...
*
     ...
     $              IROFF+1, 1, DESCV )
      CALL PCLASET( 'Upper', M, JB, ZERO, ONE, WORK, IROFF+1, 1, DESCV )
*
...
*     Apply block Householder transformation
*
      CALL PCLARFB( 'Left', 'No transpose', 'Forward', 'Columnwise', M,
