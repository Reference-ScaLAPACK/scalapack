      SUBROUTINE PZDTLASCHK( SYMM, UPLO, TRANS, N, BWL, BWU, NRHS, X,
...
*  =======
*
*  PZDTLASCHK computes the residual
...
*
      IF ( WORKSIZ .LT. WORK_MIN ) THEN
       CALL PXERBLA( ICTXT, 'PZTLASCHK', -18 )
       RETURN
      END IF
*
...
*
               IF( LSAME( SYMM, 'H' )) THEN
                 CALL PZBMATGEN( ICTXT, UPLO, 'D', BW, BW, N, BW+1,
     $                           DESCA( NB_ ), A, DESCA( LLD_ ), 0, 0,
     $                           IASEED, MYROW, MYCOL, NPROW, NPCOL )
               ELSE
*
                 CALL PZBMATGEN( ICTXT, 'N', UPLO, BWL, BWU, N,
...
*
               IF( MYCOL.GT.0 ) THEN
                  CALL ZGESD2D( ICTXT, 1, 1, A( START+2), LDA,
     $                     MYROW, MYCOL-1 )
               ENDIF
*
               IF( MYCOL.LT.NPCOL-1 ) THEN
                  CALL ZGESD2D( ICTXT, 1, 1,
...
*
               IF( MYCOL.LT.NPCOL-1 ) THEN
                  CALL ZGERV2D( ICTXT, 1, 1,
     $                     A( START+2+( DESCA( NB_ )-1 )*LDA ),
     $                     LDA, MYROW, MYCOL+1 )
               ENDIF
*
               IF( MYCOL.GT.0 ) THEN
                  CALL ZGERV2D( ICTXT, 1, 1, A( START), LDA,
     $                               MYROW, MYCOL-1 )
               ENDIF
*
*     Loop over the rhs
...
*
*
            CALL PZGBDCMV( BWL+BWU+1, BWL, BWU, TRANS, N, A, 1, DESCA,
...
*
            CALL PZMATGEN( DESCX( CTXT_ ), 'No', 'No', DESCX( M_ ),
...
*           Figure || A * X - B || & || X ||
*
            CALL PZAXPY( N, -ONE, WORK( IPPRODUCT ), 1, 1, DESCX, 1,
     $                   WORK( IPB ), 1, 1, DESCX, 1 )
*
            CALL PDZNRM2( N, NORMX,
     $           X, 1, J, DESCX, 1 )
*
            CALL PDZNRM2( N, RESID1,
     $           WORK( IPB ), 1, 1, DESCX, 1 )
*
*
