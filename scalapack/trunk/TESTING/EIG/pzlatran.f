...
*  =======
*
*  PZLATRAN transpose a lower triangular matrix on to the upper
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, ZTRRV2D, ZTRSD2D
*     ..
*     .. External Functions ..
      INTEGER            NUMROC
      EXTERNAL           NUMROC
...
*
         DO 20 J = 1, NP
            DO 10 I = J + 1, NQ
               A( J+( I-1 )*LDA ) = DCONJG( A( I+( J-1 )*LDA ) )
   10       CONTINUE
   20    CONTINUE
*
      ELSE
...
*
     ...
     $         CALL ZTRSD2D( ICTXT, 'U', 'N', IRECV, JRECV, WORK, IRECV,
     $                       MYCOL, MYROW )
*
            IF( ISEND.GT.0 .AND. JSEND.GT.0 )
     $         CALL ZTRRV2D( ICTXT, 'U', 'N', ISEND, JSEND,
