      SUBROUTINE PCHETDRV( UPLO, N, A, IA, JA, DESCA, D, E, TAU, WORK,
     $                     INFO )
*
...
*  =======
*
*  PCHETDRV computes sub( A ) = A(IA:IA+N-1,JA:JA+N-1) from Q, the
*  Hermitian tridiagonal matrix T (or D and E), and TAU, which were
*  computed by PCHETRD:  sub( A ) := Q * T * Q'.
*
*  Notes
*  =====
*
...
*     .. Parameters ..
     ...
     $                     RSRC_ = 7, CSRC_ = 8, LLD_ = 9 )
      REAL               REIGHT, RONE, RZERO
      PARAMETER            ( REIGHT = 8.0E+0, RONE = 1.0E+0,
     $                     RZERO = 0.0E+0 )
...
*     .. External Subroutines ..
     ...
     $                   PCELGET, PCGEMM, PCHEMM,
     $                   PCHER2K, PCLACPY, PCLARFT,
     $                   PCLASET, PCTRMM, PSELGET
*     ..
*     .. Intrinsic Functions ..
...
*
         DO 10 J = 0, N-1
            ...
            IF( J.LT.(N-1) ) THEN
               ...
               CALL PCELGET( 'Columnwise', ' ', E1, A, IA+J, JA+J+1,
     $                       DESCA )
            END IF
*
...
*
            CALL PCLARFT( 'Backward', 'Columnwise', K+JB-1, JB, A, IA,
     $                    J, DESCA, TAU, WORK( IPT ), WORK( IPV ) )
*
*           Copy Householder vectors into WORK( IPV ).
*
            CALL PCLACPY( 'All', K+JB-1, JB, A, IA, J, DESCA,
     $                    WORK( IPV ), 1, 1, DESCV )
*
            IF( K.GT.0 ) THEN
               CALL PCLASET( 'Lower', JB+1, JB, ZERO, ONE, WORK( IPV ),
     $                       K, 1, DESCV )
            ELSE
               CALL PCLASET( 'Lower', JB, JB-1, ZERO, ONE, WORK( IPV ),
     $                       1, 2, DESCV )
               CALL PCLASET( 'Ge', JB, 1, ZERO, ZERO, WORK( IPV ), 1,
     $                       1, DESCV )
            END IF
*
...
*
            IF( K.GT.0 ) THEN
               CALL PCLASET( 'Ge', K-1, JB, ZERO, ZERO, A, IA, J,
     $                       DESCA )
               CALL PCLASET( 'Upper', JB-1, JB-1, ZERO, ZERO, A, I-1,
     $                       J+1, DESCA )
            ELSE IF( JB.GT.1 ) THEN
               CALL PCLASET( 'Upper', JB-2, JB-2, ZERO, ZERO, A, IA,
     $                       J+2, DESCA )
            END IF
*
*           (1) X := A * V * T'
*
            CALL PCHEMM( 'Left', 'Upper', K+JB, JB, ONE, A, IA, JA,
     $                   DESCA, WORK( IPV ), 1, 1, DESCV, ZERO,
     $                   WORK( IPX ), 1, 1, DESCV )
            CALL PCTRMM( 'Right', 'Lower', 'Conjugate transpose',
     $                   'Non-Unit', K+JB, JB, ONE, WORK( IPT ), 1, 1,
     $                   DESCT, WORK( IPX ), 1, 1, DESCV )
*
*           (2) X := X - 1/2 * V * (T * V' * X)
*
            CALL PCGEMM( 'Conjugate transpose', 'No transpose', JB, JB,
     $                   K+JB, ONE, WORK( IPV ), 1, 1, DESCV,
     $                   WORK( IPX ), 1, 1, DESCV, ZERO, WORK( IPY ),
     $                   1, 1, DESCT )
            CALL PCTRMM( 'Left', 'Lower', 'No transpose', 'Non-Unit',
     $                   JB, JB, ONE, WORK( IPT ), 1, 1, DESCT,
     $                   WORK( IPY ), 1, 1, DESCT )
            CALL PCGEMM( 'No tranpose', 'No transpose', K+JB, JB, JB,
     $                   -HALF, WORK( IPV ), 1, 1, DESCV, WORK( IPY ),
     $                   1, 1, DESCT, ONE, WORK( IPX ), 1, 1, DESCV )
*
*           (3) A := A - X * V' - V * X'
*
            CALL PCHER2K( 'Upper', 'No transpose', K+JB, JB, -ONE,
...
*
            CALL PCLARFT( 'Forward', 'Columnwise', N-K, JB, A, I+1, J,
     $                    DESCA, TAU, WORK( IPT ), WORK( IPV ) )
*
*           Copy Householder vectors into WORK( IPV ).
*
            CALL PCLACPY( 'Lower', N-K, JB, A, I+1, J, DESCA,
     $                    WORK( IPV ), K+1, 1, DESCV )
            CALL PCLASET( 'Upper', N-K, JB, ZERO, ONE, WORK( IPV ),
     $                    K+1, 1, DESCV )
            CALL PCLASET( 'Ge', 1, JB, ZERO, ZERO, WORK( IPV ), K, 1,
     $                    DESCV )
*
*           Zero out the strict lower triangular part of A.
*
            CALL PCLASET( 'Lower', N-K-1, JB, ZERO, ZERO, A, I+2, J,
     $                    DESCA )
*
*           (1) X := A * V * T'
*
            CALL PCHEMM( 'Left', 'Lower', N-K+1, JB, ONE, A, I, J,
...
*
     ...
     $                   1, 1, DESCT )
            CALL PCTRMM( 'Left', 'Upper', 'No transpose', 'Non-Unit',
     $                   JB, JB, ONE, WORK( IPT ), 1, 1, DESCT,
     $                   WORK( IPY ), 1, 1, DESCT )
            CALL PCGEMM( 'No transpose', 'No transpose', N-K+1, JB, JB,
     $                   -HALF, WORK( IPV ), K, 1, DESCV, WORK( IPY ),
     $                   1, 1, DESCT, ONE, WORK( IPX ), K, 1, DESCV )
*
*           (3) A := A - X * V' - V * X'
*
            CALL PCHER2K( 'Lower', 'No tranpose', N-K+1, JB, -ONE,
