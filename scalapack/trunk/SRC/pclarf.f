      SUBROUTINE PCLARF( SIDE, M, N, V, IV, JV, DESCV, INCV, TAU,
     $                   C, IC, JC, DESCC, WORK )
*
...
*  =======
*
*  PCLARF applies a complex elementary reflector Q to a complex M-by-N
...
*     .. External Subroutines ..
     ...
     $                   CGEMV, CGERC, CGERV2D, CGESD2D,
     $                   CGSUM2D, CLASET, INFOG2L, PB_TOPGET,
     $                   PBCTRNV
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
     ...
     $                              WORK( IPW ), 1 )
                     ELSE
                        CALL CLASET( 'All', NQ, 1, ZERO, ZERO,
     $                               WORK( IPW ), MAX( 1, NQ ) )
                     END IF
                     CALL CGSUM2D( ICTXT, 'Columnwise', ' ', NQ, 1,
...
*
                        IF( MP.GT.0 ) THEN
                           CALL CGEMV( 'Conjugate transpose', MP, NQ,
...
*
                     IPW = MP+1
                     CALL CCOPY( MP, V( IOFFV ), 1, WORK, 1 )
                     WORK( IPW ) = TAU( JJV )
...
*
                        IF( NQ.GT.0 ) THEN
                           CALL CGEMV( 'No transpose', MP, NQ, ONE,
     $                                 C( IOFFC ), LDC, V( IOFFV ), LDV,
     $                                 ZERO, WORK, 1 )
                        ELSE
                           CALL CLASET( 'All', MP, 1, ZERO, ZERO,
     $                                  WORK, MAX( 1, MP ) )
                        END IF
                        CALL CGSUM2D( ICTXT, 'Rowwise', ' ', MP, 1,
...
*
                     ...
                     WORK(IPW) = TAU( IIV )
                     CALL CGESD2D( ICTXT, IPW, 1, WORK, IPW, ICROW,
     $                             MYCOL )
*
...
*
     ...
     $                           WORK( IPW ), 1 )
                  ELSE
                     CALL CLASET( 'All', MP, 1, ZERO, ZERO, WORK( IPW ),
     $                            MAX( 1, MP ) )
                  END IF
                  CALL CGSUM2D( ICTXT, 'Rowwise', ' ', MP, 1,
     $                          WORK( IPW ), MAX( 1, MP ), RDEST,
