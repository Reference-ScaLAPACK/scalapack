      SUBROUTINE PZGELQF( M, N, A, IA, JA, DESCA, TAU, WORK, LWORK,
     $                    INFO )
*
...
*  =======
*
*  PZGELQF computes a LQ factorization of a complex distributed M-by-N
...
*     .. External Subroutines ..
     ...
     $                   PB_TOPSET, PXERBLA, PZGELQ2, PZLARFB,
     $                   PZLARFT
*     ..
*     .. External Functions ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PZGELQF', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
      CALL PZGELQ2( IB, N, A, IA, JA, DESCA, TAU, WORK, LWORK, IINFO )
*
...
*        H = H(ia) H(ia+1) . . . H(in)
*
         CALL PZLARFT( 'Forward', 'Rowwise', N, IB, A, IA, JA, DESCA,
     $                 TAU, WORK, WORK( IPW ) )
*
*        Apply H to A(ia+ib:ia+m-1,ja:ja+n-1) from the right
*
         CALL PZLARFB( 'Right', 'No transpose', 'Forward', 'Rowwise',
...
*        A(i:i+ib-1:j:ja+n-1)
*
         CALL PZGELQ2( IB, N-I+IA, A, I, J, DESCA, TAU, WORK, LWORK,
     $                 IINFO )
*
...
*
            CALL PZLARFT( 'Forward', 'Rowwise', N-I+IA, IB, A, I, J,
