      SUBROUTINE PSPOSV( UPLO, N, NRHS, A, IA, JA, DESCA, B, IB, JB,
     $                   DESCB, INFO )
*
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, PCHK2MAT, PSPOTRF,
     $                   PSPOTRS, PXERBLA
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PSPOSV', -INFO )
         RETURN
      END IF
*
...
*        with X.
*
         CALL PSPOTRS( UPLO, N, NRHS, A, IA, JA, DESCA, B, IB, JB,
     $                 DESCB, INFO )
*
      END IF
*
