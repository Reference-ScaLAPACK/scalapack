      SUBROUTINE PSDTTRF( N, DL, D, DU, JA, DESCA, AF, LAF, WORK, LWORK,
     $                    INFO )
*
...
*
      IF( N.GT.NP*NB-MOD( JA-1, NB ) ) THEN
         INFO = -( 1 )
         CALL PXERBLA( ICTXT, 'PSDTTRF, D&C alg.: only 1 block per proc'
     $                 , -INFO )
         RETURN
      END IF
*
      IF( ( JA+N-1.GT.NB ) .AND. ( NB.LT.2*INT_ONE ) ) THEN
         INFO = -( 6*100+4 )
         CALL PXERBLA( ICTXT, 'PSDTTRF, D&C alg.: NB too small', -INFO )
         RETURN
      END IF
*
*
...
*        put minimum value of laf into AF( 1 )
         AF( 1 ) = LAF_MIN
         CALL PXERBLA( ICTXT, 'PSDTTRF: auxiliary storage error ',
     $                 -INFO )
         RETURN
      END IF
*
...
*
      IF( LWORK.LT.WORK_SIZE_MIN ) THEN
         IF( LWORK.NE.-1 ) THEN
            INFO = -10
            CALL PXERBLA( ICTXT, 'PSDTTRF: worksize error ', -INFO )
         END IF
         RETURN
      END IF
*
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PSDTTRF', -INFO )
         RETURN
      END IF
*
...
*
      CALL SDTTRF( ODD_SIZE, DL( PART_OFFSET+2 ), D( PART_OFFSET+1 ),
...
*
            CALL SDTTRSV( 'L', 'N', ODD_SIZE, INT_ONE,
...
*
     ...
     $                    MYCOL-1 )
*
            CALL SDTTRSV( 'U', 'T', ODD_SIZE, INT_ONE,
     $                    DL( PART_OFFSET+2 ), D( PART_OFFSET+1 ),
...
*
     ...
     $                 INT_ONE, 0, MYCOL-1 )
*
         CALL SGESD2D( ICTXT, INT_ONE, INT_ONE, AF( WORK_U+ODD_SIZE+1 ),
     $                 INT_ONE, 0, MYCOL-1 )
*
      END IF
*
...
*
     ...
     $                 INT_ONE, 0, COMM_PROC )
*
         CALL SGERV2D( ICTXT, INT_ONE, INT_ONE, AF( WORK_U+ODD_SIZE+1 ),
     $                 INT_ONE, 0, COMM_PROC )
*
         IF( INFO.EQ.0 ) THEN
*
...
*           left
*
            CALL SGERV2D( ICTXT, INT_ONE, INT_ONE,
