...
*          PSDBTRF and this is stored in AF. If a linear system
*          is to be solved using PSDBTRS after the factorization
...
*
      IF( N.GT.NP*NB-MOD( JA-1, NB ) ) THEN
         INFO = -( 1 )
         CALL PXERBLA( ICTXT, 'PSDBTRF, D&C alg.: only 1 block per proc'
     $                 , -INFO )
         RETURN
      END IF
*
      IF( ( JA+N-1.GT.NB ) .AND. ( NB.LT.2*MAX( BWL, BWU ) ) ) THEN
         INFO = -( 6*100+4 )
         CALL PXERBLA( ICTXT, 'PSDBTRF, D&C alg.: NB too small', -INFO )
         RETURN
      END IF
*
*
...
*        put minimum value of laf into AF( 1 )
         AF( 1 ) = LAF_MIN
         CALL PXERBLA( ICTXT, 'PSDBTRF: auxiliary storage error ',
     $                 -INFO )
         RETURN
      END IF
*
...
*
      IF( LWORK.LT.WORK_SIZE_MIN ) THEN
         IF( LWORK.NE.-1 ) THEN
            INFO = -10
            CALL PXERBLA( ICTXT, 'PSDBTRF: worksize error ', -INFO )
         END IF
         RETURN
      END IF
*
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PSDBTRF', -INFO )
         RETURN
      END IF
*
...
*
         CALL STRSD2D( ICTXT, 'U', 'N', NEXT_TRI_SIZE_M,
...
*
      CALL SDBTRF( ODD_SIZE, ODD_SIZE, BWL, BWU, A( OFST+1 ), LLDA,
     $             INFO )
*
      IF( INFO.NE.0 ) THEN
         INFO = MYCOL + 1
...
*           in main storage.
*
         CALL SLATCPY( 'L', BWL, BWL, AF( ODD_SIZE*BWU+2*MBW2+1+MAX_BW-
...
*
*
         CALL SGEMM( 'T', 'N', MAX_BW, MAX_BW, MAX_BW, -ONE,
...
*
*
         CALL STRRV2D( ICTXT, 'U', 'N', PREV_TRI_SIZE_M,
...
*
            DO 80 I1 = 1, ODD_SIZE
               I2 = MIN( BWU, I1-1 )
               CALL SGEMV( 'N', BWU, I2, -ONE, AF( ( I1-1-I2 )*BWU+1 ),
...
*
   ...
   90       CONTINUE
*
            CALL SGEMM( 'N', 'T', BWU, BWL, ODD_SIZE, -ONE, AF( 1 ),
...
*
            CALL SGESD2D( ICTXT, MAX_BW, MAX_BW,
...
*
     ...
     $                      MAX_BW )
*
               CALL STRMM( 'R', 'U', 'T', 'N', BWL, BWL, -ONE,
...
*
     ...
     $                      BWU ), MAX_BW )
*
               CALL STRMM( 'R', 'L', 'N', 'N', BWU, BWU, -ONE,
...
*
         CALL SGESD2D( ICTXT, MAX_BW, MAX_BW, AF( ODD_SIZE*BWU+1 ),
     $                 MAX_BW, 0, MYCOL-1 )
*
         CALL SGESD2D( ICTXT, MAX_BW, MAX_BW,
...
*
      IF( MYCOL.LT.NPCOL-1 ) THEN
*
         CALL SGERV2D( ICTXT, MAX_BW, MAX_BW,
     $                 AF( ODD_SIZE*BWU+2*MBW2+1 ), MAX_BW, 0, MYCOL+1 )
*
*          Add contribution to diagonal block
*
         CALL SAXPY( MBW2, ONE, AF( ODD_SIZE*BWU+2*MBW2+1 ), 1,
...
*
      IF( MYCOL-LEVEL_DIST.GE.0 ) THEN
         CALL SGERV2D( ICTXT, MAX_BW, MAX_BW, WORK( 1 ), MAX_BW, 0,
     $                 MYCOL-LEVEL_DIST )
*
         CALL SAXPY( MBW2, ONE, WORK( 1 ), 1, AF( ODD_SIZE*BWU+MBW2+1 ),
     $               1 )
*
      END IF
*
...
*
      CALL SDBTRF( MAX_BW, MAX_BW, MIN( MAX_BW-1, BWL ),
...
*
         CALL SGERV2D( ICTXT, MAX_BW, MAX_BW, AF( ODD_SIZE*BWU+1 ),
     $                 MAX_BW, 0, COMM_PROC )
*
         CALL SGERV2D( ICTXT, MAX_BW, MAX_BW,
...
*
*
            CALL STBTRS( 'L', 'N', 'U', BWU, MIN( BWL, BWU-1 ), BWU,
...
*
*
            CALL STBTRS( 'U', 'T', 'N', BWL, MIN( BWU, BWL-1 ), BWL,
...
*
         CALL SGESD2D( ICTXT, MAX_BW, MAX_BW, WORK( 1 ), MAX_BW, 0,
     $                 MYCOL+LEVEL_DIST )
*
      END IF
...
*
*
*         Since STBTRS has no "left-right" option, we must transpose
*
            CALL SLATCPY( 'N', MAX_BW, MAX_BW,
     $                    AF( WORK_U+ODD_SIZE*BWL+2*MBW2+1 ), MAX_BW,
     $                    WORK( 1 ), MAX_BW )
*
            CALL STBTRS( 'L', 'N', 'U', MAX_BW, MIN( BWL, MAX_BW-1 ),
     $                   BWL, AF( ODD_SIZE*BWU+MBW2+1 ), MAX_BW+1,
     $                   WORK( 1+MAX_BW*( MAX_BW-BWL ) ), MAX_BW, INFO )
*
*         Transpose back
*
            CALL SLATCPY( 'N', MAX_BW, MAX_BW, WORK( 1 ), MAX_BW,
     $                    AF( WORK_U+ODD_SIZE*BWL+2*MBW2+1 ), MAX_BW )
*
*
*
...
*
     ...
     $                    WORK( 1 ), MAX_BW )
*
            CALL STBTRS( 'U', 'T', 'N', MAX_BW, MIN( BWU, MAX_BW-1 ),
...
*
         CALL SGEMM( 'N', 'T', MAX_BW, MAX_BW, MAX_BW, -ONE,
     $               AF( ( ODD_SIZE )*BWU+2*MBW2+1 ), MAX_BW,
     $               AF( WORK_U+( ODD_SIZE )*BWL+2*MBW2+1 ), MAX_BW,
     $               ZERO, WORK( 1 ), MAX_BW )
*
...
*
            CALL SGEMM( 'N', 'N', MAX_BW, MAX_BW, MAX_BW, -ONE,
     $                  AF( WORK_U+ODD_SIZE*BWL+2*MBW2+1 ), MAX_BW,
