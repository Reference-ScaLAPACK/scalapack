...
*
            ...
            IF( MYCOL.EQ.IVCOL ) THEN
               CALL SGEBS2D( ICTXT, 'Rowwise', ROWBTOP, MPC, K,
     $                       V( IOFFV ), LDV )
               IF( MYROW.EQ.IVROW )
     ...
     $                       WORK( IPV ), LV, MYROW, IVCOL )
               IF( MYROW.EQ.IVROW )
     $            CALL STRBR2D( ICTXT, 'Rowwise', ROWBTOP, UPLO,
...
*
   10          CONTINUE
               IF( K-ITOP .GT.0 ) THEN
                  CALL SLASET( 'Upper', IINXT-IIBEG+1, K-ITOP, ZERO,
...
*
   20          CONTINUE
               IF( JJ.LE.( JJV+K-1 ) ) THEN
                  HEIGHT = IBASE - ITOP
                  CALL SLASET( 'All', KP, ITOP-JJ+JJV, ZERO, ZERO,
     $                         WORK( IPV+II-IIV+(JJ-JJV)*LV ), LV )
                  CALL SLASET( 'Lower', KP, HEIGHT, ZERO, ONE,
...
*
            IF( MPC.GT.0 ) THEN
               CALL SGEMM( 'Transpose', 'No transpose', NQC, K, MPC,
     $                    ONE, C( IOFFC ), LDC, WORK( IPV ), LV, ZERO,
     $                    WORK( IPW ), LW )
            ELSE
               CALL SLASET( 'All', NQC, K, ZERO, ZERO, WORK( IPW ), LW )
            END IF
*
            CALL SGSUM2D( ICTXT, 'Columnwise', ' ', NQC, K, WORK( IPW ),
     $                    LW, IVROW, MYCOL )
*
...
*
               CALL STRMM( 'Right', UPLO, TRANST, 'Non unit', NQC, K,
     $                     ONE, T, NBV, WORK( IPW ), LW )
               CALL SGEBS2D( ICTXT, 'Columnwise', ' ', NQC, K,
     $                       WORK( IPW ), LW )
            ELSE
               CALL SGEBR2D( ICTXT, 'Columnwise', ' ', NQC, K,
...
*
            CALL SGEMM( 'No transpose', 'Transpose', MPC, NQC, K, -ONE,
...
*
   40             CONTINUE
     ...
     $                            WORK( IPW1+II-IIV+ITOP*LW ), LW )
            ...
            END IF
*
            CALL PBSTRAN( ICTXT, 'Columnwise', 'Transpose', N+IROFFV, K,
...
*
            IF( NQC.GT.0 ) THEN
               CALL SGEMM( 'No transpose', 'Transpose', MPC, K, NQC,
     $                     ONE, C( IOFFC ), LDC, WORK( IPV ), LV, ZERO,
     $                     WORK( IPW ), LW )
            ELSE
               CALL SLASET( 'All', MPC, K, ZERO, ZERO, WORK( IPW ), LW )
            END IF
*
            CALL SGSUM2D( ICTXT, 'Rowwise', ' ', MPC, K, WORK( IPW ),
     $                    LW, MYROW, IVCOL )
*
*           WORK( IPW ) = WORK( IPW ) * T' or WORK( IPW ) * T
*
...
*
                  CALL STRBS2D( ICTXT, 'Columnwise', ' ', UPLO,
     $                          'Non unit', K, K, T, NBV )
               ELSE
                  CALL STRBR2D( ICTXT, 'Columnwise', ' ', UPLO,
     $                          'Non unit', K, K, T, NBV, IVROW, MYCOL )
               END IF
               CALL STRMM( 'Right', UPLO, TRANS, 'Non unit', MPC, K,
     $                     ONE, T, NBV, WORK( IPW ), LW )
*
               CALL SGEBS2D( ICTXT, 'Rowwise', ' ', MPC, K, WORK( IPW ),
     $                       LW )
            ELSE
               CALL SGEBR2D( ICTXT, 'Rowwise', ' ', MPC, K, WORK( IPW ),
     $                       LW, MYROW, IVCOL )
            END IF
*
*               C            C      -     W       *     V'
...
*                        MPC x NQC    MPC x K         K x NQC
*
            CALL SGEMM( 'No transpose', 'No transpose', MPC, NQC, K,
...
*
   50             CONTINUE
                  IF( ( K-ILEFT ).GT.0 ) THEN
                     CALL SLASET( 'Lower', K-ILEFT, JJNXT-JJBEG+1, ZERO,
     $                            ONE,
...
*
   60             CONTINUE
                  IF( II.LE.( IIV+K-1 ) ) THEN
                     WIDE = IRIGHT - ILEFT
                     CALL SLASET( 'All', ILEFT-II+IIV, KQ, ZERO, ZERO,
     $                            WORK( IPW1+II-IIV+(JJ-JJV)*LW ), LW )
                     CALL SLASET( 'Upper', WIDE, KQ, ZERO, ONE,
...
*
                  CALL STRBS2D( ICTXT, 'Rowwise', ' ', UPLO, 'Non unit',
     $                          K, K, T, MBV )
               ELSE
                  CALL STRBR2D( ICTXT, 'Rowwise', ' ', UPLO, 'Non unit',
...
*
            ...
            IF( MYROW.EQ.IVROW ) THEN
               CALL SGEBS2D( ICTXT, 'Columnwise', COLBTOP, K, NQC,
     $                       V( IOFFV ), LDV )
               IF( MYCOL.EQ.IVCOL )
     ...
     $                       WORK( IPV ), LV, IVROW, MYCOL )
               IF( MYCOL.EQ.IVCOL )
     $            CALL STRBR2D( ICTXT, 'Columnwise', COLBTOP, UPLO,
