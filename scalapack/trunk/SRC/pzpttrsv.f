...
*  =======
*
*  PZPTTRSV solves a tridiagonal triangular system of linear equations
*
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PZPTTRSV', -INFO )
         RETURN
      END IF
*
...
*
        CALL ZPTTRSV( UPLO, 'N', ODD_SIZE, NRHS, D( PART_OFFSET+1 ),
...
*
            CALL ZAXPY( NRHS, -E( PART_OFFSET+ODD_SIZE ),
...
*
            CALL ZGEMM( 'C', 'N', 1, NRHS, ODD_SIZE, -CONE, AF( 1 ),
...
*
        CALL ZTRTRS( 'L', 'N', 'U', INT_ONE, NRHS, AF( ODD_SIZE+2 ),
...
*       Solve with diagonal block
*
        CALL ZTRTRS( 'L', 'C', 'U', INT_ONE, NRHS, AF( ODD_SIZE+2 ),
     $               INT_ONE, B( PART_OFFSET+ODD_SIZE+1 ), LLDB, INFO )
*
        IF( INFO.NE.0 ) THEN
          GO TO 1000
...
*
          CALL ZGEMM( 'N', 'N', ODD_SIZE, NRHS, 1, -CONE, AF( 1 ),
...
*           locally stored portion of right hand side(s)
*
            CALL ZAXPY( NRHS, -DCONJG( E( PART_OFFSET+ODD_SIZE ) ),
     $                 B( PART_OFFSET+ODD_SIZE+1 ), LLDB,
     $                 B( PART_OFFSET+ODD_SIZE ), LLDB )
*
        ENDIF
...
*       Use main partition in each processor to solve locally
*
        CALL ZPTTRSV( UPLO, 'C', ODD_SIZE, NRHS, D( PART_OFFSET+1 ),
     $                E( PART_OFFSET+1 ), B( PART_OFFSET+1 ), LLDB,
     $                INFO )
*
      ENDIF
...
*           processor's righthand-side.
*
            CALL ZGEMM( 'T', 'N', 1, NRHS, ODD_SIZE, -CONE, AF( 1 ),
     $                  ODD_SIZE, B( PART_OFFSET+1 ), LLDB, CZERO,
     $                  WORK( 1+INT_ONE-1 ), INT_ONE )
        ENDIF
*
...
*         Calculate contribution from this block to next diagonal block
*
          CALL ZGEMM( 'T', 'N', INT_ONE, NRHS, INT_ONE, -CONE,
     $                     AF( (ODD_SIZE)*1+1 ),
     $                     INT_ONE,
     $                     B( PART_OFFSET+ODD_SIZE+1 ),
     $                     LLDB, CZERO,
...
*           processor's solution.
*
          CALL ZGEMM( 'C', 'N', ODD_SIZE, NRHS, 1, -CONE, AF( 1 ),
     $                INT_ONE, WORK( 1 ), INT_ONE, CONE,
     $                B( PART_OFFSET+1 ), LLDB )
*
        ENDIF
*
