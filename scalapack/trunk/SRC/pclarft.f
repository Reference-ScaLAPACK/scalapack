      SUBROUTINE PCLARFT( DIRECT, STOREV, N, K, V, IV, JV, DESCV, TAU,
     $                    T, WORK )
*
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CCOPY, CGEMV, CGSUM2D,
     $                   CLACGV, CLASET, CTRMV, INFOG2L
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
               ...
               IF( NP-II+IIV.GT.0 ) THEN
                  CALL CGEMV( 'Conjugate transpose', NP-II+IIV, ITMP0,
...
*
   10       CONTINUE
*
            CALL CGSUM2D( ICTXT, 'Columnwise', ' ', IW-1, 1, WORK, IW-1,
     $                    IVROW, MYCOL )
*
            IF( MYROW.EQ.IVROW ) THEN
*
...
*
               ...
               IF( II-IIV+1.GT.0 ) THEN
                  CALL CGEMV( 'Conjugate transpose', II-IIV+1, ITMP0,
...
*
               ...
               IF( NQ-JJ+JJV.GT.0 ) THEN
                  CALL CLACGV( NQ-JJ+JJV, V( II+(JJ-1)*LDV ), LDV )
                  CALL CGEMV( 'No transpose', ITMP0, NQ-JJ+JJV,
     $                        -TAU(II), V( IIV+(JJ-1)*LDV ), LDV,
     $                        V( II+(JJ-1)*LDV ), LDV, ZERO,
     $                        WORK( IW ), 1 )
                  CALL CLACGV( NQ-JJ+JJV, V( II+(JJ-1)*LDV ), LDV )
               ELSE
                  CALL CLASET( 'All', ITMP0, 1, ZERO, ZERO,
     $                         WORK( IW ), ITMP0 )
               END IF
*
               IW = IW + ITMP0
...
*
               ...
               IF( JJ-JJV+1.GT.0 ) THEN
                  CALL CLACGV( JJ-JJV+1, V( II+(JJV-1)*LDV ), LDV )
                  CALL CGEMV( 'No transpose', ITMP0, JJ-JJV+1,
