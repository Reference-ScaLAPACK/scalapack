      SUBROUTINE DLARRV2_CV( N, VL, VU, D, L, PIVMIN,
...
*  =======
*
*  DLARRV2_CV computes the eigenvectors of the tridiagonal matrix
...
*          = 0:  successful exit
*
*          > 0:  A problem occured in DLARRV2_CV.
...
*     .. External Subroutines ..
      EXTERNAL           DAXPY, DCOPY, DLAR1VA_CV, DLARRB2_CV,
     $                   DLARRF2_CV, DLASET, DSCAL
*     ..
*     .. Intrinsic Functions ..
...
C                    DLARRF2 needs LWORK = 2*N
                     CALL DLARRF2_CV( IN, D( IBEGIN ), L( IBEGIN ),
     $                         WORK(INDLD+IBEGIN-1), 
     $                         SPLACE(1), SPLACE(2), 
...
*                    Given LAMBDA, compute the eigenvector.
                     CALL DLAR1VA_CV( IN, 1, IN, LAMBDA, D(IBEGIN),
...
*                             compute eigenvector and stop
     ...
     $                     BSTRES.LE.RESID) THEN
                        ...
                        ENDIF
                        IF (STP2II) THEN
                           CALL DLAR1VA_CV( IN, 1, IN, LAMBDA,
     $                          D( IBEGIN ), L( IBEGIN ), 
     $                          WORK(INDLD+IBEGIN-1), 
     $                          WORK(INDLLD+IBEGIN-1),
     $                          PIVMIN, GAPTOL, 
