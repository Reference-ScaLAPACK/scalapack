      DOUBLE PRECISION FUNCTION PZLANHE( NORM, UPLO, N, A, IA, JA,
     $                                   DESCA, WORK )
*
               ...
               DO 110 K = (JJ-1)*LDA, (JJ+IB-2)*LDA, LDA
                  IF( MYROW.EQ.IAROW ) THEN
                     IF( II.LE.IIA+NP-1 ) THEN
                        VALUE = MAX( VALUE, ABS( DBLE( A( II+K ) ) ) )
...
*
            IF( MYCOL.EQ.IACOL ) THEN
               DO 370 K = (JJ-1)*LDA, (JJ+IB-2)*LDA, LDA
                  CALL ZLASSQ( II-IIA, A( IIA+K ), 1, SCALE, SUM )
                  CALL ZLASSQ( II-IIA, A( IIA+K ), 1, SCALE, SUM )
                  IF( MYROW.EQ.IAROW ) THEN
                     IF( DBLE( A( II+K ) ).NE.ZERO ) THEN
                        ABSA = ABS( DBLE( A( II+K ) ) )
            ...
            IF( MYCOL.EQ.IACOL ) THEN
               DO 400 K = (JJ-1)*LDA, (JJ+IB-2)*LDA, LDA
                  IF( MYROW.EQ.IAROW ) THEN
                     ...
                     II = II + 1
                  END IF
                  CALL ZLASSQ( IIA+NP-II, A( II+K ), 1, SCALE, SUM )
                  CALL ZLASSQ( IIA+NP-II, A( II+K ), 1, SCALE, SUM )
  400          CONTINUE
*
               JJ = JJ + IB
