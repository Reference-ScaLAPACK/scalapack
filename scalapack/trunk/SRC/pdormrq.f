      SUBROUTINE PDORMRQ( SIDE, TRANS, M, N, K, A, IA, JA, DESCA, TAU,
...
*     .. External Subroutines ..
     ...
     $                   PDLARFT, PDORMR2, PB_TOPGET, PB_TOPSET, PXERBLA
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PDORMRQ', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
     ...
     $    ( .NOT.LEFT .AND. NOTRAN ) ) THEN
         ...
         ELSE
            NI = N - K + IB
         END IF
         CALL PDORMR2( SIDE, TRANS, MI, NI, IB, A, IA, JA, DESCA, TAU,
...
*
         CALL PDLARFT( 'Backward', 'Rowwise', NQ-K+I+IB-IA, IB,
...
*        Apply H or H'
*
         CALL PDLARFB( SIDE, TRANST, 'Backward', 'Rowwise', MI, NI,
