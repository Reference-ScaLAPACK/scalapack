...
*     .. External Subroutines ..
     ...
     $                   INFOG2L, PCHK2MAT, PXERBLA, PZAHEMV, PZAXPY,
     $                   PZCOPY, PZHEMV, PZPOTRS, PZLACON,
     $                   ZGEBR2D, ZGEBS2D
*     ..
*     .. Intrinsic Functions ..
...
*
     ...
     $                  IX, JX, DESCX, 19, 5, IDUM1, IDUM2, INFO )
      END IF
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PZPORFS', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
     ...
     $                DESCW, 1 )
         CALL PZHEMV( UPLO, N, -ONE, A, IA, JA, DESCA, X, IX, JX+K,
...
*
   ...
   30          CONTINUE
         ...
         END IF
*
         CALL PZAHEMV( UPLO, N, RONE, A, IA, JA, DESCA, X, IX, JX+K,
...
*
            CALL PZPOTRS( UPLO, N, 1, AF, IAF, JAF, DESCAF,
...
*
     ...
     $                   DESCW, 1 )
            CALL PZHEMV( UPLO, N, -ONE, A, IA, JA, DESCA, X, IX, J+K,
     $                  DESCX, 1, ONE, WORK( IPR ), IW, JW, DESCW, 1 )
*
*           Compute componentwise relative backward error from formula
*
...
*
  ...
  120             CONTINUE
            ...
            END IF
*
            CALL PZAHEMV( UPLO, N, RONE, A, IA, JA, DESCA, X, IX, J+K,
