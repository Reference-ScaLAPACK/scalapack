...
*     .. Array Arguments ..
     ...
     $                   DESCX( * ), IPIV( * ), IWORK( * )
      DOUBLE PRECISION   A( * ), AF( * ), B( * ), BERR( * ), C( * ),
     $                   FERR( * ), R( * ), WORK( * ), X( * )
*     ..
*
*  Purpose
...
*     .. External Subroutines ..
     ...
     $                   DGAMX2D, INFOG2L, PDCOPY, PDGECON,
     $                   PDGEEQU, PDGERFS, PDGETRF, PDGETRS,
     $                   PDLACPY, PDLAQGE, PXERBLA
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      INTEGER            ICEIL, ILCM, INDXG2P, NUMROC
      DOUBLE PRECISION   PDLAMCH, PDLANGE
      EXTERNAL           ICEIL, ILCM, INDXG2P, LSAME, NUMROC, PDLANGE,
     $                   PDLAMCH
*     ..
*     .. Intrinsic Functions ..
...
*
         CALL PDGEEQU( N, N, A, IA, JA, DESCA, R, C, ROWCND, COLCND,
     $                 AMAX, INFEQU )
         IF( INFEQU.EQ.0 ) THEN
*
...
*
     ...
     $                 IACOL, ICTXT, 1 )
         CALL PDCOPY( N, C, 1, JA, CDESC, CDESC( LLD_ ), WORK, IB, JB,
     $                DESCB, 1 )
         IF( MYCOL.EQ.IBCOL ) THEN
            CALL DGEBS2D( ICTXT, 'Rowwise', ' ', NP, 1, WORK( IIB ),
     $                    DESCB( LLD_ ) )
         ELSE
            CALL DGEBR2D( ICTXT, 'Rowwise', ' ', NP, 1, WORK( IIB ),
...
*
     ...
     $                 DESCAF )
         CALL PDGETRF( N, N, AF, IAF, JAF, DESCAF, IPIV, INFO )
*
...
*
      ...
      ELSE
         NORM = 'I'
      END IF
      ANORM = PDLANGE( NORM, N, N, A, IA, JA, DESCA, WORK )
*
*     Compute the reciprocal of the condition number of A.
*
      CALL PDGECON( NORM, N, AF, IAF, JAF, DESCAF, ANORM, RCOND, WORK,
...
*
     ...
     $              DESCX )
      CALL PDGETRS( TRANS, N, NRHS, AF, IAF, JAF, DESCAF, IPIV, X, IX,
     $              JX, DESCX, INFO )
*
*     Use iterative refinement to improve the computed solution and
*     compute error bounds and backward error estimates for it.
*
      CALL PDGERFS( TRANS, N, NRHS, A, IA, JA, DESCA, AF, IAF, JAF,
