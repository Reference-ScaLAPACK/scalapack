...
*     .. External Subroutines ..
     ...
     $                   DGERV2D, DGESD2D, DPTTRF, DPTTRSV, DTRRV2D,
...
*
      IF( N.GT.NP*NB-MOD( JA-1, NB ) ) THEN
         INFO = -( 1 )
         CALL PXERBLA( ICTXT, 'PDPTTRF, D&C alg.: only 1 block per proc'
     $                 , -INFO )
         RETURN
      END IF
*
      IF( ( JA+N-1.GT.NB ) .AND. ( NB.LT.2*INT_ONE ) ) THEN
         INFO = -( 5*100+4 )
         CALL PXERBLA( ICTXT, 'PDPTTRF, D&C alg.: NB too small', -INFO )
         RETURN
      END IF
*
*
...
*        put minimum value of laf into AF( 1 )
         AF( 1 ) = LAF_MIN
         CALL PXERBLA( ICTXT, 'PDPTTRF: auxiliary storage error ',
     $                 -INFO )
         RETURN
      END IF
*
...
*
      IF( LWORK.LT.WORK_SIZE_MIN ) THEN
         IF( LWORK.NE.-1 ) THEN
            INFO = -9
            CALL PXERBLA( ICTXT, 'PDPTTRF: worksize error ', -INFO )
         END IF
         RETURN
      END IF
*
