...
*  =======
*
*  PZPOTRS solves a system of linear equations
*
*                      sub( A ) * X = sub( B )
...
*          rization sub( A ) = L*L**H or U**H*U, as computed by PZPOTRF.
*
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PZPOTRS', -INFO )
         RETURN
      END IF
*
...
*
         CALL PZTRSM( 'Left', 'Lower', 'No transpose', 'Non-unit', N,
     $              NRHS, ONE, A, IA, JA, DESCA, B, IB, JB, DESCB )
*
*        Solve L'*X = sub( B ), overwriting sub( B ) with X.
*
