      SUBROUTINE PZGEQLF( M, N, A, IA, JA, DESCA, TAU, WORK, LWORK,
     $                    INFO )
*
...
*  =======
*
*  PZGEQLF computes a QL factorization of a complex distributed M-by-N
...
*     .. External Subroutines ..
     ...
     $                   PB_TOPSET, PXERBLA, PZGEQL2, PZLARFB,
     $                   PZLARFT
*     ..
*     .. External Functions ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PZGEQLF', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
            CALL PZGEQL2( M-N+J+JB-JA, JB, A, IA, J, DESCA, TAU, WORK,
     $                    LWORK, IINFO )
*
            IF( J.GT.JA ) THEN
*
...
*              left
*
               CALL PZLARFB( 'Left', 'Conjugate transpose', 'Backward',
...
*
     ...
     $   CALL PZGEQL2( MU, NU, A, IA, JA, DESCA, TAU, WORK, LWORK,
     $                 IINFO )
*
