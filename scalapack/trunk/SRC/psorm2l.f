      SUBROUTINE PSORM2L( SIDE, TRANS, M, N, K, A, IA, JA, DESCA, TAU,
...
*     .. External Subroutines ..
     ...
     $                   PSELSET, PSELSET2, PSLARF, PB_TOPGET,
     $                   PB_TOPSET, PXERBLA, SGEBR2D, SGEBS2D,
     $                   SGERV2D, SGESD2D, SSCAL
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
      ...
      END IF
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PSORM2L', -INFO )
...
*
     ...
     $                      NPCOL )
               IF( MYCOL.EQ.IACOL ) THEN
                  AJJ = ONE - TAU( JJ )
                  CALL SGEBS2D( ICTXT, 'Rowwise', ' ', 1, 1, AJJ, 1 )
                  CALL SSCAL( NQ-JCC+1, AJJ,
     $                        C( ICC+(JCC-1)*DESCC( LLD_ ) ),
     $                        DESCC( LLD_ ) )
               ELSE
                  CALL SGEBR2D( ICTXT, 'Rowwise', ' ', 1, 1, AJJ, 1,
     $                          IAROW, IACOL )
                  CALL SSCAL( NQ-JCC+1, AJJ,
     $                        C( ICC+(JCC-1)*DESCC( LLD_ ) ),
     $                        DESCC( LLD_ ) )
...
*
     ...
     $                      NPROW )
               CALL SSCAL( MP-ICC+1, AJJ, C( ICC+(JCC-1)*
     $                     DESCC( LLD_ ) ), 1 )
            END IF
*
         END IF
...
*           Apply H(j) or H(j)'
*
            CALL PSELSET2( AJJ, A, IA+NQ-K+J-JA, J, DESCA, ONE )
            CALL PSLARF( SIDE, MI, NI, A, IA, J, DESCA, 1, TAU, C, IC,
     $                   JC, DESCC, WORK )
            CALL PSELSET( A, IA+NQ-K+J-JA, J, DESCA, AJJ )
*
   10    CONTINUE
*
