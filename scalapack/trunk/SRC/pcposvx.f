      SUBROUTINE PCPOSVX( FACT, UPLO, N, NRHS, A, IA, JA, DESCA, AF,
...
*  =======
*
*  PCPOSVX uses the Cholesky factorization A = U**H*U or A = L*L**H to
...
*          output variable.
*
*  SR      (local input/local output) COMPLEX array,
*                                                    dimension (LLD_A)
...
*
*  X       (local input/local output) COMPLEX pointer into
*          the local memory to an array of local dimension
...
*     .. External Subroutines ..
     ...
     $                   PCPOCON, PCPOEQU,
     $                   PCPORFS, PCPOTRF, PCPOTRS,
     $                   PCLACPY, PCLAQSY, PXERBLA,
     $                   SGAMN2D, SGAMX2D
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PCPOSVX', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
         CALL PCPOEQU( N, A, IA, JA, DESCA, SR, SC, SCOND, AMAX,
     $                 INFEQU )
*
         IF( INFEQU.EQ.0 ) THEN
*
...
*
            CALL PCLAQSY( UPLO, N, A, IA, JA, DESCA, SR, SC, SCOND,
     $                    AMAX, EQUED )
            RCEQU = LSAME( EQUED, 'Y' )
         END IF
      END IF
...
*
     ...
     $                 DESCAF )
         CALL PCPOTRF( UPLO, N, AF, IAF, JAF, DESCAF, INFO )
*
...
*
      ANORM = PCLANHE( '1', UPLO, N, A, IA, JA, DESCA, RWORK )
*
*     Compute the reciprocal of the condition number of A.
*
      CALL PCPOCON( UPLO, N, AF, IAF, JAF, DESCAF, ANORM, RCOND, WORK,
...
*
     ...
     $              DESCX )
      CALL PCPOTRS( UPLO, N, NRHS, AF, IAF, JAF, DESCAF, X, IX, JX,
     $              DESCX, INFO )
*
*     Use iterative refinement to improve the computed solution and
*     compute error bounds and backward error estimates for it.
*
      CALL PCPORFS( UPLO, N, NRHS, A, IA, JA, DESCA, AF, IAF, JAF,
