...
*  =======
*
*  PZDTTRSV solves a tridiagonal triangular system of linear equations
*
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PZDTTRSV', -INFO )
         RETURN
      END IF
*
...
*
        CALL ZDTTRSV( UPLO, 'N', ODD_SIZE, NRHS, DL( PART_OFFSET+2 ),
...
*
            CALL ZAXPY( NRHS, -DL( PART_OFFSET+ODD_SIZE+1 ),
...
*
            CALL ZGEMM( 'C', 'N', INT_ONE, NRHS, ODD_SIZE, -CONE,
...
*
        IF( MYCOL .GT. 0) THEN
*
          CALL ZGESD2D( ICTXT, INT_ONE, NRHS,
     $                     WORK( 1 ), INT_ONE,
     $                     0, MYCOL - 1 )
*
        ENDIF
...
*
          CALL ZGERV2D( ICTXT, INT_ONE, NRHS,
     $                     WORK( 1 ), INT_ONE,
     $                     0, MYCOL + 1 )
*
*         Combine contribution to locally stored right hand sides
*
          CALL ZMATADD( INT_ONE, NRHS, CONE,
     $                WORK( 1 ), INT_ONE, CONE,
     $                B( PART_OFFSET+ODD_SIZE + 1 ), LLDB )
*
        ENDIF
...
*
        CALL ZTBTRS( 'L', 'N', 'U', INT_ONE, MIN( INT_ONE, INT_ONE-1 ),
...
*
          CALL ZGEMM( 'C', 'N', INT_ONE, NRHS, INT_ONE, -CONE,
...
*
          CALL ZGEMM( 'N', 'N', INT_ONE, NRHS, INT_ONE, -CONE,
...
*       Solve with diagonal block
*
        CALL ZTBTRS( 'L', 'C', 'U', INT_ONE, MIN( INT_ONE, INT_ONE-1 ),
     $               NRHS, AF( ODD_SIZE+2 ), INT_ONE+1,
     $               B( PART_OFFSET+ODD_SIZE+1 ), LLDB, INFO )
*
        IF( INFO.NE.0 ) THEN
...
*
          CALL ZGEMM( 'N', 'N', ODD_SIZE, NRHS, INT_ONE, -CONE, AF( 1 ),
...
*           locally stored portion of right hand side(s)
*
            CALL ZAXPY( NRHS, -DCONJG( DL( PART_OFFSET+ODD_SIZE+1 ) ),
     $                 B( PART_OFFSET+ODD_SIZE+1 ), LLDB,
     $                 B( PART_OFFSET+ODD_SIZE ), LLDB )
*
        ENDIF
...
*       Use main partition in each processor to solve locally
*
        CALL ZDTTRSV( UPLO, 'C', ODD_SIZE, NRHS, DL( PART_OFFSET+2 ),
     $                D( PART_OFFSET+1 ), DU( PART_OFFSET+1 ),
     $                B( PART_OFFSET+1 ), LLDB, INFO )
*
      ENDIF
...
*           locally stored portion of right hand side(s)
*
            CALL ZAXPY( NRHS, -DCONJG( DU( PART_OFFSET+ODD_SIZE ) ),
     $            B( PART_OFFSET+ODD_SIZE ), LLDB,
     $            B( PART_OFFSET+ODD_SIZE+1 ), LLDB )
*
        ENDIF
...
*       Solve with diagonal block
*
        CALL ZTBTRS( 'U', 'C', 'N', INT_ONE, MIN( INT_ONE, INT_ONE-1 ),
     $               NRHS, AF( ODD_SIZE+2 ), INT_ONE+1,
     $               B( PART_OFFSET+ODD_SIZE+1 ), LLDB, INFO )
*
        IF( INFO.NE.0 ) THEN
...
*       Solve with diagonal block
*
        CALL ZTBTRS( 'U', 'N', 'N', INT_ONE, MIN( INT_ONE, INT_ONE-1 ),
     $               NRHS, AF( ODD_SIZE+2 ), INT_ONE+1,
     $               B( PART_OFFSET+ODD_SIZE+1 ), LLDB, INFO )
*
        IF( INFO.NE.0 ) THEN
...
*           processor's solution.
*
          CALL ZGEMM( 'N', 'N', ODD_SIZE, NRHS, INT_ONE, -CONE,
...
*           locally stored portion of right hand side(s)
*
            CALL ZAXPY( NRHS, -( DU( PART_OFFSET+ODD_SIZE ) ),
     $                 B( PART_OFFSET+ODD_SIZE+1 ), LLDB,
     $                 B( PART_OFFSET+ODD_SIZE ), LLDB )
*
        ENDIF
...
*       Use main partition in each processor to solve locally
*
        CALL ZDTTRSV( UPLO, 'N', ODD_SIZE, NRHS, DU( PART_OFFSET+2 ),
     $                D( PART_OFFSET+1 ), DU( PART_OFFSET+1 ),
     $                B( PART_OFFSET+1 ), LLDB, INFO )
*
      ENDIF
