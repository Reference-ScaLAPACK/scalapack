...
*  =======
*
*  PCPBTRSV solves a banded triangular system of linear equations
*
...
*  Cholesky factorization code PCPBTRF
...
*
     ...
     $                WORK( 1 ), BW )
*
            CALL CTRMM( 'L', 'U', 'N', 'N', BW, NRHS, -CONE,
     $                  A(( OFST+(BW+1)+(ODD_SIZE-BW)*LLDA )), LLDA-1,
     $                  WORK( 1 ), BW )
*
            CALL CMATADD( BW, NRHS, CONE, WORK( 1 ), BW,
...
*
            CALL CGEMM( 'C', 'N', BW, NRHS, ODD_SIZE, -CONE, AF( 1 ),
...
*
        CALL CTRTRS( 'L', 'N', 'N', BW, NRHS, AF( ODD_SIZE*BW+MBW2+1 ),
...
*
          CALL CGEMM( 'C', 'N', BW, NRHS, BW, -CONE,
     $                     AF( (ODD_SIZE)*BW+1 ),
     $                     BW,
...
*
          CALL CGEMM( 'N', 'N', BW, NRHS, BW, -CONE,
...
*       Solve with diagonal block
*
        CALL CTRTRS( 'L', 'C', 'N', BW, NRHS, AF( ODD_SIZE*BW+MBW2+1 ),
     $               BW, B( PART_OFFSET+ODD_SIZE+1 ), LLDB, INFO )
*
        IF( INFO.NE.0 ) THEN
          GO TO 1000
...
*
          CALL CGEMM( 'N', 'N', ODD_SIZE, NRHS, BW, -CONE, AF( 1 ),
...
*
     ...
     $                WORK( 1+BW-BW ), BW )
*
          CALL CMATADD( BW, NRHS, CONE, WORK( 1+BW-BW ), BW, CONE,
     $                  B( PART_OFFSET+ODD_SIZE-BW+1 ), LLDB )
*
        ENDIF
*
...
*
     ...
     $                WORK( 1 ), BW )
*
            CALL CTRMM( 'L', 'L', 'C', 'N', BW, NRHS, -CONE,
