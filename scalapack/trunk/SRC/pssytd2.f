      SUBROUTINE PSSYTD2( UPLO, N, A, IA, JA, DESCA, D, E, TAU, WORK,
     $                    LWORK, INFO )
*
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PSSYTD2', -INFO )
...
*
                     CALL SSYMV( UPLO, J, TAUI, A( II+(JJ-1)*LDA ),
...
*                       A := A - v * w' - w * v'
*
                     CALL SSYR2( UPLO, J, -ONE, A( II+JK*LDA ), 1,
...
*
   10          CONTINUE
               ...
               WORK( 2*N+1 ) = ZERO
*
               CALL SGEBS2D( ICTXT, 'Columnwise', ' ', 1, 3*N, WORK, 1 )
*
            ELSE
               CALL SGEBR2D( ICTXT, 'Columnwise', ' ', 1, 3*N, WORK, 1,
     $                       IAROW, IACOL )
               DO 20 J = 2, N
                  JN = JJ + J - 1
                  D( JN ) = WORK( J )
...
*
                     CALL SSYMV( UPLO, N-J, TAUI, A( IK+1+JK*LDA ),
...
*
   ...
   30          CONTINUE
               ...
               WORK( 2*N ) = ZERO
*
               CALL SGEBS2D( ICTXT, 'Columnwise', ' ', 1, 3*N-1, WORK,
     $                            1 )
*
            ELSE
               CALL SGEBR2D( ICTXT, 'Columnwise', ' ', 1, 3*N-1, WORK,
     $                       1, IAROW, IACOL )
               DO 40 J = 1, N - 1
                  JN = JJ + J - 1
                  D( JN ) = WORK( J )
