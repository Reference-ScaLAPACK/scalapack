...
*     .. External Subroutines ..
     ...
     $                   IGEBS2D, INFOG2L, PB_TOPGET, PXERBLA, SGEMV,
     $                   SSCAL
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MOD, SQRT
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PSPOTF2', -INFO )
...
*
                  IF( J.LT.JA+N-1 ) THEN
                     ICURR = IDIAG + LDA
                     CALL SGEMV( 'Transpose', J-JA, JA+N-J-1, -ONE,
     $                           A( IOFFA+LDA ), LDA, A( IOFFA ), 1,
     $                           ONE, A( ICURR ), LDA )
                     CALL SSCAL( N-J+JA-1, ONE / AJJ, A( ICURR ), LDA )
                  END IF
                  IDIAG = IDIAG + LDA + 1
                  IOFFA = IOFFA + LDA
   10          CONTINUE
...
*
                  AJJ = A( IDIAG ) -
     $                  SDOT( J-JA, A( IOFFA ), LDA, A( IOFFA ), LDA )
                  IF ( AJJ.LE.ZERO ) THEN
                     A( IDIAG ) = AJJ
                     INFO = J - JA + 1
                     GO TO 40
...
*
                  IF( J.LT.JA+N-1 ) THEN
                     ICURR = IDIAG + 1
                     CALL SGEMV( 'No transpose', JA+N-J-1, J-JA, -ONE,
     $                           A( IOFFA+1 ), LDA, A( IOFFA ), LDA,
     $                           ONE, A( ICURR ), 1 )
                     CALL SSCAL( JA+N-J-1, ONE / AJJ, A( ICURR ), 1 )
                  END IF
                  IDIAG = IDIAG + LDA + 1
                  IOFFA = IOFFA + 1
   30          CONTINUE
