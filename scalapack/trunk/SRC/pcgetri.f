...
*          factorization sub( A ) = P*L*U computed by PCGETRF. On
...
*     .. External Subroutines ..
     ...
     $                   PCGEMM, PCLACPY, PCLASET, PCLAPIV,
     $                   PCTRSM, PCTRTRI, PXERBLA
*     ..
*     .. External Functions ..
...
*
         CALL PCLACPY( 'Lower', JA+N-1-J, JB, A, I+1, J, DESCA,
     $                 WORK, IW+J-JA+1, 1, DESCW )
         CALL PCLASET( 'Lower', JA+N-1-J, JB, ZERO, ZERO, A, I+1, J,
     $                 DESCA )
*
*        Compute current block column of inv(A).
*
         IF( J+JB.LE.JA+N-1 )
     $      CALL PCGEMM( 'No transpose', 'No transpose', N, JB,
     $                   JA+N-J-JB, -ONE, A, IA, J+JB, DESCA, WORK,
     $                   IW+J+JB-JA, 1, DESCW, ONE, A, IA, J, DESCA )
         CALL PCTRSM( 'Right', 'Lower', 'No transpose', 'Unit', N, JB,
...
*     Copy current block column of L to WORK and replace with zeros.
*
      CALL PCLACPY( 'Lower', N-1, JB, A, IA+1, JA, DESCA, WORK, IW+1,
     $              1, DESCW )
      CALL PCLASET( 'Lower', N-1, JB, ZERO, ZERO, A, IA+1, JA, DESCA )
*
*     Compute current block column of inv(A).
*
      IF( JA+JB.LE.JA+N-1 )
...
*
     ...
     $              MP+DESCA( MB_ ) )
      CALL PCLAPIV( 'Backward', 'Columns', 'Column', N, N, A, IA,
