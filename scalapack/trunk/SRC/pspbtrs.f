      SUBROUTINE PSPBTRS( UPLO, N, BW, NRHS, A, JA, DESCA, B, IB, DESCB,
...
*  =======
*
*  PSPBTRS solves a system of linear equations
*
...
*     .. External Subroutines ..
     ...
     $                   GLOBCHK, PSPBTRSV, PXERBLA, RESHAPE
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      EXTERNAL           LSAME
...
*
      IF( N.GT.NP*NB-MOD( JA-1, NB ) ) THEN
         INFO = -( 2 )
         CALL PXERBLA( ICTXT, 'PSPBTRS, D&C alg.: only 1 block per proc'
     $                 , -INFO )
         RETURN
      END IF
*
      IF( ( JA+N-1.GT.NB ) .AND. ( NB.LT.2*BW ) ) THEN
         INFO = -( 7*100+4 )
         CALL PXERBLA( ICTXT, 'PSPBTRS, D&C alg.: NB too small', -INFO )
         RETURN
      END IF
*
*
...
*
      IF( LWORK.LT.WORK_SIZE_MIN ) THEN
         IF( LWORK.NE.-1 ) THEN
            INFO = -14
            CALL PXERBLA( ICTXT, 'PSPBTRS: worksize error', -INFO )
         END IF
         RETURN
      END IF
*
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PSPBTRS', -INFO )
         RETURN
      END IF
*
...
*
         CALL PSPBTRSV( 'L', 'N', N, BW, NRHS, A( PART_OFFSET+1 ),
     $                  JA_NEW, DESCA_1XP, B, IB, DESCB_PX1, AF, LAF,
     $                  WORK, LWORK, INFO )
*
      ELSE
*
         CALL PSPBTRSV( 'U', 'T', N, BW, NRHS, A( PART_OFFSET+1 ),
     $                  JA_NEW, DESCA_1XP, B, IB, DESCB_PX1, AF, LAF,
     $                  WORK, LWORK, INFO )
*
      END IF
...
*
      IF( LSAME( UPLO, 'L' ) ) THEN
*
         CALL PSPBTRSV( 'L', 'T', N, BW, NRHS, A( PART_OFFSET+1 ),
     $                  JA_NEW, DESCA_1XP, B, IB, DESCB_PX1, AF, LAF,
     $                  WORK, LWORK, INFO )
*
      ELSE
*
         CALL PSPBTRSV( 'U', 'N', N, BW, NRHS, A( PART_OFFSET+1 ),
     $                  JA_NEW, DESCA_1XP, B, IB, DESCB_PX1, AF, LAF,
     $                  WORK, LWORK, INFO )
*
      END IF
