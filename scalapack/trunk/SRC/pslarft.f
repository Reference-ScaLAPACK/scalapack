      SUBROUTINE PSLARFT( DIRECT, STOREV, N, K, V, IV, JV, DESCV, TAU,
     $                    T, WORK )
*
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, INFOG2L, SCOPY, SGEMV,
     $                   SGSUM2D, SLASET, STRMV
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
               ...
               IF( NP-II+IIV.GT.0 ) THEN
                  CALL SGEMV( 'Transpose', NP-II+IIV, ITMP0,
...
*
   10       CONTINUE
*
            CALL SGSUM2D( ICTXT, 'Columnwise', ' ', IW-1, 1, WORK, IW-1,
     $                    IVROW, MYCOL )
*
            IF( MYROW.EQ.IVROW ) THEN
*
...
*
               ...
               IF( II-IIV+1.GT.0 ) THEN
                  CALL SGEMV( 'Transpose', II-IIV+1, ITMP0, -TAU( JJ ),
     $                        V( IIV+JJ*LDV ), LDV,
...
*
               ...
               IF( NQ-JJ+JJV.GT.0 ) THEN
                  CALL SGEMV( 'No transpose', ITMP0, NQ-JJ+JJV,
     $                        -TAU(II), V( IIV+(JJ-1)*LDV ), LDV,
     $                        V( II+(JJ-1)*LDV ), LDV, ZERO,
     $                        WORK( IW ), 1 )
               ELSE
                  CALL SLASET( 'All', ITMP0, 1, ZERO, ZERO,
     $                         WORK( IW ), ITMP0 )
               END IF
*
               IW = IW + ITMP0
...
*
               ...
               IF( JJ-JJV+1.GT.0 ) THEN
                  CALL SGEMV( 'No transpose', ITMP0, JJ-JJV+1,
