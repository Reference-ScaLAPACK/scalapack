      SUBROUTINE PCHETD2( UPLO, N, A, IA, JA, DESCA, D, E, TAU, WORK,
     $                    LWORK, INFO )
*
...
*  =======
*
*  PCHETD2 reduces a complex Hermitian matrix sub( A ) to Hermitian
...
*
            ELSE
               CALL CGEBR2D( ICTXT, 'Columnwise', ' ', 1, 3*N, WORK, 1,
     $                       IAROW, IACOL )
               DO 20 J = 2, N
                  JN = JJ + J - 1
                  D( JN ) = REAL( WORK( J ) )
...
*
                  ...
                  CALL CLARFG( N-J, ALPHA, A( IK+2+(JK-1)*LDA ), 1,
     $                         TAUI )
                  E( JK ) = REAL( ALPHA )
*
                  IF( TAUI.NE.ZERO ) THEN
...
*
   ...
   30          CONTINUE
               ...
               WORK( 2*N ) = ZERO
*
               CALL CGEBS2D( ICTXT, 'Columnwise', ' ', 1, 3*N-1, WORK,
     $                            1 )
*
            ELSE
