*
*
      SUBROUTINE PZHEGS2( IBTYPE, UPLO, N, A, IA, JA, DESCA, B, IB, JB,
     $                    DESCB, INFO )
*
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_EXIT, BLACS_GRIDINFO, CHK1MAT, INFOG2L,
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PZHEGS2', -INFO )
...
*
               AKK = DBLE( A( IOFFA-LDA ) )
               BKK = DBLE( B( IOFFB-LDB ) )
               AKK = AKK / BKK**2
               A( IOFFA-LDA ) = AKK
               IF( K.LT.N ) THEN
                  CALL ZDSCAL( N-K, ONE / BKK, A( IOFFA ), LDA )
                  CT = -HALF*AKK
                  CALL ZLACGV( N-K, A( IOFFA ), LDA )
                  CALL ZLACGV( N-K, B( IOFFB ), LDB )
                  CALL ZAXPY( N-K, CT, B( IOFFB ), LDB, A( IOFFA ),
     $                        LDA )
                  CALL ZHER2( UPLO, N-K, -CONE, A( IOFFA ), LDA,
...
*
               IF( K.LT.N ) THEN
                  CALL ZDSCAL( N-K, ONE / BKK, A( IOFFA ), 1 )
                  CT = -HALF*AKK
                  CALL ZAXPY( N-K, CT, B( IOFFB ), 1, A( IOFFA ), 1 )
                  CALL ZHER2( UPLO, N-K, -CONE, A( IOFFA ), 1,
...
*
               AKK = DBLE( A( IOFFA+K-1 ) )
               BKK = DBLE( B( IOFFB+K-1 ) )
               CALL ZTRMV( UPLO, 'No transpose', 'Non-unit', K-1,
     $                     B( IIB+( JJB-1 )*LDB ), LDB, A( IOFFA ), 1 )
               CT = HALF*AKK
               CALL ZAXPY( K-1, CT, B( IOFFB ), 1, A( IOFFA ), 1 )
               CALL ZHER2( UPLO, K-1, CONE, A( IOFFA ), 1, B( IOFFB ),
     $                     1, A( IIA+( JJA-1 )*LDA ), LDA )
               CALL ZAXPY( K-1, CT, B( IOFFB ), 1, A( IOFFA ), 1 )
               CALL ZDSCAL( K-1, BKK, A( IOFFA ), 1 )
...
*
               AKK = DBLE( A( IOFFA+( K-1 )*LDA ) )
               BKK = DBLE( B( IOFFB+( K-1 )*LDB ) )
               CALL ZLACGV( K-1, A( IOFFA ), LDA )
               CALL ZTRMV( UPLO, 'Conjugate transpose', 'Non-unit', K-1,
     $                     B( IIB+( JJB-1 )*LDB ), LDB, A( IOFFA ),
     $                     LDA )
               CT = HALF*AKK
               CALL ZLACGV( K-1, B( IOFFB ), LDB )
               CALL ZAXPY( K-1, CT, B( IOFFB ), LDB, A( IOFFA ), LDA )
               CALL ZHER2( UPLO, K-1, CONE, A( IOFFA ), LDA, B( IOFFB ),
     $                     LDB, A( IIA+( JJA-1 )*LDA ), LDA )
               CALL ZAXPY( K-1, CT, B( IOFFB ), LDB, A( IOFFA ), LDA )
               CALL ZLACGV( K-1, B( IOFFB ), LDB )
               CALL ZDSCAL( K-1, BKK, A( IOFFA ), LDA )
               CALL ZLACGV( K-1, A( IOFFA ), LDA )
