      SUBROUTINE PSORMTR( SIDE, UPLO, TRANS, M, N, A, IA, JA, DESCA,
...
*          define the elementary reflectors, as returned by PSSYTRD.
...
*
*  TAU     (local input) REAL array, dimension LTAU, where
...
*          reflector H(i), as returned by PSSYTRD. TAU is tied to the
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, PCHK2MAT, PSORMQL,
     $                   PSORMQR, PXERBLA
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PSORMTR', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
      IF( UPPER ) THEN
*
*        Q was determined by a call to PSSYTRD with UPLO = 'U'
*
         CALL PSORMQL( SIDE, TRANS, MI, NI, NQ-1, A, IAA, JAA, DESCA,
     $                 TAU, C, ICC, JCC, DESCC, WORK, LWORK, IINFO )
*
      ELSE
*
*        Q was determined by a call to PSSYTRD with UPLO = 'L'
*
         CALL PSORMQR( SIDE, TRANS, MI, NI, NQ-1, A, IAA, JAA, DESCA,
     $                 TAU, C, ICC, JCC, DESCC, WORK, LWORK, IINFO )
*
      END IF
*
