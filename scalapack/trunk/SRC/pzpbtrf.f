...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PZPBTRF', -INFO )
         RETURN
      END IF
*
...
*
          CALL ZTRSD2D( ICTXT, 'U', 'N', NEXT_TRI_SIZE_M,
...
*
        CALL ZPBTRF( UPLO, ODD_SIZE, BW, A( OFST + 1),
     $                    LLDA, INFO )
*
        IF( INFO.NE.0 ) THEN
           INFO = MYCOL+1
...
*           in main storage.
*
          CALL ZLATCPY( 'L', BW, BW, AF( ODD_SIZE*BW+2*MBW2+1+BW-BW ),
...
*
*
          CALL ZTRRV2D( ICTXT, 'U', 'N', PREV_TRI_SIZE_M,
...
*
          CALL ZTBTRS( 'L', 'N', 'N', ODD_SIZE, BW, BW, A( OFST + 1 ),
     $                 LLDA, AF( 1 ), ODD_SIZE, INFO )
*
*
*         Calculate the update block for previous proc, E_i = G_i{G_i}^C
*
          CALL ZHERK( 'L', 'C', BW, ODD_SIZE,
     $                     -ONE, AF( 1 ), ODD_SIZE, ZERO,
     $                     AF( 1 + (ODD_SIZE+2*BW)*BW), BW )
*
*
*         Initiate send of E_i to previous processor to overlap
*           with next computation.
*
          CALL ZGESD2D( ICTXT, BW, BW, AF( ODD_SIZE*BW+2*MBW2+1 ), BW,
     $                  0, MYCOL-1 )
*
*
          IF ( MYCOL .LT. NP-1 ) THEN
...
*             H_i^C to H_i.
*
            CALL ZLATCPY( 'N', BW, BW,
     $               AF( ODD_SIZE-BW+1 ), ODD_SIZE,
     $               AF( (ODD_SIZE)*BW+1), BW  )
*
            CALL ZTRMM( 'R', 'U', 'C', 'N', BW, BW, -CONE,
     $                  A( ( OFST+(BW+1)+(ODD_SIZE-BW)*LLDA ) ), LLDA-1,
     $                  AF( (ODD_SIZE)*BW+1 ), BW )
*
*
...
*
           CALL ZAXPY( MBW2, CONE,
     $               AF( ODD_SIZE*BW+2*MBW2+1 ),
     $               1, AF( ODD_SIZE*BW+MBW2+1 ), 1 )
*
        ENDIF
*
*
...
*
          IF( MYCOL-LEVEL_DIST .GE. 0 ) THEN
            CALL ZGERV2D( ICTXT, BW, BW, WORK( 1 ),
     $                     BW, 0, MYCOL-LEVEL_DIST )
*
            CALL ZAXPY( MBW2, CONE, WORK( 1 ), 1,
...
*
        CALL ZPOTRF( 'L', BW, AF( ODD_SIZE*BW+MBW2+1 ),
     $                    BW, INFO )
*
        IF( INFO.NE.0 ) THEN
               INFO = NPCOL + MYCOL
...
*
          CALL ZHERK( 'L', 'C', BW, BW, -ONE,
     $                  AF( (ODD_SIZE)*BW+1 ), BW, ZERO,
     $                  WORK( 1 ), BW )
*
*         Send contribution to diagonal block's owning processor.
*
          CALL ZGESD2D( ICTXT, BW, BW, WORK( 1 ), BW,
     $                     0, MYCOL+LEVEL_DIST )
*
        ENDIF
...
*
              CALL ZGEMM( 'N', 'N', BW, BW, BW, -CONE,
     $                    AF( ODD_SIZE*BW+2*MBW2+1 ), BW,
     $                    AF( ODD_SIZE*BW+1 ), BW, CZERO, WORK( 1 ),
     $                    BW )
*
...
*
          CALL ZLATCPY( 'L', PREV_TRI_SIZE_N, PREV_TRI_SIZE_M,
     $                  A( OFST+1 ), LLDA-1, AF( 1 ), ODD_SIZE )
*
          IF ( INFO.EQ.0 ) THEN
*
          CALL ZTBTRS( 'U', 'C', 'N', ODD_SIZE, BW, BW,
     $                    A( OFST + 1 ), LLDA,
     $                    AF( 1 ), ODD_SIZE, INFO )
*
*
