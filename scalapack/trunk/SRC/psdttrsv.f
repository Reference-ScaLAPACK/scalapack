...
*  =======
*
*  PSDTTRSV solves a tridiagonal triangular system of linear equations
*
...
*  Gaussian elimination code PS@(dom_pre)TTRF
...
*     .. External Subroutines ..
     ...
     $                   GLOBCHK, PXERBLA, RESHAPE, SAXPY, SDTTRSV,
     $                   SGEMM, SGERV2D, SGESD2D, SMATADD, STBTRS
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      INTEGER            NUMROC
...
*
      IF( ( JA+N-1.GT.NB ) .AND. ( NB.LT.2*INT_ONE ) ) THEN
         INFO = -( 9*100+4 )
         CALL PXERBLA( ICTXT, 'PSDTTRSV, D&C alg.: NB too small',
     $                 -INFO )
         RETURN
      END IF
*
...
*
      IF( LWORK.LT.WORK_SIZE_MIN ) THEN
         IF( LWORK.NE.-1 ) THEN
            INFO = -16
            CALL PXERBLA( ICTXT, 'PSDTTRSV: worksize error', -INFO )
         END IF
         RETURN
      END IF
*
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PSDTTRSV', -INFO )
         RETURN
      END IF
*
...
*
            CALL SDTTRSV( UPLO, 'N', ODD_SIZE, NRHS,
...
*
               CALL SAXPY( NRHS, -DL( PART_OFFSET+ODD_SIZE+1 ),
...
*
               CALL SGEMM( 'T', 'N', INT_ONE, NRHS, ODD_SIZE, -ONE,
...
*
            IF( MYCOL.GT.0 ) THEN
*
               CALL SGESD2D( ICTXT, INT_ONE, NRHS, WORK( 1 ), INT_ONE,
     $                       0, MYCOL-1 )
*
            END IF
*
...
*
               CALL SGERV2D( ICTXT, INT_ONE, NRHS, WORK( 1 ), INT_ONE,
     $                       0, MYCOL+1 )
*
*         Combine contribution to locally stored right hand sides
*
               CALL SMATADD( INT_ONE, NRHS, ONE, WORK( 1 ), INT_ONE,
...
*
               CALL SGEMM( 'T', 'N', INT_ONE, NRHS, INT_ONE, -ONE,
...
*
               CALL SGEMM( 'N', 'N', INT_ONE, NRHS, INT_ONE, -ONE,
...
*
            IF( MYCOL+LEVEL_DIST.LT.NPCOL-1 ) THEN
*
               CALL SGESD2D( ICTXT, INT_ONE, NRHS,
...
*
               CALL SGEMM( 'N', 'N', ODD_SIZE, NRHS, INT_ONE, -ONE,
...
*       Use main partition in each processor to solve locally
*
            CALL SDTTRSV( UPLO, 'T', ODD_SIZE, NRHS,
     $                    DL( PART_OFFSET+2 ), D( PART_OFFSET+1 ),
     $                    DU( PART_OFFSET+1 ), B( PART_OFFSET+1 ), LLDB,
     $                    INFO )
*
...
*           locally stored portion of right hand side(s)
*
               CALL SAXPY( NRHS, -( DU( PART_OFFSET+ODD_SIZE ) ),
     $                     B( PART_OFFSET+ODD_SIZE ), LLDB,
     $                     B( PART_OFFSET+ODD_SIZE+1 ), LLDB )
*
            END IF
