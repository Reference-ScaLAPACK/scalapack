      SUBROUTINE PCPBSV( UPLO, N, BW, NRHS, A, JA, DESCA, B, IB, DESCB,
     $                   WORK, LWORK, INFO )
*
*
*
...
*     .. External Subroutines ..
      EXTERNAL           PCPBTRF, PCPBTRS, PXERBLA
*     ..
...
*     Factor the matrix
*
      CALL PCPBTRF( UPLO, N, BW, A, JA, DESCA, WORK,
...
*
      IF( INFO.NE.0 ) THEN
         IF( INFO .LT. 0 ) THEN
            CALL PXERBLA( ICTXT, 'PCPBSV', -INFO )
         ENDIF
         RETURN
      END IF
*
...
*
      CALL PCPBTRS( UPLO, N, BW, NRHS, A, JA, DESCA, B, IB, DESCB, WORK,
     $              MIN( LWORK, WS_FACTOR ), WORK( 1+WS_FACTOR),
     $              LWORK-WS_FACTOR, INFO )
*
*     Check info for error conditions
