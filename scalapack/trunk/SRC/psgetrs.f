...
*  =======
*
*  PSGETRS solves a system of distributed linear equations
*
*                   op( sub( A ) ) * X = sub( B )
*
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PSGETRS', -INFO )
         RETURN
      END IF
*
...
*
         CALL PSTRSM( 'Left', 'Lower', 'No transpose', 'Unit', N, NRHS,
...
*        Solve L'*X = sub( B ), overwriting sub( B ) with X.
*
         CALL PSTRSM( 'Left', 'Lower', 'Transpose', 'Unit', N, NRHS,
     $                ONE, A, IA, JA, DESCA, B, IB, JB, DESCB )
*
