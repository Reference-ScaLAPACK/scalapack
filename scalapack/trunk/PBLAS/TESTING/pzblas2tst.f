...
*  =======
*
*  PZBLA2TST is the main testing program for the PBLAS Level 2 routines.
*
...
*     .. External Subroutines ..
     ...
     $                   PMDIMCHK, PVDESCCHK, PVDIMCHK, PZBLA2TSTINFO,
     $                   PZBLAS2TSTCHK, PZBLAS2TSTCHKE, PZCHKARG2,
     $                   PZCHKVOUT, PZGEMV, PZGERC, PZGERU, PZHEMV,
     $                   PZHER, PZHER2, PZIPSET, PZLAGEN, PZLASCAL,
     $                   PZLASET, PZMPRNT, PZTRMV, PZTRSV, PZVPRNT
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      INTEGER            PB_FCEIL
...
*
      ...
      CALL PZBLA2TSTINFO( OUTFILE, NOUT, NTESTS, DIAGVAL, TRANVAL,
...
*
      IF( TEE )
     $   CALL PZBLAS2TSTCHKE( LTEST, NOUT, NPROCS )
*
...
*
               INFO = 0
               CALL PZCHKARG2( ICTXT, NOUT, SNAMES( K ), UPLO, TRANS,
...
*
     ...
     $                              NOUT, MEM( IPG ) )
               ELSE IF( IVERB.GE.3 ) THEN
                  CALL PB_PZLAPRNT( MA, NA, MEM( IPA ), 1, 1, DESCA, 0,
...
*
               IF( IVERB.EQ.2 ) THEN
                  IF( INCX.EQ.DESCX( M_ ) ) THEN
                     CALL PB_PZLAPRNT( 1, NLX, MEM( IPX ), IX, JX,
     $                                 DESCX, 0, 0,
     $                                 'PARALLEL_INITIAL_X', NOUT,
     $                                 MEM( IPG ) )
                  ELSE
                     CALL PB_PZLAPRNT( NLX, 1, MEM( IPX ), IX, JX,
     $                                 DESCX, 0, 0,
     $                                 'PARALLEL_INITIAL_X', NOUT,
     $                                 MEM( IPG ) )
                  END IF
               ...
               IF( YCHECK( K ) ) THEN
                  IF( IVERB.EQ.2 ) THEN
                     IF( INCY.EQ.DESCY( M_ ) ) THEN
                        CALL PB_PZLAPRNT( 1, NLY, MEM( IPY ), IY, JY,
     $                                    DESCY, 0, 0,
     $                                    'PARALLEL_INITIAL_Y', NOUT,
     $                                    MEM( IPG ) )
                     ELSE
                        CALL PB_PZLAPRNT( NLY, 1, MEM( IPY ), IY, JY,
     $                                    DESCY, 0, 0,
     $                                    'PARALLEL_INITIAL_Y', NOUT,
     $                                    MEM( IPG ) )
                     END IF
...
*                 Test PZHEMV
*
                  CALL PZIPSET( 'Bignum', N, MEM( IPA ), IA, JA, DESCA )
*
                  CALL PZHEMV( UPLO, N, ALPHA, MEM( IPA ), IA, JA,
     $                         DESCA, MEM( IPX ), IX, JX, DESCX, INCX,
     $                         BETA, MEM( IPY ), IY, JY, DESCY, INCY )
*
                  CALL PZIPSET( 'Zero', N, MEM( IPA ), IA, JA, DESCA )
*
               ELSE IF( K.EQ.3 ) THEN
*
*                 Test PZTRMV
...
*                 Test PZHER
*
                  IF( DCMPLX( DBLE( ALPHA ) ).NE.ZERO )
     $               CALL PZIPSET( 'Bignum', N, MEM( IPA ), IA, JA,
     $                             DESCA )
*
...
*              Check padding
*
               CALL PB_ZCHEKPAD( ICTXT, SNAMES( K ), MPA, NQA,
...
*
               CALL PZBLAS2TSTCHK( ICTXT, NOUT, K, UPLO, TRANS, DIAG, M,
...
*
               CALL PZCHKMOUT( NROWA, NCOLA, MEM( IPMATA ), MEM( IPA ),
     $                         IA, JA, DESCA, IERR( 4 ) )
               CALL PZCHKVOUT( NLX, MEM( IPMATX ), MEM( IPX ), IX, JX,
     $                         DESCX, INCX, IERR( 5 ) )
*
               IF( IERR( 4 ).NE.0 ) THEN
                  IF( IAM.EQ.0 )
...
*
               IF( YCHECK( K ) ) THEN
                  CALL PZCHKVOUT( NLY, MEM( IPMATY ), MEM( IPY ), IY,
...
*
      END
      SUBROUTINE PZBLA2TSTINFO( SUMMRY, NOUT, NMAT, DIAGVAL, TRANVAL,
...
*
      END
      SUBROUTINE PZCHKARG2( ICTXT, NOUT, SNAME, UPLO, TRANS, DIAG, M,
...
*
      END
      SUBROUTINE PZBLAS2TSTCHK( ICTXT, NOUT, NROUT, UPLO, TRANS, DIAG,
