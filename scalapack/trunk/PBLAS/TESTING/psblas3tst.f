...
*  =======
*
*  PSBLA3TST is the main testing program for the Level 3 PBLAS routines.
*
...
*     .. External Subroutines ..
     ...
     $                   PMDIMCHK, PSBLA3TSTINFO, PSBLAS3TSTCHK,
     $                   PSBLAS3TSTCHKE, PSCHKARG3, PSCHKMOUT, PSGEADD,
     $                   PSGEMM, PSLAGEN, PSLASCAL, PSLASET, PSMPRNT,
     $                   PSSYMM, PSSYR2K, PSSYRK, PSTRADD, PSTRMM,
     $                   PSTRSM
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
      ...
      CALL PSBLA3TSTINFO( OUTFILE, NOUT, NTESTS, DIAGVAL, SIDEVAL,
...
*
      IF( TEE )
     $   CALL PSBLAS3TSTCHKE( LTEST, NOUT, NPROCS )
*
...
*
            IPW = IPG + 2*MAX( M, MAX( N, K ) )
            MEMREQD = IPW - 1 + MAX( MAX( MAX( IMBA, MBA ),
     $                                    MAX( IMBB, MBB ) ),
     $                               MAX( IMBC, MBC ) )
            IERR( 1 ) = 0
...
*
               END IF
*
               CALL PSLAGEN( .FALSE., AFORM, ADIAGDO, OFFDA, MA, NA,
     $                       1, 1, DESCA, IASEED, MEM( IPA ),
     $                       DESCA( LLD_ ) )
*
               IF( BCHECK( L ) )
     $            CALL PSLAGEN( .FALSE., 'None', 'No diag', 0, MB, NB,
     $                          1, 1, DESCB, IBSEED, MEM( IPB ),
     $                          DESCB( LLD_ ) )
*
               IF( CCHECK( L ) )
     $            CALL PSLAGEN( .FALSE., CFORM, 'No diag', OFFDC, MC,
...
*
               IF( ( L.EQ.2 ).AND.( MAX( NROWA, NCOLA ).GT.1 ) ) THEN
*
...
*
                  END IF
*
               ELSE IF( ( ( L.EQ.3 ).OR.( L.EQ.4 ) ).AND.
     $                  ( MAX( NROWC, NCOLC ).GT.1 ) ) THEN
*
*                 The distributed matrix C is symmetric
*
                  IF( LSAME( UPLO, 'L' ) ) THEN
*
...
*
                     IF( MAX( NROWC, NCOLC ).GT.1 ) THEN
                        CALL PSLASET( 'Upper', NROWC-1, NCOLC-1, ROGUE,
     $                                ROGUE, MEM( IPC ), IC, JC+1,
     $                                DESCC )
                        CALL PB_SLASET( 'Upper', NROWC-1, NCOLC-1, 0,
     $                                  ROGUE, ROGUE,
...
*
                     IF( MAX( NROWC, NCOLC ).GT.1 ) THEN
                        CALL PSLASET( 'Lower', NROWC-1, NCOLC-1, ROGUE,
     $                                ROGUE, MEM( IPC ), IC+1, JC,
     $                                DESCC )
                        CALL PB_SLASET( 'Lower', NROWC-1, NCOLC-1, 0,
     $                                  ROGUE, ROGUE,
...
*
                     IF( MAX( NROWC, NCOLC ).GT.1 ) THEN
                        CALL PSLASET( 'Upper', NROWC-1, NCOLC-1,
...
*
                     IF( MAX( NROWC, NCOLC ).GT.1 ) THEN
                        CALL PSLASET( 'Lower', NROWC-1, NCOLC-1,
...
*
               IF( BCHECK( L ) ) THEN
                  CALL PB_SFILLPAD( ICTXT, MPB, NQB, MEM( IPB-IPREB ),
     $                              DESCB( LLD_ ), IPREB, IPOSTB,
     $                              PADVAL )
               END IF
*
               IF( CCHECK( L ) ) THEN
                  CALL PB_SFILLPAD( ICTXT, MPC, NQC, MEM( IPC-IPREC ),
...
*
               INFO = 0
               CALL PSCHKARG3( ICTXT, NOUT, SNAMES( L ), SIDE, UPLO,
...
*
               IF( BCHECK( L ) ) THEN
                  IF( IVERB.EQ.2 ) THEN
                     CALL PB_PSLAPRNT( NROWB, NCOLB, MEM( IPB ), IB, JB,
     $                                 DESCB, 0, 0,
     $                                 'PARALLEL_INITIAL_B', NOUT,
     $                                 MEM( IPW ) )
                  ELSE IF( IVERB.GE.3 ) THEN
                     CALL PB_PSLAPRNT( MB, NB, MEM( IPB ), 1, 1, DESCB,
...
*
               IF( CCHECK( L ) ) THEN
                  IF( IVERB.EQ.2 ) THEN
                     CALL PB_PSLAPRNT( NROWC, NCOLC, MEM( IPC ), IC, JC,
     $                                 DESCC, 0, 0,
     $                                 'PARALLEL_INITIAL_C', NOUT,
     $                                 MEM( IPW ) )
                  ELSE IF( IVERB.GE.3 ) THEN
                     CALL PB_PSLAPRNT( MC, NC, MEM( IPC ), 1, 1, DESCC,
...
*                 Test PSGEMM
*
                  CALL PSGEMM( TRANSA, TRANSB, M, N, K, ALPHA,
...
*                 Test PSSYMM
*
                  CALL PSSYMM( SIDE, UPLO, M, N, ALPHA, MEM( IPA ), IA,
...
*                 Test PSSYRK
*
                  CALL PSSYRK( UPLO, TRANSA, N, K, ALPHA, MEM( IPA ),
...
*                 Test PSSYR2K
*
                  CALL PSSYR2K( UPLO, TRANSA, N, K, ALPHA, MEM( IPA ),
...
*                 Test PSTRMM
*
                  CALL PSTRMM( SIDE, UPLO, TRANSA, DIAG, M, N, ALPHA,
     $                         MEM( IPA ), IA, JA, DESCA, MEM( IPB ),
     $                         IB, JB, DESCB )
*
               ELSE IF( L.EQ.6 ) THEN
...
*                 Test PSTRSM
*
                  CALL PSTRSM( SIDE, UPLO, TRANSA, DIAG, M, N, ALPHA,
     $                         MEM( IPA ), IA, JA, DESCA, MEM( IPB ),
     $                         IB, JB, DESCB )
*
*
...
*                 Test PSGEADD
*
                  CALL PSGEADD( TRANSA, M, N, ALPHA, MEM( IPA ), IA, JA,
...
*                 Test PSTRADD
*
                  CALL PSTRADD( UPLO, TRANSA, M, N, ALPHA, MEM( IPA ),
     $                          IA, JA, DESCA, BETA, MEM( IPC ), IC, JC,
     $                          DESCC )
*
               END IF
...
*              Check padding
*
               CALL PB_SCHEKPAD( ICTXT, SNAMES( L ), MPA, NQA,
     $                           MEM( IPA-IPREA ), DESCA( LLD_ ),
     $                           IPREA, IPOSTA, PADVAL )
*
               IF( BCHECK( L ) ) THEN
                  CALL PB_SCHEKPAD( ICTXT, SNAMES( L ), MPB, NQB,
     $                              MEM( IPB-IPREB ), DESCB( LLD_ ),
     $                              IPREB, IPOSTB, PADVAL )
               END IF
*
               IF( CCHECK( L ) ) THEN
                  CALL PB_SCHEKPAD( ICTXT, SNAMES( L ), MPC, NQC,
...
*
               CALL PSBLAS3TSTCHK( ICTXT, NOUT, L, SIDE, UPLO, TRANSA,
...
*
               CALL PSCHKMOUT( NROWA, NCOLA, MEM( IPMATA ),
...
*
               IF( BCHECK( L ) ) THEN
                  CALL PSCHKMOUT( NROWB, NCOLB, MEM( IPMATB ),
...
*
               IF( CCHECK( L ) ) THEN
                  CALL PSCHKMOUT( NROWC, NCOLC, MEM( IPMATC ),
...
*
     ...
     $                                 MEM( IPMATA ) )
                  ...
                  IF( BCHECK( L ) ) THEN
                     IF( IERR( 5 ).NE.0 .OR. IVERB.GE.3 ) THEN
                        CALL PSMPRNT( ICTXT, NOUT, MB, NB,
     $                                MEM( IPMATB ), LDB, 0, 0,
     $                                'SERIAL_B' )
                        CALL PB_PSLAPRNT( MB, NB, MEM( IPB ), 1, 1,
     ...
     $                                   LDB, 0, 0, 'SERIAL_B' )
                        CALL PB_PSLAPRNT( NROWB, NCOLB, MEM( IPB ), IB,
     ...
     $                                    NOUT, MEM( IPMATB ) )
                  ...
                  IF( CCHECK( L ) ) THEN
                     IF( IERR( 6 ).NE.0 .OR. IVERB.GE.3 ) THEN
                        CALL PSMPRNT( ICTXT, NOUT, MC, NC,
     $                                MEM( IPMATC ), LDC, 0, 0,
     $                                'SERIAL_C' )
                        CALL PB_PSLAPRNT( MC, NC, MEM( IPC ), 1, 1,
     ...
     $                                   LDC, 0, 0, 'SERIAL_C' )
                        CALL PB_PSLAPRNT( NROWC, NCOLC, MEM( IPC ), IC,
...
*
      END
      SUBROUTINE PSBLA3TSTINFO( SUMMRY, NOUT, NMAT, DIAGVAL, SIDEVAL,
...
*  =======
*
*  PSBLA3TSTINFO  get the needed startup information for testing various
...
*          MAX( 3, 2*NGRIDS+38*NMAT+NSUBS+4 )  with  NSUBS  equal  to 8.
...
*
         OPEN( NIN, FILE='PSBLAS3TST.dat', STATUS='OLD' )
         READ( NIN, FMT = * ) SUMMRY
         SUMMRY = ' '
*
...
*  =======
*
*  PSBLAS3TSTCHKE tests the error exits of the Level 3 PBLAS.
*
*  Arguments
*  =========
*
...
*             If LTEST( 1 ) is .TRUE., PSGEMM  will be tested;
*             If LTEST( 2 ) is .TRUE., PSSYMM  will be tested;
*             If LTEST( 3 ) is .TRUE., PSSYRK  will be tested;
*             If LTEST( 4 ) is .TRUE., PSSYR2K will be tested;
*             If LTEST( 5 ) is .TRUE., PSTRMM  will be tested;
*             If LTEST( 6 ) is .TRUE., PSTRSM  will be tested;
*             If LTEST( 7 ) is .TRUE., PSGEADD will be tested;
*             If LTEST( 8 ) is .TRUE., PSTRADD will be tested;
*
...
*     .. External Subroutines ..
     ...
     $                   BLACS_GRIDINIT, PSDIMEE, PSGEADD, PSGEMM,
     $                   PSMATEE, PSOPTEE, PSSYMM, PSSYR2K, PSSYRK,
     $                   PSTRADD, PSTRMM, PSTRSM
*     ..
*     .. Common Blocks ..
      LOGICAL            ABRTFLG
      INTEGER            NOUT
...
*     ..
*     .. Data Statements ..
      DATA               SCODE/31, 32, 33, 35, 38, 38, 39, 40/
*     ..
*     .. Executable Statements ..
*
*     Temporarily define blacs grid to include all processes so
...
*
      I = 1
      IF( LTEST( I ) ) THEN
         CALL PSOPTEE( ICTXT, NOUT, PSGEMM, SCODE( I ), SNAMES( I ) )
         CALL PSDIMEE( ICTXT, NOUT, PSGEMM, SCODE( I ), SNAMES( I ) )
         CALL PSMATEE( ICTXT, NOUT, PSGEMM, SCODE( I ), SNAMES( I ) )
      END IF
*
*     Test PSSYMM
*
      I = I + 1
      IF( LTEST( I ) ) THEN
         CALL PSOPTEE( ICTXT, NOUT, PSSYMM, SCODE( I ), SNAMES( I ) )
         CALL PSDIMEE( ICTXT, NOUT, PSSYMM, SCODE( I ), SNAMES( I ) )
         CALL PSMATEE( ICTXT, NOUT, PSSYMM, SCODE( I ), SNAMES( I ) )
      END IF
*
*     Test PSSYRK
*
      I = I + 1
      IF( LTEST( I ) ) THEN
         CALL PSOPTEE( ICTXT, NOUT, PSSYRK, SCODE( I ), SNAMES( I ) )
         CALL PSDIMEE( ICTXT, NOUT, PSSYRK, SCODE( I ), SNAMES( I ) )
         CALL PSMATEE( ICTXT, NOUT, PSSYRK, SCODE( I ), SNAMES( I ) )
      END IF
*
*     Test PSSYR2K
*
      I = I + 1
      IF( LTEST( I ) ) THEN
         CALL PSOPTEE( ICTXT, NOUT, PSSYR2K, SCODE( I ), SNAMES( I ) )
         CALL PSDIMEE( ICTXT, NOUT, PSSYR2K, SCODE( I ), SNAMES( I ) )
         CALL PSMATEE( ICTXT, NOUT, PSSYR2K, SCODE( I ), SNAMES( I ) )
      END IF
*
*     Test PSTRMM
*
      I = I + 1
      IF( LTEST( I ) ) THEN
         CALL PSOPTEE( ICTXT, NOUT, PSTRMM, SCODE( I ), SNAMES( I ) )
         CALL PSDIMEE( ICTXT, NOUT, PSTRMM, SCODE( I ), SNAMES( I ) )
         CALL PSMATEE( ICTXT, NOUT, PSTRMM, SCODE( I ), SNAMES( I ) )
      END IF
*
*     Test PSTRSM
*
      I = I + 1
      IF( LTEST( I ) ) THEN
         CALL PSOPTEE( ICTXT, NOUT, PSTRSM, SCODE( I ), SNAMES( I ) )
         CALL PSDIMEE( ICTXT, NOUT, PSTRSM, SCODE( I ), SNAMES( I ) )
         CALL PSMATEE( ICTXT, NOUT, PSTRSM, SCODE( I ), SNAMES( I ) )
      END IF
*
*     Test PSGEADD
*
      I = I + 1
      IF( LTEST( I ) ) THEN
         CALL PSOPTEE( ICTXT, NOUT, PSGEADD, SCODE( I ), SNAMES( I ) )
         CALL PSDIMEE( ICTXT, NOUT, PSGEADD, SCODE( I ), SNAMES( I ) )
         CALL PSMATEE( ICTXT, NOUT, PSGEADD, SCODE( I ), SNAMES( I ) )
      END IF
*
*     Test PSTRADD
*
      I = I + 1
      IF( LTEST( I ) ) THEN
         CALL PSOPTEE( ICTXT, NOUT, PSTRADD, SCODE( I ), SNAMES( I ) )
         CALL PSDIMEE( ICTXT, NOUT, PSTRADD, SCODE( I ), SNAMES( I ) )
         CALL PSMATEE( ICTXT, NOUT, PSTRADD, SCODE( I ), SNAMES( I ) )
      END IF
*
...
*
      RETURN
*
*     End of PSBLAS3TSTCHKE
*
      END
      SUBROUTINE PSCHKARG3( ICTXT, NOUT, SNAME, SIDE, UPLO, TRANSA,
...
*
      END
      SUBROUTINE PSBLAS3TSTCHK( ICTXT, NOUT, NROUT, SIDE, UPLO, TRANSA,
...
*  =======
*
*  PSBLAS3TSTCHK performs the computational tests of the Level 3 PBLAS.
*
*  Notes
*  =====
*
...
*          follows:
*             If NROUT = 1, PSGEMM  will be tested;
*             else if NROUT = 2, PSSYMM  will be tested;
*             else if NROUT = 3, PSSYRK  will be tested;
*             else if NROUT = 4, PSSYR2K will be tested;
*             else if NROUT = 5, PSTRMM  will be tested;
*             else if NROUT = 6, PSTRSM  will be tested;
*             else if NROUT = 7, PSGEADD will be tested;
*             else if NROUT = 8, PSTRADD will be tested;
*
*  SIDE    (global input) CHARACTER*1
...
*          at least 2*MAX( M, MAX( N, K ) ). This array is used to store
*          a copy of a column of C and the computed gauges (see PSMMCH).
*
...
*     ..
*     .. Local Scalars ..
      INTEGER            I, IPG, MYCOL, MYROW, NPCOL, NPROW
      REAL               ERR
*     ..
*     .. Local Arrays ..
      INTEGER            IERR( 3 )
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, PB_SLASET, PSCHKMIN, PSMMCH,
     $                   PSMMCH1, PSMMCH2, PSMMCH3, PSTRMM, STRSM
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      EXTERNAL           LSAME
...
*
         CALL PSMMCH( ICTXT, TRANSA, TRANSB, M, N, K, ALPHA, A, IA, JA,
     $                DESCA, B, IB, JB, DESCB, BETA, C, PC, IC, JC,
     $                DESCC, WORK, WORK( IPG ), ERR, IERR( 3 ) )
*
         IF( IERR( 3 ).NE.0 ) THEN
            IF( MYROW.EQ.0 .AND. MYCOL.EQ.0 )
     $         WRITE( NOUT, FMT = 9998 )
...
*
         IF( LSAME( TRANSA, 'N' ) ) THEN
            CALL PSCHKMIN( ERR, M, K, A, PA, IA, JA, DESCA, IERR( 1 ) )
         ELSE
            CALL PSCHKMIN( ERR, K, M, A, PA, IA, JA, DESCA, IERR( 1 ) )
         END IF
         IF( LSAME( TRANSB, 'N' ) ) THEN
            CALL PSCHKMIN( ERR, K, N, B, PB, IB, JB, DESCB, IERR( 2 ) )
         ELSE
            CALL PSCHKMIN( ERR, N, K, B, PB, IB, JB, DESCB, IERR( 2 ) )
         END IF
*
...
*
         IF( LSAME( SIDE, 'L' ) ) THEN
            CALL PSMMCH( ICTXT, 'No transpose', 'No transpose', M, N, M,
     $                   ALPHA, A, IA, JA, DESCA, B, IB, JB, DESCB,
     $                   BETA, C, PC, IC, JC, DESCC, WORK, WORK( IPG ),
     $                   ERR, IERR( 3 ) )
         ELSE
            CALL PSMMCH( ICTXT, 'No transpose', 'No transpose', M, N, N,
...
*
         IF( LSAME( UPLO, 'L' ) ) THEN
            IF( LSAME( SIDE, 'L' ) ) THEN
               CALL PB_SLASET( 'Upper', M-1, M-1, 0, ROGUE, ROGUE,
     ...
     $                         A( IA+JA*DESCA( M_ ) ), DESCA( M_ ) )
         ...
         ELSE
            IF( LSAME( SIDE, 'L' ) ) THEN
               CALL PB_SLASET( 'Lower', M-1, M-1, 0, ROGUE, ROGUE,
...
*
         IF( LSAME( SIDE, 'L' ) ) THEN
            CALL PSCHKMIN( ERR, M, M, A, PA, IA, JA, DESCA, IERR( 1 ) )
         ELSE
            CALL PSCHKMIN( ERR, N, N, A, PA, IA, JA, DESCA, IERR( 1 ) )
         END IF
         CALL PSCHKMIN( ERR, M, N, B, PB, IB, JB, DESCB, IERR( 2 ) )
*
...
*
         IF( LSAME( TRANSA, 'N' ) ) THEN
            CALL PSMMCH1( ICTXT, UPLO, 'No transpose', N, K, ALPHA, A,
     $                    IA, JA, DESCA, BETA, C, PC, IC, JC, DESCC,
     $                    WORK, WORK( IPG ), ERR, IERR( 3 ) )
         ELSE
            CALL PSMMCH1( ICTXT, UPLO, 'Transpose', N, K, ALPHA, A, IA,
...
*
         IF( LSAME( TRANSA, 'N' ) ) THEN
            CALL PSCHKMIN( ERR, N, K, A, PA, IA, JA, DESCA, IERR( 1 ) )
         ELSE
            CALL PSCHKMIN( ERR, K, N, A, PA, IA, JA, DESCA, IERR( 1 ) )
         END IF
*
...
*
         IF( LSAME( TRANSA, 'N' ) ) THEN
            CALL PSMMCH2( ICTXT, UPLO, 'No transpose', N, K, ALPHA, A,
     $                    IA, JA, DESCA, B, IB, JB, DESCB, BETA, C, PC,
     $                    IC, JC, DESCC, WORK, WORK( IPG ), ERR,
     $                    IERR( 3 ) )
         ELSE
            CALL PSMMCH2( ICTXT, UPLO, 'Transpose', N, K, ALPHA, A,
     $                    IA, JA, DESCA, B, IB, JB, DESCB, BETA, C, PC,
     $                    IC, JC, DESCC, WORK, WORK( IPG ), ERR,
     $                    IERR( 3 ) )
         END IF
...
*
         IF( LSAME( SIDE, 'L' ) ) THEN
            CALL PSMMCH( ICTXT, TRANSA, 'No transpose', M, N, M,
     $                   ALPHA, A, IA, JA, DESCA, C, IB, JB, DESCB,
     $                   ZERO, B, PB, IB, JB, DESCB, WORK,
     $                   WORK( IPG ), ERR, IERR( 2 ) )
         ELSE
            CALL PSMMCH( ICTXT, 'No transpose', TRANSA, M, N, N,
...
*
     ...
     $                            A( IA+JA*DESCA( M_ ) ), DESCA( M_ ) )
               ELSE
                  CALL PB_SLASET( 'Upper', M, M, 0, ROGUE, ONE,
     $                            A( IA+(JA-1)*DESCA( M_ ) ),
     $                            DESCA( M_ ) )
               END IF
            ELSE
     ...
     $                            DESCA( M_ ) )
               ELSE
                  CALL PB_SLASET( 'Lower', M, M, 0, ROGUE, ONE,
     $                            A( IA+(JA-1)*DESCA( M_ ) ),
     $                            DESCA( M_ ) )
               END IF
            END IF
...
*        Check the resulting matrix B
*
         CALL STRSM( SIDE, UPLO, TRANSA, DIAG, M, N, ALPHA,
     $               A( IA+(JA-1)*DESCA( M_ ) ), DESCA( M_ ),
     $               B( IB+(JB-1)*DESCB( M_ ) ), DESCB( M_ ) )
         CALL PSTRMM( SIDE, UPLO, TRANSA, DIAG, M, N, ALPHA, PA, IA, JA,
     $                DESCA, PB, IB, JB, DESCB )
         IF( LSAME( SIDE, 'L' ) ) THEN
            CALL PSMMCH( ICTXT, TRANSA, 'No transpose', M, N, M, ALPHA,
     $                   A, IA, JA, DESCA, B, IB, JB, DESCB, ZERO, C,
     $                   PB, IB, JB, DESCB, WORK, WORK( IPG ), ERR,
     $                   IERR( 2 ) )
         ELSE
            CALL PSMMCH( ICTXT, 'No transpose', TRANSA, M, N, N, ALPHA,
...
*        Check the resulting matrix C
*
         CALL PSMMCH3( 'All', TRANSA, M, N, ALPHA, A, IA, JA, DESCA,
...
*
         ...
         ELSE
            CALL PSCHKMIN( ERR, N, M, A, PA, IA, JA, DESCA, IERR( 1 ) )
         END IF
*
...
*        Check the resulting matrix C
*
         CALL PSMMCH3( UPLO, TRANSA, M, N, ALPHA, A, IA, JA, DESCA,
     $                 BETA, C, PC, IC, JC, DESCC, ERR, IERR( 3 ) )
*
*        Check the input-only arguments
*
