...
*  =======
*
*  PSCASUM  returns  the sum of absolute values of the entries of a sub-
*  vector sub( X ),
*
*  where
*
...
*/
   ...
   if( !( info = ( ( nprow == -1 ) ? -( 601 + CTXT_ ) : 0 ) ) )
      PB_Cchkvec( ctxt, "PSCASUM", "X", *N, 1, Xi, Xj, Xd, *INCX, 6, &info );
   if( info ) { PB_Cabort( ctxt, "PSCASUM", info ); return; }
#endif
/*
*  Initialize ASUM
*/
...
*/
          ...
          ( ( mycol == Xcol ) || ( Xcol < 0 ) ) )
      {
         ...
         *ASUM = ABS( ((float *) Xptr)[REAL_PART] ) +
                 ABS( ((float *) Xptr)[IMAG_PART] );
      }
      return;
   }
   else if( *INCX == Xd[M_] )
...
*/
         ...
         if( Xnq > 0 )
         {
            Xld = Xd[LLD_];
            type = PB_Cctypeset();
            scvasum_( &Xnq, ((char *) ASUM), Mptr( ((char *) X), Xii,
