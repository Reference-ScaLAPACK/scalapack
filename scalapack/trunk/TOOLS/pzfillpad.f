...
*  =======
*
*  PZFILLPAD surrounds a two dimensional local array with a guard-
*  zone initialized to the value CHKVAL. The user may later call the
*  routine PZCHEKPAD to discover if the guardzone has been
...
*
*  A       (local input/local output) COMPLEX*16, array of
...
*
   ...
   10    CONTINUE
      ELSE
         WRITE( *, FMT = * ) 'WARNING no pre-guardzone in PZFILLPAD'
      END IF
*
...
*
   ...
   20    CONTINUE
      ELSE
         WRITE( *, FMT = * ) 'WARNING no post-guardzone in PZFILLPAD'
      END IF
*
