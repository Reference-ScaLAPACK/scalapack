...
*  =======
*
*  PZGEQLRV computes sub( A ) = A(IA:IA+M-1,JA:JA+N-1) from L, Q
*  computed by PZGEQLF.
*
*  Notes
*  =====
...
*          by PZGEQLF. On exit, the original matrix is restored.
*
...
*
*  TAU     (local input) COMPLEX*16, array, dimension LOCc(N_A).
*          This array contains the scalar factors TAU of the elementary
*          reflectors computed by PZGEQLF. TAU is tied to the dis-
*          tributed matrix A.
*
...
*     .. External Subroutines ..
     ...
     $                   PB_TOPSET, PZLACPY, PZLARFB, PZLARFT,
     $                   PZLASET
*     ..
*     .. External Functions ..
...
*
      CALL PZLARFT( 'Backward', 'Columnwise', M-N+JN-JA+1, JB, A, IA,
     $              JA+N-K, DESCA, TAU, WORK( IPT ), WORK( IPW ) )
*
*     Copy Householder vectors into workspace
*
      CALL PZLACPY( 'All', M-N+JN-JA+1, JB, A, IA, JA+N-K, DESCA,
     $              WORK( IPV ), IROFF+1, 1, DESCV )
      CALL PZLASET( 'Lower', JB, JB, ZERO, ONE, WORK( IPV ), IV,
     $              1, DESCV )
*
*     Zeoes the strict upper triangular part of A to get block
*     row of L
*
      CALL PZLASET( 'All', M-K, JB, ZERO, ZERO, A, IA, JA+N-K,
     $              DESCA )
      CALL PZLASET( 'Upper', JB, JB-1, ZERO, ZERO, A, IA+M-K,
     $              JA+N-K+1, DESCA )
*
*     Apply block Householder transformation
*
      CALL PZLARFB( 'Left', 'No transpose', 'Backward', 'Columnwise',
...
*        Compute upper triangular matrix T
*
         CALL PZLARFT( 'Backward', 'Columnwise', M-N+J+JB-JA, JB, A, IA,
     $                 J, DESCA, TAU, WORK( IPT ), WORK( IPW ) )
*
*        Copy Householder vectors into workspace
*
         CALL PZLACPY( 'All', M-N+J+JB-JA, JB, A, IA, J, DESCA,
     $                 WORK( IPV ), IROFF+1, 1, DESCV )
         CALL PZLASET( 'Lower', JB, JB, ZERO, ONE, WORK( IPV ), IV,
     $                 1, DESCV )
*
...
*        block row of L
*
         CALL PZLASET( 'All', M-N+J-JA, JB, ZERO, ZERO, A, IA, J,
     $                 DESCA )
         CALL PZLASET( 'Upper', JB, JB-1, ZERO, ZERO, A, IA+M-N+J-JA,
     $                 J+1, DESCA )
*
*        Apply block Householder transformation
*
