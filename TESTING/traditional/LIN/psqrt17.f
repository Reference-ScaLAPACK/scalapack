      REAL             FUNCTION PSQRT17( TRANS, IRESID, M, N, NRHS, A,
     $                                   IA, JA, DESCA, X, IX, JX,
...
*     .. External Functions ..
      ...
      EXTERNAL           INDXG2P, LSAME, NUMROC, PSLANGE, PSLAMCH
*     ..
...
*
     ...
     $              NRHSP ) )
*
      NORMA = PSLANGE( 'One-norm', M, N, A, IA, JA, DESCA, RWORK )
...
*
      IW = 1 + IROFFB
      JW = 1 + ICOFFB
      CALL PSLACPY( 'All', NROWS, NRHS, B, IB, JB, DESCB, WORK, IW, JW,
     $              DESCW )
      CALL PSGEMM( TRANS, 'No transpose', NROWS, NRHS, NCOLS, -ONE, A,
     $             IA, JA, DESCA, X, IX, JX, DESCX, ONE, WORK, IW, JW,
     $             DESCW )
      NORMRS = PSLANGE( 'Max', NROWS, NRHS, WORK, IW, JW, DESCW,
     $                  RWORK )
      IF( NORMRS.GT.SMLNUM ) THEN
         ISCL = 1
         CALL PSLASCL( 'General', NORMRS, ONE, NROWS, NRHS, WORK,
     $                 IW, JW, DESCW, INFO )
      END IF
*
*     compute R'*sub( A )
*
      IW2 = 1 + ICOFFB
      JW2 = 1 + IOFFA
      CALL PSGEMM( 'Transpose', TRANS, NRHS, NCOLS, NROWS, ONE, WORK,
     $             IW, JW, DESCW, A, IA, JA, DESCA, ZERO,
     $             WORK( NROWSP*NRHSQ+1 ), IW2, JW2, DESCW2 )
*
*     compute and properly scale error
*
      ERR = PSLANGE( 'One-norm', NRHS, NCOLS, WORK( NROWSP*NRHSQ+1 ),
...
*
      IF( IRESID.EQ.1 ) THEN
         NORMB = PSLANGE( 'One-norm', NROWS, NRHS, B, IB, JB, DESCB,
     $                    RWORK )
         IF( NORMB.NE.ZERO )
     $      ERR = ERR / NORMB
      ELSE
         NORMX = PSLANGE( 'One-norm', NCOLS, NRHS, X, IX, JX, DESCX,
     $                    RWORK )
         IF( NORMX.NE.ZERO )
     $      ERR = ERR / NORMX
      END IF
*
      PSQRT17 = ERR / ( PSLAMCH( ICTXT, 'Epsilon' ) *
     $                  REAL( MAX( M, N, NRHS ) ) )
*
      RETURN
*
