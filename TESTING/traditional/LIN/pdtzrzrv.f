...
*  =======
*
*  PDTZRZRV computes sub( A ) = A(IA:IA+M-1,JA:JA+N-1) from T, Z
*  computed by PDTZRZF.
*
*  Notes
*  =====
...
*          by PDTZRZF. On exit, the original matrix is restored.
*
...
*
*  TAU     (local input) DOUBLE PRECISION, array, dimension LOCr(M_A).
*          This array contains the scalar factors TAU of the elementary
*          reflectors computed by PDTZRZF. TAU is tied to the dis-
*          tributed matrix A.
*
...
*     .. External Subroutines ..
     ...
     $                   PDLARZB, PDLARZT, PDLASET, PB_TOPGET,
     $                   PB_TOPSET
*     ..
*     .. External Functions ..
...
*
      CALL PDLARZT( 'Backward', 'Rowwise', L, IB, A, IA, JM1, DESCA,
     $              TAU, WORK( IPT ), WORK( IPW ) )
*
*     Copy Householder vectors into workspace
*
      CALL PDLACPY( 'All', IB, L, A, IA, JM1, DESCA, WORK( IPV ), 1,
     $              JV, DESCV )
*
*     Save temporarily strict lower part of A(IA:IA+IB-1,JA:JA+IB-1)
*
      CALL PDLACPY( 'Lower', IB-1, IB-1, A, IA+1, JA, DESCA,
     $              WORK( IPV ), 1, ICOFF+1, DESCV )
*
*     Zeroes the row panel of sub( A ) to get T(IA:IN,JA:JA+N-1)
*
      CALL PDLASET( 'All', IB, L, ZERO, ZERO, A, IA, JM1, DESCA )
      CALL PDLASET( 'Lower', IB-1, IB-1, ZERO, ZERO, A, IA+1, JA,
     $              DESCA )
*
*     Apply block Householder transformation
*
      CALL PDLARZB( 'Right', 'Transpose', 'Backward', 'Rowwise',
     $              IN-IA+1, N, IB, L, WORK( IPV ), 1, JV, DESCV,
     $              WORK( IPT ), A, IA, JA, DESCA, WORK( IPW ) )
*
*     Restore strict lower part of A( IA:IA+IB-1, JA:JA+N-1 )
*
      CALL PDLACPY( 'Lower', IB-1, IB-1, WORK( IPV ), 1, ICOFF+1, DESCV,
     $              A, IA+1, JA, DESCA )
*
...
*        Compute upper triangular matrix T
*
         CALL PDLARZT( 'Backward', 'Rowwise', L, IB, A, I, JM1, DESCA,
     $                 TAU, WORK( IPT ), WORK( IPW ) )
*
*        Copy Householder vectors into workspace
*
         CALL PDLACPY( 'All', IB, L, A, I, JM1, DESCA, WORK( IPV ), 1,
     $                 JV, DESCV )
*
*        Save temporarily strict lower part of A(I:I+IB-1,J:J+IB-1 )
*
         CALL PDLACPY( 'Lower', IB-1, IB-1, A, I+1, JA+I-IA, DESCA,
     $                 WORK( IPV ), 1, ICOFF+1+I-IA, DESCV )
*
*        Zeoes the row panel of sub( A ) to get T(IA:I-1,JA+I-IA:JA+N-1)
*
         CALL PDLASET( 'All', IB, L, ZERO, ZERO, A, I, JM1, DESCA )
         CALL PDLASET( 'Lower', IB-1, IB-1, ZERO, ZERO, A, I+1, JA+I-IA,
     $                 DESCA )
*
*        Apply block Householder transformation
*
         CALL PDLARZB( 'Right', 'Transpose', 'Backward', 'Rowwise',
     $                 I+IB-IA, N-I+IA, IB, L, WORK( IPV ), 1, JV,
     $                 DESCV, WORK( IPT ), A, IA, JA+I-IA, DESCA,
     $                 WORK( IPW ) )
*
         CALL PDLACPY( 'Lower', IB-1, IB-1, WORK( IPV ), 1,
