...
*  =======
*
*  PCTZRZRV computes sub( A ) = A(IA:IA+M-1,JA:JA+N-1) from T, Z
*  computed by PCTZRZF.
*
*  Notes
*  =====
...
*          by PCTZRZF. On exit, the original matrix is restored.
*
...
*          reflectors computed by PCTZRZF. TAU is tied to the dis-
*          tributed matrix A.
*
...
*     .. External Subroutines ..
     ...
     $                   PCLARZB, PCLARZT, PCLASET, PB_TOPGET,
     $                   PB_TOPSET
*     ..
*     .. External Functions ..
...
*
      CALL PCLARZT( 'Backward', 'Rowwise', L, IB, A, IA, JM1, DESCA,
     $              TAU, WORK( IPT ), WORK( IPW ) )
*
*     Copy Householder vectors into workspace
*
      CALL PCLACPY( 'All', IB, L, A, IA, JM1, DESCA, WORK( IPV ), 1,
     $              JV, DESCV )
*
*     Save temporarily strict lower part of A(IA:IA+IB-1,JA:JA+IB-1)
*
      CALL PCLACPY( 'Lower', IB-1, IB-1, A, IA+1, JA, DESCA,
     $              WORK( IPV ), 1, ICOFF+1, DESCV )
*
*     Zeroes the row panel of sub( A ) to get T(IA:IN,JA:JA+N-1)
*
      CALL PCLASET( 'All', IB, L, ZERO, ZERO, A, IA, JM1, DESCA )
      CALL PCLASET( 'Lower', IB-1, IB-1, ZERO, ZERO, A, IA+1, JA,
     $              DESCA )
*
*     Apply block Householder transformation
*
      CALL PCLARZB( 'Right', 'Conjugate transpose', 'Backward',
...
*
      CALL PCLACPY( 'Lower', IB-1, IB-1, WORK( IPV ), 1, ICOFF+1, DESCV,
     $              A, IA+1, JA, DESCA )
*
...
*        Compute upper triangular matrix T
*
         CALL PCLARZT( 'Backward', 'Rowwise', L, IB, A, I, JM1, DESCA,
     $                 TAU, WORK( IPT ), WORK( IPW ) )
*
*        Copy Householder vectors into workspace
*
         CALL PCLACPY( 'All', IB, L, A, I, JM1, DESCA, WORK( IPV ), 1,
     $                 JV, DESCV )
*
*        Save temporarily strict lower part of A(I:I+IB-1,J:J+IB-1 )
*
         CALL PCLACPY( 'Lower', IB-1, IB-1, A, I+1, JA+I-IA, DESCA,
     $                 WORK( IPV ), 1, ICOFF+1+I-IA, DESCV )
*
*        Zeoes the row panel of sub( A ) to get T(IA:I-1,JA+I-IA:JA+N-1)
*
         CALL PCLASET( 'All', IB, L, ZERO, ZERO, A, I, JM1, DESCA )
         CALL PCLASET( 'Lower', IB-1, IB-1, ZERO, ZERO, A, I+1, JA+I-IA,
     $                 DESCA )
*
*        Apply block Householder transformation
*
     ...
     $                 WORK( IPW ) )
*
         CALL PCLACPY( 'Lower', IB-1, IB-1, WORK( IPV ), 1,
