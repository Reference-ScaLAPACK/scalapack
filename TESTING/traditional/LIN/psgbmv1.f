      SUBROUTINE PSGBDCMV( LDBW, BWL, BWU, TRANS, N, A, JA, DESCA, NRHS,
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PSDBDCMV', -INFO )
         RETURN
      END IF
*
...
*
        CALL SGBMV( TRANS, NUMROC_SIZE, NUMROC_SIZE, BWL, BWU, ONE,
     $              A( OFST+1 ), LLDA, B(PART_OFFSET+1), 1, ZERO,
     $              X( PART_OFFSET+1 ), 1 )
*
*
*
...
*         Send result back
*
          CALL SGESD2D( ICTXT, BWU, 1, WORK(MAX_BW*MAX_BW+1 ),
     $                   BWU, MYROW, MYCOL-1 )
*
*         Receive vector result from neighboring processor
*
          CALL SGERV2D( ICTXT, BWL, 1, WORK( MAX_BW*MAX_BW+1 ),
     $                    BWL, MYROW, MYCOL-1 )
*
...
*
           CALL SGERV2D( ICTXT, BWU, 1, WORK( MAX_BW*MAX_BW+1 ),
     $                    BWU, MYROW, MYCOL+1 )
*
*          Do addition of received vector
*
...
*
          CALL SLATCPY( 'L', BWU, BWU, A( OFST+1 ),
...
*
          CALL STRRV2D(ICTXT, 'U', 'N',
     $                  BWU, BWU,
     $                  WORK( 1 ), MAX_BW, MYROW, MYCOL+1 )
*
         CALL STRMV( 'U', 'N', 'N', BWU,
     $            WORK( 1 ), MAX_BW,
     $            WORK( MAX_BW*MAX_BW+1 ), 1)
*
*        Zero out extraneous elements caused by TRMV if any
...
*         Send result back
*
          CALL SGESD2D( ICTXT, BWL, 1, WORK(MAX_BW*MAX_BW+1 ),
     $                   BWL, MYROW, MYCOL-1 )
*
*         Receive vector result from neighboring processor
*
