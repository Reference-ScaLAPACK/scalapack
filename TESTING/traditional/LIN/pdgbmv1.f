      SUBROUTINE PDGBDCMV( LDBW, BWL, BWU, TRANS, N, A, JA, DESCA, NRHS,
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PDDBDCMV', -INFO )
         RETURN
      END IF
*
...
*
        CALL DGBMV( TRANS, NUMROC_SIZE, NUMROC_SIZE, BWL, BWU, ONE,
...
*         Send result back
*
          CALL DGESD2D( ICTXT, BWU, 1, WORK(MAX_BW*MAX_BW+1 ),
     $                   BWU, MYROW, MYCOL-1 )
*
*         Receive vector result from neighboring processor
*
          CALL DGERV2D( ICTXT, BWL, 1, WORK( MAX_BW*MAX_BW+1 ),
     $                    BWL, MYROW, MYCOL-1 )
*
...
*
           CALL DGERV2D( ICTXT, BWU, 1, WORK( MAX_BW*MAX_BW+1 ),
     $                    BWU, MYROW, MYCOL+1 )
*
*          Do addition of received vector
*
...
*
          CALL DLATCPY( 'L', BWU, BWU, A( OFST+1 ),
...
*         Send result back
*
          CALL DGESD2D( ICTXT, BWL, 1, WORK(MAX_BW*MAX_BW+1 ),
     $                   BWL, MYROW, MYCOL-1 )
*
*         Receive vector result from neighboring processor
*
