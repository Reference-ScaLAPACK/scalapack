      SUBROUTINE PDPTLASCHK( SYMM, UPLO, N, BWL, BWU, NRHS, X, IX, JX,
...
*  =======
*
*  PDPTLASCHK computes the residual
...
*
      IF ( WORKSIZ .LT. WORK_MIN ) THEN
       CALL PXERBLA( ICTXT, 'PDTLASCHK', -18 )
       RETURN
      END IF
*
...
*
               IF( MYCOL.GT.0 ) THEN
                  CALL DGERV2D( ICTXT, 1, 1, A( START), LDA,
     $                               MYROW, MYCOL-1 )
               ENDIF
*
             ENDIF
...
*
*
            CALL PDPBDCMV( BW+1, BW, UPLO, N, A, 1, DESCA,
