      SUBROUTINE PSPBDCMV( LDBW, BW, UPLO, N, A, JA, DESCA, NRHS, B, IB,
...
*
      IF( N .GT. NP*NB-MOD( JA-1, NB )) THEN
         INFO = -( 2 )
         CALL PXERBLA( ICTXT,
     $      'PSPBDCMV, D&C alg.: only 1 block per proc',
     $      -INFO )
         RETURN
      ENDIF
*
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PSPBDCMV', -INFO )
         RETURN
      END IF
*
...
*
        CALL SSBMV( 'L', NUMROC_SIZE, BW, ONE, A( OFST+1 ), LLDA,
...
*         Send result back
*
          CALL SGESD2D( ICTXT, BW, 1, WORK(BW*BW+1 ),
     $                   BW, MYROW, MYCOL-1 )
*
*         Receive vector result from neighboring processor
*
          CALL SGERV2D( ICTXT, BW, 1, WORK( BW*BW+1 ),
     $                    BW, MYROW, MYCOL-1 )
*
...
*       Use main partition in each processor to multiply locally
*
        CALL SSBMV( 'U', NUMROC_SIZE, BW, ONE, A( OFST+1 ), LLDA,
     $              B(PART_OFFSET+1), 1, ZERO, X( PART_OFFSET+1 ), 1 )
*
*
*
