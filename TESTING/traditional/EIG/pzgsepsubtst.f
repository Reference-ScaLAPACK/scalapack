...
*  =======
*
*  PZGSEPSUBTST calls PZHEGVX and then tests the output of
*  PZHEGVX
...
*     PZHEGVX when eigenvectors are requested
*
*  Arguments
*  =========
*
...
*          Must be 'V' on first call to PZGSEPSUBTST
*
...
*          Must be 'A' on first call to PZGSEPSUBTST
*
...
*            and columns.
*            See PZHEGVX for a description of block cyclic layout.
...
*
*  COPYB   (local input) COMPLEX*16 array, dim (N, N)
...
*          The eigenvalues as copmuted by this call to PZHEGVX
...
*          The amount of real workspace to pass to PZHEGVX
*
...
*          The result of this call to PZHEGVX
...
*     .. External Subroutines ..
     ...
     $                   PZFILLPAD, PZGSEPCHK, PZHEGVX, PZLASIZEGSEP,
     $                   PZLASIZEHEEVX, SLBOOT, SLTIMER, ZLACPY
*     ..
*     .. Intrinsic Functions ..
...
*
     ...
     $             DESCA( LLD_ ) )
*
      CALL ZLACPY( 'A', NP, NQ, COPYB, DESCA( LLD_ ), B( 1+IPREPAD ),
     $             DESCA( LLD_ ) )
*
      CALL PZFILLPAD( DESCA( CTXT_ ), NP, NQ, B, DESCA( LLD_ ), IPREPAD,
     $                IPOSTPAD, ZPADVAL+1.0D+2 )
*
...
*
*     Make sure that PZHEGVX does not cheat (i.e. use answers
*     already computed.)
*
      DO 70 I = 1, N, 1
         DO 60 J = 1, MAXEIGS, 1
...
*
      ...
      CALL SLTIMER( 1 )
      CALL SLTIMER( 6 )
      CALL PZHEGVX( IBTYPE, JOBZ, RANGE, UPLO, N, A( 1+IPREPAD ), IA,
...
*
      IF( THRESH.LE.0 ) THEN
         RESULT = 0
      ELSE
         CALL PZCHEKPAD( DESCA( CTXT_ ), 'PZHEGVX-B', NP, NQ, B,
     $                   DESCA( LLD_ ), IPREPAD, IPOSTPAD,
     $                   ZPADVAL+1.0D+2 )
*
         CALL PZCHEKPAD( DESCA( CTXT_ ), 'PZHEGVX-A', NP, NQ, A,
     $                   DESCA( LLD_ ), IPREPAD, IPOSTPAD, ZPADVAL )
*
         CALL PZCHEKPAD( DESCZ( CTXT_ ), 'PZHEGVX-Z', NP, MQ, Z,
     $                   DESCZ( LLD_ ), IPREPAD, IPOSTPAD,
     $                   ZPADVAL+1.0D+0 )
*
         CALL PDCHEKPAD( DESCA( CTXT_ ), 'PZHEGVX-WNEW', N, 1, WNEW, N,
     $                   IPREPAD, IPOSTPAD, PADVAL+2.0D+0 )
*
         CALL PDCHEKPAD( DESCA( CTXT_ ), 'PZHEGVX-GAP', NPROW*NPCOL, 1,
     $                   GAP, NPROW*NPCOL, IPREPAD, IPOSTPAD,
     $                   PADVAL+3.0D+0 )
*
         CALL PDCHEKPAD( DESCA( CTXT_ ), 'PZHEGVX-rWORK', LWORK1, 1,
     $                   RWORK, LWORK1, IPREPAD, IPOSTPAD,
     $                   PADVAL+4.0D+0 )
*
         CALL PZCHEKPAD( DESCA( CTXT_ ), 'PZHEGVX-WORK', LWORK, 1, WORK,
     $                   LWORK, IPREPAD, IPOSTPAD, ZPADVAL+4.1D+0 )
*
         CALL PICHEKPAD( DESCA( CTXT_ ), 'PZHEGVX-IWORK', LIWORK, 1,
     $                   IWORK, LIWORK, IPREPAD, IPOSTPAD, IPADVAL )
*
         CALL PICHEKPAD( DESCA( CTXT_ ), 'PZHEGVX-IFAIL', N, 1, IFAIL,
     $                   N, IPREPAD, IPOSTPAD, IPADVAL )
*
         CALL PICHEKPAD( DESCA( CTXT_ ), 'PZHEGVX-ICLUSTR',
...
*
     ...
     $      GO TO 160
*
*     Note that a couple key variables get redefined in PZGSEPCHK
*     as described by this table:
*
*     PZGSEPTST name         PZGSEPCHK name
*     -------------         -------------
*     COPYA                 A
*     Z                     Q
*     B                     B
...
*
            CALL PZGSEPCHK( IBTYPE, N, NZ, COPYA, IA, JA, DESCA, COPYB,
     $                      IA, JA, DESCA, THRESH, Z( 1+IPREPAD ), IA,
     $                      JA, DESCZ, A( 1+IPREPAD ), IA, JA, DESCA,
     $                      WNEW( 1+IPREPAD ), RWORK( 1+IPREPAD ),
     $                      RSIZECHK, TSTNRM, RES )
*
            CALL PDCHEKPAD( DESCA( CTXT_ ), 'PZGSEPCHK-rWORK', RSIZECHK,
...
*
      RETURN
*
 9999 FORMAT( 'PZHEGVX returned INFO=', I7 )
 9998 FORMAT( 'PZSEPQTQ returned INFO=', I7 )
 9997 FORMAT( 'PZGSEPSUBTST minerror =', D11.2, ' normwin=', D11.2 )
 9996 FORMAT( 'PZHEGVX returned INFO=', I7,
