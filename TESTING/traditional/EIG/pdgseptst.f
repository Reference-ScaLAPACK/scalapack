*
*
      SUBROUTINE PDGSEPTST( DESCA, UPLO, N, MATTYPE, IBTYPE, SUBTESTS,
...
*     .. Array Arguments ..
     ...
     $                   ISEED( 4 ), IWORK( * )
      DOUBLE PRECISION   A( LDA, * ), B( LDA, * ), COPYA( LDA, * ),
...
*  =======
*
*  PDGSEPTST builds a random matrix A, and a well conditioned
*  matrix B, runs PDSYGVX() to compute the eigenvalues
*  and eigenvectors and then calls PDSYGVCHK to compute
*  the residual.
*
...
*
*  COPYB   (local workspace) DOUBLE PRECISION array, dim (N, N)
...
*          On normal exit from PDSYGVX, the first M entries
...
*     .. External Subroutines ..
     ...
     $                   PDELSET, PDFILLPAD, PDGSEPSUBTST, PDLASET,
     $                   PDLASIZEGSEP, PDLASIZESYEVX, PDLATMS, PDMATGEN,
     $                   SLCOMBINE
*     ..
*     .. Intrinsic Functions ..
...
*
         ...
         WKNOWN = .FALSE.
*
         CALL PDGSEPSUBTST( WKNOWN, IBTYPE, 'v', 'a', UPLO, N, VL, VU,
...
*
            LSYEVXSIZE = VECSIZE
*
            CALL PDGSEPSUBTST( .TRUE., IBTYPE, JOBZ, RANGE, UPLO, N, VL,
...
*
 ...
 9980 FORMAT( ' Increase TOTMEM in PDGSEPDRIVER' )
*
*     End of PDGSEPTST
*
      END
