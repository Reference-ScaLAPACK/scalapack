...
*  =======
*
*  PZGEHDRV computes sub( A ) = A(IA:IA+N-1,JA:JA+N-1) from the
*  unitary matrix Q, the Hessenberg matrix, and the array TAU returned
*  by PZGEHRD:
*                       sub( A ) := Q * H * Q'
*
*  Arguments
...
*          form by PZGEHRD. The upper triangle and the first sub-
...
*
*  TAU     (local input) COMPLEX*16 array, dimension LOCc(JA+N-2)
*          The scalar factors of the elementary reflectors returned by
*          PZGEHRD. TAU is tied to the distributed matrix A.
*
...
*     .. External Subroutines ..
      EXTERNAL             BLACS_GRIDINFO, DESCSET, INFOG2L, PZLARFB,
     $                     PZLARFT, PZLACPY, PZLASET
*     ..
*     .. Intrinsic Functions ..
...
*
         CALL PZLARFT( 'Forward', 'Columnwise', IHI-K, JB, A, I+1, J,
     $                 DESCA, TAU, WORK( IPT ), WORK( IPW ) )
*
*        Copy Householder vectors into workspace.
*
         CALL PZLACPY( 'All', IHI-K, JB, A, I+1, J, DESCA, WORK( IPV ),
     $                 IV+1, 1, DESCV )
*
*        Zero out the strict lower triangular part of A.
*
         CALL PZLASET( 'Lower', IHI-K-1, JB, ZERO, ZERO, A, I+2, J,
     $                 DESCA )
*
...
*
     ...
     $                 'Columnwise', IHI, IHI-K, JB, WORK( IPV ), IV+1,
     $                 1, DESCV, WORK( IPT ), A, IA, J+1, DESCA,
     $                 WORK( IPW ) )
*
...
*     Compute upper triangular matrix T from TAU.
*
      CALL PZLARFT( 'Forward', 'Columnwise', IHI-ILO, JB, A, I+1, J,
     $              DESCA, TAU, WORK( IPT ), WORK( IPW ) )
*
*     Copy Householder vectors into workspace.
*
      CALL PZLACPY( 'All', IHI-ILO, JB, A, I+1, J, DESCA, WORK( IPV ),
     $              IV+1, 1, DESCV )
*
*     Zero out the strict lower triangular part of A.
*
      IF( IHI-ILO.GT.0 )
     $   CALL PZLASET( 'Lower', IHI-ILO-1, JB, ZERO, ZERO, A, I+2, J,
     $                 DESCA )
*
*     Apply block Householder transformation from Left.
*
