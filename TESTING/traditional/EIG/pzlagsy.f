...
*  =======
*
*  PZLAGHE generates a real Hermitian matrix A, by pre- and post-
...
*
*  D       (global input) COMPLEX*16 array, dimension (N)
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, PXERBLA, PZGEQRF,
     $                   PZLASIZESEP, PZMATGEN, PZUNMQR, ZLASET
*     ..
*     .. External Functions ..
...
*
      ...
      END IF
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( DESCA( CTXT_ ), 'PZLAGHE', -INFO )
         RETURN
      END IF
*
      INDAA = 1
...
*
      IF( K.NE.0 ) THEN
         CALL ZLASET( 'A', LDAA, NQ, ZZERO, ZZERO, WORK( INDAA ), LDAA )
*
*
*        Build a random matrix
*
*
         CALL PZMATGEN( DESCA( CTXT_ ), 'N', 'N', N, ORDER,
     $                  DESCA( MB_ ), DESCA( NB_ ), WORK( INDAA ),
     $                  DESCA( LLD_ ), DESCA( RSRC_ ), DESCA( CSRC_ ),
     $                  ISEED( 1 ), 0, NP, 0, NQ, MYROW, MYCOL, NPROW,
     $                  NPCOL )
         CALL PZGEQRF( N, ORDER, WORK( INDAA ), IA, JA, DESCA,
...
*
      IF( K.NE.0 ) THEN
*
         CALL PZUNMQR( 'L', 'Conjugate transpose', N, N, ORDER,
...
*
*
         CALL PZUNMQR( 'R', 'N', N, N, ORDER, WORK( INDAA ), IA, JA,
