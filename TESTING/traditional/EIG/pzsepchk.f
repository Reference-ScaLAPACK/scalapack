*
*
      SUBROUTINE PZSEPCHK( MS, NV, A, IA, JA, DESCA, EPSNORMA, THRESH,
...
*
*  A       (local input) COMPLEX*16 pointer to an
...
*
*          Contains the eigenvectors as computed by PZHEEVX
*
...
*     .. External Functions ..
      ...
      EXTERNAL           INDXG2L, INDXG2P, NUMROC, PZLANGE
*     ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( DESCA( CTXT_ ), 'PZSEPCHK', -INFO )
         RETURN
      END IF
*
*     C = Q * W
*
      CALL ZLACPY( 'A', MP, NQ, Q, DESCQ( LLD_ ), C, DESCC( LLD_ ) )
*
*
      DO 10 J = 1, NV
...
*
         IF( MYCOL.EQ.PCOL ) THEN
            CALL ZDSCAL( MP, W( J ), C( ( LOCALCOL-1 )*DESCC( LLD_ )+
     $                   1 ), 1 )
         END IF
   10 CONTINUE
*
...
*     C = C - A * Q
*
      CALL PZGEMM( 'N', 'N', MS, NV, MS, NEGONE, A, 1, 1, DESCA, Q, 1,
     $             1, DESCQ, ONE, C, 1, 1, DESCC )
*
*     Compute the norm of C
*
*
      NORM = PZLANGE( 'M', MS, NV, C, 1, 1, DESCC, WORK )
*
*
