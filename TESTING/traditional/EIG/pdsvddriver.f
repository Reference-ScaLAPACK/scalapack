...
*
      ...
      ELSE
         CALL IGEBR2D( CONTEXT, 'All', ' ', 1, NMATSIZES(1), MM, 1, 0,
     $                 0 )
      END IF
*
      IF( IAM.EQ.0 ) THEN
      ...
      ELSE
         CALL IGEBR2D( CONTEXT, 'All', ' ', 1, NMATSIZES(1), NN,
     $                 1, 0, 0 )
      END IF
*
...
*     Deal with error
      IF( NPCONFIGS( 1 ).LT.1 .OR. NPCONFIGS( 1 ).GT.MAXSETSIZE ) THEN
         IF( IAM.EQ.0 ) THEN
            WRITE( NOUT, FMT = 9999 )'# proc configs', NPCONFIGS(1), 1,
     $         MAXSETSIZE
         END IF
         ERR = -1
         GO TO 80
...
*
         CALL IGEBS2D( CONTEXT, 'All', ' ', 1, NPCONFIGS(1), NPROWS, 1 )
      ELSE
         CALL IGEBR2D( CONTEXT, 'All', ' ', 1, NPCONFIGS(1), NPROWS, 1,
     $                 0, 0 )
      END IF
*     Deal with error
...
*
      IF( IAM.EQ.0 ) THEN
         ...
         CALL IGEBS2D( CONTEXT, 'All', ' ', 1, NPCONFIGS(1), NPCOLS, 1 )
      ELSE
         CALL IGEBR2D( CONTEXT, 'All', ' ', 1, NPCONFIGS(1), NPCOLS, 1,
     $                 0, 0 )
      END IF
*
*     Deal with error.
...
*
      ...
      ELSE
         CALL IGEBR2D( CONTEXT, 'All', ' ', 1, NPCONFIGS(1), NBS, 1, 0,
     $                 0 )
      END IF
*     Deal with error
