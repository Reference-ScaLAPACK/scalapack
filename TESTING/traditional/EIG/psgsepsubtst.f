*
*
      SUBROUTINE PSGSEPSUBTST( WKNOWN, IBTYPE, JOBZ, RANGE, UPLO, N, VL,
...
*  =======
*
*  PSGSEPSUBTST calls PSSYGVX and then tests the output of
*  PSSYGVX
...
*     PSSYGVX when eigenvectors are requested
*
*  Arguments
*  =========
*
...
*          Must be 'V' on first call to PSGSEPSUBTST
*
...
*          Must be 'A' on first call to PSGSEPSUBTST
*
...
*            and columns.
*            See PSSYGVX for a description of block cyclic layout.
...
*
*  COPYB   (local input) REAL array, dim (N, N)
...
*          The eigenvalues as copmuted by this call to PSSYGVX
...
*          The amount of real workspace to pass to PSSYGVX
*
...
*          The result of this call to PSSYGVX
...
*     .. External Subroutines ..
     ...
     $                   PSFILLPAD, PSGSEPCHK, PSLASIZEGSEP,
     $                   PSLASIZESYEVX, PSSYGVX, SGAMN2D, SGAMX2D,
...
*
     ...
     $             DESCA( LLD_ ) )
*
      CALL SLACPY( 'A', NP, NQ, COPYB, DESCA( LLD_ ), B( 1+IPREPAD ),
     $             DESCA( LLD_ ) )
*
      CALL PSFILLPAD( DESCA( CTXT_ ), NP, NQ, B, DESCA( LLD_ ), IPREPAD,
     $                IPOSTPAD, PADVAL+1.0E+2 )
*
...
*
*     Make sure that PSSYGVX does not cheat (i.e. use answers
*     already computed.)
*
      DO 60 I = 1, N, 1
         DO 50 J = 1, MAXEIGS, 1
...
*
      ...
      CALL SLTIMER( 1 )
      CALL SLTIMER( 6 )
      CALL PSSYGVX( IBTYPE, JOBZ, RANGE, UPLO, N, A( 1+IPREPAD ), IA,
...
*
      IF( THRESH.LE.0 ) THEN
         RESULT = 0
      ELSE
         CALL PSCHEKPAD( DESCA( CTXT_ ), 'PSSYGVX-B', NP, NQ, B,
     $                   DESCA( LLD_ ), IPREPAD, IPOSTPAD,
     $                   PADVAL+1.0E+2 )
*
         CALL PSCHEKPAD( DESCA( CTXT_ ), 'PSSYGVX-A', NP, NQ, A,
     $                   DESCA( LLD_ ), IPREPAD, IPOSTPAD, PADVAL )
*
         CALL PSCHEKPAD( DESCZ( CTXT_ ), 'PSSYGVX-Z', NP, MQ, Z,
     $                   DESCZ( LLD_ ), IPREPAD, IPOSTPAD,
     $                   PADVAL+1.0E+0 )
*
         CALL PSCHEKPAD( DESCA( CTXT_ ), 'PSSYGVX-WNEW', N, 1, WNEW, N,
     $                   IPREPAD, IPOSTPAD, PADVAL+2.0E+0 )
*
         CALL PSCHEKPAD( DESCA( CTXT_ ), 'PSSYGVX-GAP', NPROW*NPCOL, 1,
     $                   GAP, NPROW*NPCOL, IPREPAD, IPOSTPAD,
     $                   PADVAL+3.0E+0 )
*
         CALL PSCHEKPAD( DESCA( CTXT_ ), 'PSSYGVX-WORK', LWORK1, 1,
     $                   WORK, LWORK1, IPREPAD, IPOSTPAD,
     $                   PADVAL+4.0E+0 )
*
         CALL PICHEKPAD( DESCA( CTXT_ ), 'PSSYGVX-IWORK', LIWORK, 1,
     $                   IWORK, LIWORK, IPREPAD, IPOSTPAD, IPADVAL )
*
         CALL PICHEKPAD( DESCA( CTXT_ ), 'PSSYGVX-IFAIL', N, 1, IFAIL,
     $                   N, IPREPAD, IPOSTPAD, IPADVAL )
*
         CALL PICHEKPAD( DESCA( CTXT_ ), 'PSSYGVX-ICLUSTR',
...
*
     ...
     $      GO TO 150
*
*     Note that a couple key variables get redefined in PSGSEPCHK
*     as described by this table:
*
*     PSGSEPTST name         PSGSEPCHK name
*     -------------         -------------
*     COPYA                 A
*     Z                     Q
*     B                     B
...
*
            CALL PSGSEPCHK( IBTYPE, N, NZ, COPYA, IA, JA, DESCA, COPYB,
     $                      IA, JA, DESCA, THRESH, Z( 1+IPREPAD ), IA,
     $                      JA, DESCZ, A( 1+IPREPAD ), IA, JA, DESCA,
     $                      WNEW( 1+IPREPAD ), WORK( 1+IPREPAD ),
     $                      SIZECHK, TSTNRM, RES )
*
            CALL PSCHEKPAD( DESCA( CTXT_ ), 'PSGSEPCHK-WORK', SIZECHK,
...
*
      RETURN
*
 9999 FORMAT( 'PSSYGVX returned INFO=', I7 )
 9998 FORMAT( 'PSSEPQTQ returned INFO=', I7 )
 9997 FORMAT( 'PSGSEPSUBTST minerror =', D11.2, ' normwin=', D11.2 )
 9996 FORMAT( 'PSSYGVX returned INFO=', I7,
 ...
 9982 FORMAT( 'IL, IU, VL or VU altered by PSSYGVX' )
 9981 FORMAT( 'NZ altered by PSSYGVX with JOBZ=N' )
*
*     End of PSGSEPSUBTST
*
      END
