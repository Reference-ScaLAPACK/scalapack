*
*
      SUBROUTINE PSSEPCHK( MS, NV, A, IA, JA, DESCA, EPSNORMA, THRESH,
...
*     .. External Functions ..
      ...
      REAL               PSLANGE
      EXTERNAL           INDXG2L, INDXG2P, NUMROC, PSLANGE
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, PSGEMM, PXERBLA,
     $                   SLACPY, SSCAL
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( DESCA( CTXT_ ), 'PSSEPCHK', -INFO )
         RETURN
      END IF
*
*     C = Q * W
*
      CALL SLACPY( 'A', MP, NQ, Q, DESCQ( LLD_ ), C, DESCC( LLD_ ) )
*
*
      DO 10 J = 1, NV
...
*
         IF( MYCOL.EQ.PCOL ) THEN
            CALL SSCAL( MP, W( J ), C( ( LOCALCOL-1 )*DESCC( LLD_ )+1 ),
     $                  1 )
         END IF
   10 CONTINUE
*
...
*     C = C - A * Q
*
      CALL PSGEMM( 'N', 'N', MS, NV, MS, NEGONE, A, 1, 1, DESCA, Q, 1,
     $             1, DESCQ, ONE, C, 1, 1, DESCC )
*
*     Compute the norm of C
*
*
      NORM = PSLANGE( 'M', MS, NV, C, 1, 1, DESCC, WORK )
*
*
