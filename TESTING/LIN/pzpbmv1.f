      SUBROUTINE PZPBDCMV( LDBW, BW, UPLO, N, A, JA, DESCA, NRHS, B, IB,
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PZPBDCMV', -INFO )
         RETURN
      END IF
*
...
*
        CALL ZHBMV( 'L', NUMROC_SIZE, BW, CONE, A( OFST+1 ), LLDA,
...
*         Send result back
*
          CALL ZGESD2D( ICTXT, BW, 1, WORK(BW*BW+1 ),
     $                   BW, MYROW, MYCOL-1 )
*
*         Receive vector result from neighboring processor
*
          CALL ZGERV2D( ICTXT, BW, 1, WORK( BW*BW+1 ),
     $                    BW, MYROW, MYCOL-1 )
*
...
*       Use main partition in each processor to multiply locally
*
        CALL ZHBMV( 'U', NUMROC_SIZE, BW, CONE, A( OFST+1 ), LLDA,
     $              B(PART_OFFSET+1), 1, CZERO, X( PART_OFFSET+1 ), 1 )
*
*
*
