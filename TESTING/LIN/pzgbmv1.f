...
*
      IF( N .GT. NP*NB-MOD( JA-1, NB )) THEN
         INFO = -( 2 )
         CALL PXERBLA( ICTXT,
     $      'PZDBDCMV, D&C alg.: only 1 block per proc',
     $      -INFO )
         RETURN
      ENDIF
*
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PZDBDCMV', -INFO )
         RETURN
      END IF
*
...
*
        CALL ZGBMV( TRANS, NUMROC_SIZE, NUMROC_SIZE, BWL, BWU, CONE,
...
*         Send result back
*
          CALL ZGESD2D( ICTXT, BWU, 1, WORK(MAX_BW*MAX_BW+1 ),
     $                   BWU, MYROW, MYCOL-1 )
*
*         Receive vector result from neighboring processor
*
          CALL ZGERV2D( ICTXT, BWL, 1, WORK( MAX_BW*MAX_BW+1 ),
     $                    BWL, MYROW, MYCOL-1 )
*
...
*
           CALL ZGERV2D( ICTXT, BWU, 1, WORK( MAX_BW*MAX_BW+1 ),
     $                    BWU, MYROW, MYCOL+1 )
*
*          Do addition of received vector
*
...
*
          CALL ZLATCPY( 'L', BWU, BWU, A( OFST+1 ),
...
*
          CALL ZTRRV2D(ICTXT, 'U', 'N',
     $                  BWU, BWU,
     $                  WORK( 1 ), MAX_BW, MYROW, MYCOL+1 )
*
         CALL ZTRMV( 'U', 'N', 'N', BWU,
     $            WORK( 1 ), MAX_BW,
     $            WORK( MAX_BW*MAX_BW+1 ), 1)
*
*        Zero out extraneous elements caused by TRMV if any
...
*         Send result back
*
          CALL ZGESD2D( ICTXT, BWL, 1, WORK(MAX_BW*MAX_BW+1 ),
     $                   BWL, MYROW, MYCOL-1 )
*
*         Receive vector result from neighboring processor
*
