      SUBROUTINE PCPBDCMV( LDBW, BW, UPLO, N, A, JA, DESCA, NRHS, B, IB,
...
*
      IF( N .GT. NP*NB-MOD( JA-1, NB )) THEN
         INFO = -( 2 )
         CALL PXERBLA( ICTXT,
     $      'PCPBDCMV, D&C alg.: only 1 block per proc',
     $      -INFO )
         RETURN
      ENDIF
*
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PCPBDCMV', -INFO )
         RETURN
      END IF
*
...
*
        CALL CHBMV( 'L', NUMROC_SIZE, BW, CONE, A( OFST+1 ), LLDA,
     $              B(PART_OFFSET+1), 1, CZERO, X( PART_OFFSET+1 ), 1 )
*
*
*
        IF ( MYCOL .LT. NPCOL-1 ) THEN
...
*         Send result back
*
          CALL CGESD2D( ICTXT, BW, 1, WORK(BW*BW+1 ),
     $                   BW, MYROW, MYCOL-1 )
*
*         Receive vector result from neighboring processor
*
          CALL CGERV2D( ICTXT, BW, 1, WORK( BW*BW+1 ),
     $                    BW, MYROW, MYCOL-1 )
*
...
*       Use main partition in each processor to multiply locally
*
        CALL CHBMV( 'U', NUMROC_SIZE, BW, CONE, A( OFST+1 ), LLDA,
     $              B(PART_OFFSET+1), 1, CZERO, X( PART_OFFSET+1 ), 1 )
*
*
*
