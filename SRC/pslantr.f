      REAL               FUNCTION PSLANTR( NORM, UPLO, DIAG, M, N, A,
     $                                     IA, JA, DESCA, WORK )
*
               ...
               IF( MYROW.EQ.IAROW ) THEN
                  IF( UDIAG ) THEN
                     DO 840 LL = JJ, JJ + JB -1
                        CALL SLASSQ( MIN( II+LL-JJ, IIA+MP-1 )-IIA+1,
     $                               A( IIA+IOFFA ), 1, SCALE, SUM )
                        IOFFA = IOFFA + LDA
  840                CONTINUE
                  ELSE
                     DO 850 LL = JJ, JJ + JB -1
                        CALL SLASSQ( MIN( II+LL-JJ+1, IIA+MP-1 )-IIA+1,
     $                               A( IIA+IOFFA ), 1, SCALE, SUM )
                        IOFFA = IOFFA + LDA
  850                CONTINUE
                  END IF
               ELSE
                  DO 860 LL = JJ, JJ + JB -1
                     CALL SLASSQ( MIN( II-1, IIA+MP-1 )-IIA+1,
     $                            A( IIA+IOFFA ), 1, SCALE, SUM )
                     IOFFA = IOFFA + LDA
  860             CONTINUE
               END IF
                  ...
                  IF( MYROW.EQ.IAROW ) THEN
                     IF( UDIAG ) THEN
                        DO 870 LL = JJ, JJ + JB -1
                           CALL SLASSQ( MIN( II+LL-JJ+1, IIA+MP-1 )-
  ...
  870                   CONTINUE
                     ELSE
                        DO 880 LL = JJ, JJ + JB -1
                           CALL SLASSQ( MIN( II+LL-JJ, IIA+MP-1 )-
     $                                  IIA+1, A( IIA+IOFFA ), 1, SCALE,
     $                                  SUM )
                           IOFFA = IOFFA + LDA
  880                   CONTINUE
  ...
  920                CONTINUE
               ...
               ELSE
                  DO 930 LL = JJ, JJ + JB -1
                     CALL SLASSQ( IIA+MP-II, A( II+IOFFA ), 1, SCALE,
     $                            SUM )
                     IOFFA = IOFFA + LDA
  930             CONTINUE
               END IF
