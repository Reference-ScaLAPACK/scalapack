...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PCPTTRSV', -INFO )
         RETURN
      END IF
*
...
*
        CALL CPTTRSV( UPLO, 'N', ODD_SIZE, NRHS, D( PART_OFFSET+1 ),
...
*
            CALL CAXPY( NRHS, -E( PART_OFFSET+ODD_SIZE ),
...
*
            CALL CGEMM( 'C', 'N', 1, NRHS, ODD_SIZE, -CONE, AF( 1 ),
...
*
        CALL CTRTRS( 'L', 'N', 'U', INT_ONE, NRHS, AF( ODD_SIZE+2 ),
...
*       Solve with diagonal block
*
        CALL CTRTRS( 'L', 'C', 'U', INT_ONE, NRHS, AF( ODD_SIZE+2 ),
     $               INT_ONE, B( PART_OFFSET+ODD_SIZE+1 ), LLDB, INFO )
*
        IF( INFO.NE.0 ) THEN
          GO TO 1000
...
*
          CALL CGEMM( 'N', 'N', ODD_SIZE, NRHS, 1, -CONE, AF( 1 ),
...
*           locally stored portion of right hand side(s)
*
            CALL CAXPY( NRHS, -CONJG( E( PART_OFFSET+ODD_SIZE ) ),
     $                 B( PART_OFFSET+ODD_SIZE+1 ), LLDB,
     $                 B( PART_OFFSET+ODD_SIZE ), LLDB )
*
        ENDIF
...
*       Use main partition in each processor to solve locally
*
        CALL CPTTRSV( UPLO, 'C', ODD_SIZE, NRHS, D( PART_OFFSET+1 ),
     $                E( PART_OFFSET+1 ), B( PART_OFFSET+1 ), LLDB,
     $                INFO )
*
      ENDIF
...
*           processor's righthand-side.
*
            CALL CGEMM( 'T', 'N', 1, NRHS, ODD_SIZE, -CONE, AF( 1 ),
     $                  ODD_SIZE, B( PART_OFFSET+1 ), LLDB, CZERO,
     $                  WORK( 1+INT_ONE-1 ), INT_ONE )
        ENDIF
*
...
*         Calculate contribution from this block to next diagonal block
*
          CALL CGEMM( 'T', 'N', INT_ONE, NRHS, INT_ONE, -CONE,
     $                     AF( (ODD_SIZE)*1+1 ),
     $                     INT_ONE,
     $                     B( PART_OFFSET+ODD_SIZE+1 ),
     $                     LLDB, CZERO,
...
*           processor's solution.
*
          CALL CGEMM( 'C', 'N', ODD_SIZE, NRHS, 1, -CONE, AF( 1 ),
