...
*     .. External Subroutines ..
     ...
     $                   PB_TOPSET, PSPOTF2, PSSYRK, PSTRSM,
     $                   PXERBLA
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PSPOTRF', -INFO )
         RETURN
      END IF
*
...
*
         CALL PSPOTF2( UPLO, JB, A, IA, JA, DESCA, INFO )
         IF( INFO.NE.0 )
     $      GO TO 30
*
         IF( JB+1.LE.N ) THEN
...
*
            CALL PSSYRK( UPLO, 'Transpose', N-JB, JB, -ONE, A, IA,
...
*
            CALL PSSYRK( UPLO, 'No Transpose', N-JB, JB, -ONE, A, IA+JB,
...
*              Update the trailing matrix, A = A - L*L'
*
               CALL PSSYRK( UPLO, 'No Transpose', N-J-JB+JA, JB, -ONE,
