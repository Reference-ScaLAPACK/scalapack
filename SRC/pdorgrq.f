      SUBROUTINE PDORGRQ( M, N, K, A, IA, JA, DESCA, TAU, WORK, LWORK,
     $                    INFO )
*
...
*     .. External Subroutines ..
     ...
     $                   PDLARFT, PDLASET, PDORGR2, PB_TOPGET,
     $                   PB_TOPSET, PXERBLA
*     ..
*     .. External Functions ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PDORGRQ', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
      CALL PDLASET( 'All', IN-IA+1, M-IN+IA-1, ZERO, ZERO, A, IA,
     $              JA+N-M+IN-IA+1, DESCA )
*
*     Use unblocked code for the first or only block.
*
      CALL PDORGR2( IN-IA+1, N-M+IN-IA+1, IN-IA-M+K+1, A, IA, JA, DESCA,
...
*
         CALL PDORGR2( IB, N-M+I+IB-IA, IB, A, I, JA, DESCA, TAU, WORK,
     $                 LWORK, IINFO )
*
*        Set rows i:i+ib-1,ja+n-m+i+ib-ia:ja+n-1 of current block to
*        zero
*
         CALL PDLASET( 'All', IB, M-I-IB+IA, ZERO, ZERO, A, I,
