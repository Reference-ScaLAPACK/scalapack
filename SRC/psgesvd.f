      SUBROUTINE PSGESVD( JOBU, JOBVT, M, N, A, IA, JA, DESCA, S, U, IU,
...
*     .. Parameters ..
     ...
     $                   MB_, NB_, RSRC_, CSRC_, LLD_, ITHVAL
      PARAMETER          ( BLOCK_CYCLIC_2D = 1, DLEN_ = 9, DTYPE_ = 1,
     $                   CTXT_ = 2, M_ = 3, N_ = 4, MB_ = 5, NB_ = 6,
     $                   RSRC_ = 7, CSRC_ = 8, LLD_ = 9, ITHVAL = 10 )
      REAL               ZERO, ONE
      PARAMETER          ( ZERO = 0.0E+0, ONE = 1.0E+0 )
*     ..
*     .. Local Scalars ..
      CHARACTER          UPLO
      INTEGER            CONTEXTC, CONTEXTR, I, INDD, INDD2, INDE,
     $                   INDE2, INDTAUP, INDTAUQ, INDU, INDV, INDWORK,
     $                   IOFFD, IOFFE, ISCALE, J, K, LDU, LDVT, LLWORK,
     $                   LWMIN, MAXIM, MB, MP, MYPCOL, MYPCOLC, MYPCOLR,
     $                   MYPROW, MYPROWC, MYPROWR, NB, NCVT, NPCOL,
     $                   NPCOLC, NPCOLR, NPROCS, NPROW, NPROWC, NPROWR,
     $                   NQ, NRU, SIZE, SIZEB, SIZEP, SIZEPOS, SIZEQ,
     $                   WANTU, WANTVT, WATOBD, WBDTOSVD, WPSGEBRD,
     $                   WPSLANGE, WPSORMBRPRT, WPSORMBRQLN, WSBDSQR
      REAL               ANRM, BIGNUM, EPS, RMAX, RMIN, SAFMIN, SIGMA,
     $                   SMLNUM
*     ..
*     .. Local Arrays ..
      INTEGER            DESCTU( DLEN_ ), DESCTVT( DLEN_ ), IDUM1( 3 ),
     $                   IDUM2( 3 )
      REAL               C( 1, 1 )
*     ..
*     .. External Functions ..
      ...
      EXTERNAL           LSAME, NUMROC, PSLAMCH, PSLANGE
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_GET, BLACS_GRIDEXIT, BLACS_GRIDINFO,
     $                   BLACS_GRIDINIT, CHK1MAT, DESCINIT, IGAMX2D,
     $                   IGEBR2D, IGEBS2D, PCHK1MAT, PSGEBRD, PSGEMR2D,
     $                   PSLARED1D, PSLARED2D, PSLASCL, PSLASET,
     $                   PSORMBR, PXERBLA, SBDSQR, SGAMN2D, SGAMX2D,
     $                   SSCAL
*     ..
*     .. Intrinsic Functions ..
...
*     This is just to keep ftnchek happy
      IF( BLOCK_CYCLIC_2D*DTYPE_*LLD_*MB_*M_*NB_*N_.LT.0 )
     $   RETURN
*
...
*
         ...
         END IF
         IF( WANTVT.EQ.1 ) THEN
            CALL CHK1MAT( SIZE, SIZEPOS, N, 4, IVT, JVT, DESCVT, 17,
     $                    INFO )
         END IF
         CALL IGAMX2D( DESCA( CTXT_ ), 'A', ' ', 1, 1, INFO, 1, 1, 1,
     $                 -1, -1, 0 )
*
         IF( INFO.EQ.0 ) THEN
*
...
*
            NRU = NUMROC( SIZE, 1, MYPROWC, 0, NPROCS )
            NCVT = NUMROC( SIZE, 1, MYPCOLR, 0, NPROCS )
            NB = DESCA( NB_ )
            MB = DESCA( MB_ )
            MP = NUMROC( M, MB, MYPROW, DESCA( RSRC_ ), NPROW )
            NQ = NUMROC( N, NB, MYPCOL, DESCA( CSRC_ ), NPCOL )
            IF( WANTVT.EQ.1 ) THEN
               SIZEP = NUMROC( SIZE, DESCVT( MB_ ), MYPROW,
     $                 DESCVT( RSRC_ ), NPROW )
            ELSE
               SIZEP = 0
            END IF
            IF( WANTU.EQ.1 ) THEN
               SIZEQ = NUMROC( SIZE, DESCU( NB_ ), MYPCOL,
     $                 DESCU( CSRC_ ), NPCOL )
            ELSE
               SIZEQ = 0
            END IF
...
*
            IF( MYPROW.EQ.0 .AND. MYPCOL.EQ.0 ) THEN
               MAXIM = MAX( NQ, MP )
               CALL IGEBS2D( DESCA( CTXT_ ), 'All', ' ', 1, 1, MAXIM,
     $                       1 )
            ELSE
               CALL IGEBR2D( DESCA( CTXT_ ), 'All', ' ', 1, 1, MAXIM, 1,
     $                       0, 0 )
            END IF
*
            WPSLANGE = MP
...
*
            WSBDSQR = MAX( 1, 2*SIZE+( 2*SIZE-4 )*MAX( WANTU, WANTVT ) )
            WPSORMBRQLN = MAX( ( NB*( NB-1 ) ) / 2, ( SIZEQ+MP )*NB ) +
     $                    NB*NB
            WPSORMBRPRT = MAX( ( MB*( MB-1 ) ) / 2, ( SIZEP+NQ )*MB ) +
     $                    MB*MB
            WBDTOSVD = SIZE*( WANTU*NRU+WANTVT*NCVT ) +
...
*
            LWMIN = 2 + 6*SIZEB + MAX( WATOBD, WBDTOSVD )
            WORK( 1 ) = REAL( LWMIN )
*
            IF( WANTU.NE.1 .AND. .NOT.( LSAME( JOBU, 'N' ) ) ) THEN
               INFO = -1
            ELSE IF( WANTVT.NE.1 .AND. .NOT.( LSAME( JOBVT, 'N' ) ) )
     $                THEN
               INFO = -2
...
*
         ...
         IDUM2( 2 ) = 2
         IDUM2( 3 ) = 19
         CALL PCHK1MAT( M, 3, N, 4, IA, JA, DESCA, 8, 3, IDUM1, IDUM2,
     $                  INFO )
         IF( INFO.EQ.0 ) THEN
            IF( WANTU.EQ.1 ) THEN
               CALL PCHK1MAT( M, 3, N, 4, IU, JU, DESCU, 13, 0, IDUM1,
     $                        IDUM2, INFO )
            END IF
            IF( WANTVT.EQ.1 ) THEN
               CALL PCHK1MAT( M, 3, N, 4, IVT, JVT, DESCVT, 17, 0,
     $                        IDUM1, IDUM2, INFO )
            END IF
         END IF
*
...
*
     ...
     $              WORK( INDTAUQ ), WORK( INDTAUP ), WORK( INDWORK ),
     $              LLWORK, INFO )
*
...
*
      IF( M.GE.N ) THEN
*        Distribute D
         CALL PSLARED1D( N+IOFFD, IA, JA, DESCA, WORK( INDD ),
     $                   WORK( INDD2 ), WORK( INDWORK ), LLWORK )
*        Distribute E
         CALL PSLARED2D( M+IOFFE, IA, JA, DESCA, WORK( INDE ),
     $                   WORK( INDE2 ), WORK( INDWORK ), LLWORK )
      ELSE
*        Distribute D
         CALL PSLARED2D( M+IOFFD, IA, JA, DESCA, WORK( INDD ),
     $                   WORK( INDD2 ), WORK( INDWORK ), LLWORK )
*        Distribute E
         CALL PSLARED1D( N+IOFFE, IA, JA, DESCA, WORK( INDE ),
     $                   WORK( INDE2 ), WORK( INDWORK ), LLWORK )
      END IF
*
...
*
      INDU = INDWORK
      INDV = INDU + SIZE*NRU
      INDWORK = INDV + SIZE*NCVT
*
      LDU = MAX( 1, NRU )
      LDVT = MAX( 1, SIZE )
*
      CALL DESCINIT( DESCTU, SIZE, SIZE, 1, 1, 0, 0, CONTEXTC, LDU,
     $               INFO )
      CALL DESCINIT( DESCTVT, SIZE, SIZE, 1, 1, 0, 0, CONTEXTR, LDVT,
     $               INFO )
*
      IF( WANTU.EQ.1 ) THEN
         CALL PSLASET( 'Full', M, SIZE, ZERO, ONE, WORK( INDU ), 1, 1,
     $                 DESCTU )
      ELSE
         NRU = 0
      END IF
*
      IF( WANTVT.EQ.1 ) THEN
         CALL PSLASET( 'Full', SIZE, N, ZERO, ONE, WORK( INDV ), 1, 1,
     $                 DESCTVT )
      ELSE
         NCVT = 0
      END IF
*
     ...
     $             WORK( INDE2+IOFFE ), WORK( INDV ), SIZE,
     $             WORK( INDU ), LDU, C, 1, WORK( INDWORK ), INFO )
*
*     Redistribute elements of U and VT in the block-cyclic fashion.
*
      IF( WANTU.EQ.1 )
     $   CALL PSGEMR2D( SIZE, SIZE, WORK( INDU ), 1, 1, DESCTU, U, IU,
     $                  JU, DESCU, DESCU( CTXT_ ) )
*
      IF( WANTVT.EQ.1 )
     $   CALL PSGEMR2D( SIZE, SIZE, WORK( INDV ), 1, 1, DESCTVT, VT,
     $                  IVT, JVT, DESCVT, DESCVT( CTXT_ ) )
*
*     Set to ZERO "non-square" elements of the larger matrices U, VT.
*
      IF( M.GT.N .AND. WANTU.EQ.1 ) THEN
         CALL PSLASET( 'Full', M-SIZE, SIZE, ZERO, ZERO, U, IA+SIZE, JU,
     $                 DESCU )
      ELSE IF( N.GT.M .AND. WANTVT.EQ.1 ) THEN
         CALL PSLASET( 'Full', SIZE, N-SIZE, ZERO, ZERO, VT, IVT,
     $                 JVT+SIZE, DESCVT )
      END IF
*
...
*
      IF( WANTU.EQ.1 )
     $   CALL PSORMBR( 'Q', 'L', 'N', M, SIZE, N, A, IA, JA, DESCA,
     $                 WORK( INDTAUQ ), U, IU, JU, DESCU,
     $                 WORK( INDWORK ), LLWORK, INFO )
*
      IF( WANTVT.EQ.1 )
     $   CALL PSORMBR( 'P', 'R', 'T', SIZE, N, M, A, IA, JA, DESCA,
...
*
   ...
   20 CONTINUE
*
      CALL SGAMN2D( DESCA( CTXT_ ), 'a', ' ', J, 1, WORK( 1+INDE ), J,
     $              1, 1, -1, -1, 0 )
      CALL SGAMX2D( DESCA( CTXT_ ), 'a', ' ', J, 1, WORK( 1+INDD2 ), J,
     $              1, 1, -1, -1, 0 )
*
      DO 30 I = 1, J
