      SUBROUTINE PCUNMBR( VECT, SIDE, TRANS, M, N, K, A, IA, JA, DESCA,
...
*          distributed matrix reduced by PCGEBRD.
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, PCHK2MAT, PCUNMLQ,
     $                   PCUNMQR, PXERBLA
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PCUNMBR', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
         IF( NQ.GE.K ) THEN
*
*           Q was determined by a call to PCGEBRD with nq >= k
*
            CALL PCUNMQR( SIDE, TRANS, M, N, K, A, IA, JA, DESCA, TAU,
     $                    C, IC, JC, DESCC, WORK, LWORK, IINFO )
         ELSE IF( NQ.GT.1 ) THEN
*
*           Q was determined by a call to PCGEBRD with nq < k
*
            CALL PCUNMQR( SIDE, TRANS, MI, NI, NQ-1, A, IA+1, JA, DESCA,
...
*
         ...
         IF( NQ.GT.K ) THEN
*
*           P was determined by a call to PCGEBRD with nq > k
*
            CALL PCUNMLQ( SIDE, TRANST, M, N, K, A, IA, JA, DESCA, TAU,
     $                    C, IC, JC, DESCC, WORK, LWORK, IINFO )
         ELSE IF( NQ.GT.1 ) THEN
*
*           P was determined by a call to PCGEBRD with nq <= k
*
            CALL PCUNMLQ( SIDE, TRANST, MI, NI, NQ-1, A, IA, JA+1,
