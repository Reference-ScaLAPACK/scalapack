      SUBROUTINE PDORG2R( M, N, K, A, IA, JA, DESCA, TAU, WORK, LWORK,
     $                    INFO )
*
...
*          returned by PDGEQRF in the K columns of its array
...
*
      ...
      END IF
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PDORG2R', -INFO )
...
*
      CALL PDLASET( 'All', K, N-K, ZERO, ZERO, A, IA, JA+K, DESCA )
      CALL PDLASET( 'All', M-K, N-K, ZERO, ONE, A, IA+K, JA+K, DESCA )
*
      TAUJ = ZERO
...
*
         IF( J.LT.JA+N-1 ) THEN
            CALL PDELSET( A, IA+J-JA, J, DESCA, ONE )
            CALL PDLARF( 'Left', M-J+JA, JA+N-J-1, A, IA+J-JA, J, DESCA,
...
*
     ...
     $      TAUJ = TAU( MIN( JJ, KQ ) )
         IF( J-JA.LT.M-1 )
     $      CALL PDSCAL( M-J+JA-1, -TAUJ, A, IA+J-JA+1, J, DESCA, 1 )
         CALL PDELSET( A, IA+J-JA, J, DESCA, ONE-TAUJ )
*
*        Set A(ia:ia+j-ja-1,j) to zero
*
         CALL PDLASET( 'All', J-JA, 1, ZERO, ZERO, A, IA, J, DESCA )
*
   10 CONTINUE
*
