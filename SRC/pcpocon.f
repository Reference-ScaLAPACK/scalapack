...
*     .. Local Scalars ..
     ...
     $                   NPCOL, NPROW, NPMOD, NQ, NQMOD
      REAL               AINVNM, SCALE, SL, SU, SMLNUM
      COMPLEX            WMAX, ZDUM
*     ..
*     .. Local Arrays ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PCPOCON', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
            DESCX( CSRC_ ) = IACOL
            CALL PCLATRS( 'Lower', 'No transpose', 'Non-unit', NORMIN,
     $                    N, A, IA, JA, DESCA, WORK( IPX ), IX, JX,
...
*
            DESCX( CSRC_ ) = IACOL
            CALL PCLATRS( 'Lower', 'Conjugate transpose', 'Non-unit',
     $                    NORMIN, N, A, IA, JA, DESCA, WORK( IPX ),
