      SUBROUTINE PDTRCON( NORM, UPLO, DIAG, N, A, IA, JA, DESCA, RCOND,
...
*     .. External Functions ..
      ...
      DOUBLE PRECISION   PDLAMCH, PDLANTR
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PDTRCON', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
      ANORM = PDLANTR( NORM, UPLO, DIAG, N, N, A, IA, JA, DESCA, WORK )
*
...
*
         ...
         KASE = 0
   10    CONTINUE
         CALL PDLACON( N, WORK( IPV ), IV, JV, DESCV, WORK( IPX ),
...
*
               DESCX( CSRC_ ) = IACOL
               CALL PDLATRS( UPLO, 'No transpose', DIAG, NORMIN,
...
*
               DESCX( CSRC_ ) = IACOL
               CALL PDLATRS( UPLO, 'Transpose', DIAG, NORMIN,
     $                       N, A, IA, JA, DESCA, WORK( IPX ), IX, JX,
     $                       DESCX, SCALE, WORK( IPN ), WORK( IPW ) )
               DESCX( CSRC_ ) = MYCOL
            END IF
...
*
            IF( SCALE.NE.ONE ) THEN
               CALL PDAMAX( N, WMAX, IXX, WORK( IPX ), IX, JX,
     $                   DESCX, 1 )
               IF( DESCX( M_ ).EQ.1 .AND. N.EQ.1 ) THEN
                  CALL PB_TOPGET( ICTXT, 'Broadcast', 'Columnwise',
     $                          CBTOP )
                  IF( MYROW.EQ.IAROW ) THEN
                     CALL DGEBS2D( ICTXT, 'Column', CBTOP, 1, 1, WMAX,
     $                             1 )
                  ELSE
                     CALL DGEBR2D( ICTXT, 'Column', CBTOP, 1, 1, WMAX,
     $                             1, IAROW, MYCOL )
                  END IF
               END IF
