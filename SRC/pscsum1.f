...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, INFOG2L, SGSUM2D, PB_TOPGET
*     ..
*     .. External Functions ..
      INTEGER            NUMROC
      REAL               SCSUM1
      EXTERNAL           NUMROC, SCSUM1
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          ABS, MOD
*     ..
...
*
         IF( MYROW.EQ.IXROW ) THEN
            ...
            IF( MYCOL.EQ.IXCOL )
     $         NQ = NQ-ICOFF
            ASUM = SCSUM1( NQ, X( IIX+(JJX-1)*LDX ), LDX )
            CALL SGSUM2D( ICTXT, 'Rowwise', RCTOP, 1, 1, ASUM, 1,
     $                    -1, MYCOL )
         END IF
*
      ELSE
...
*
         IF( MYCOL.EQ.IXCOL ) THEN
            ...
            IF( MYROW.EQ.IXROW )
     $         NP = NP-IROFF
            ASUM = SCSUM1( NP, X( IIX+(JJX-1)*LDX ), 1 )
            CALL SGSUM2D( ICTXT, 'Columnwise', CCTOP, 1, 1, ASUM, 1,
     $                    -1, MYCOL )
         END IF
*
      END IF
