...
*  =======
*
*  PCGELS solves overdetermined or underdetermined complex linear
...
*     .. External Functions ..
      ...
      EXTERNAL           ILCM, INDXG2P, LSAME, NUMROC, PCLANGE,
     $                   PSLAMCH
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, PCHK2MAT, PCGELQF,
     $                   PCGEQRF, PSLABAD, PCLASCL, PCLASET,
     $                   PCTRSM, PCUNMLQ, PCUNMQR, PXERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PCGELS', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
      IF( MIN( M, N, NRHS ).EQ.0 ) THEN
         CALL PCLASET( 'Full', MAX( M, N ), NRHS, CZERO, CZERO, B,
     $                 IB, JB, DESCB )
         RETURN
      END IF
*
...
*
      ...
      SMLNUM = SMLNUM / PSLAMCH( ICTXT, 'P' )
      BIGNUM = ONE / SMLNUM
      CALL PSLABAD( ICTXT, SMLNUM, BIGNUM )
*
*     Scale A, B if max entry outside range [SMLNUM,BIGNUM]
*
      ANRM = PCLANGE( 'M', M, N, A, IA, JA, DESCA, RWORK )
      IASCL = 0
      IF( ANRM.GT.ZERO .AND. ANRM.LT.SMLNUM ) THEN
*
*        Scale matrix norm up to SMLNUM
*
         CALL PCLASCL( 'G', ANRM, SMLNUM, M, N, A, IA, JA, DESCA,
     $                 INFO )
         IASCL = 1
...
*
         CALL PCLASCL( 'G', ANRM, BIGNUM, M, N, A, IA, JA, DESCA,
     $                 INFO )
         IASCL = 2
...
*
         CALL PCLASET( 'F', MAX( M, N ), NRHS, CZERO, CZERO, B, IB,
     $                 JB, DESCB )
         GO TO 10
      END IF
*
     ...
     $   BROW = N
*
      BNRM = PCLANGE( 'M', BROW, NRHS, B, IB, JB, DESCB, RWORK )
*
      IBSCL = 0
...
*        Scale matrix norm up to SMLNUM
*
         CALL PCLASCL( 'G', BNRM, SMLNUM, BROW, NRHS, B, IB, JB,
     $                 DESCB, INFO )
         IBSCL = 1
...
*        Scale matrix norm down to BIGNUM
*
         CALL PCLASCL( 'G', BNRM, BIGNUM, BROW, NRHS, B, IB, JB,
     $                 DESCB, INFO )
         IBSCL = 2
      END IF
*
...
*
         CALL PCGEQRF( M, N, A, IA, JA, DESCA, WORK, WORK( IPW ),
     $                 LWORK-LTAU, INFO )
*
...
*
            CALL PCUNMQR( 'Left', 'Conjugate transpose', M, NRHS, N, A,
...
*           sub( B ) := inv(R') * sub( B )
*
            CALL PCTRSM( 'Left', 'Upper', 'Conjugate transpose',
     $                   'Non-unit', N, NRHS, CONE, A, IA, JA, DESCA,
     $                   B, IB, JB, DESCB )
*
*           B(IB+N:IB+M-1,JB:JB+NRHS-1) = ZERO
*
            CALL PCLASET( 'All', M-N, NRHS, CZERO, CZERO, B, IB+N, JB,
     $                    DESCB )
*
*           B(IB:IB+M-1,JB:JB+NRHS-1) := Q(1:N,:) *
*                                        B(IB:IB+N-1,JB:JB+NRHS-1)
*
            CALL PCUNMQR( 'Left', 'No transpose', M, NRHS, N, A, IA, JA,
...
*
         CALL PCGELQF( M, N, A, IA, JA, DESCA, WORK, WORK( IPW ),
     $                 LWORK-LTAU, INFO )
*
...
*
            CALL PCTRSM( 'Left', 'Lower', 'No transpose', 'Non-unit', M,
     $                   NRHS, CONE, A, IA, JA, DESCA, B, IB, JB,
     $                   DESCB )
*
*           B(IB+M:IB+N-1,JB:JB+NRHS-1) = 0
*
            CALL PCLASET( 'All', N-M, NRHS, CZERO, CZERO, B, IB+M, JB,
     $                    DESCB )
*
*           B(IB:IB+N-1,JB:JB+NRHS-1) := Q(1:N,:)' *
*                                        B(IB:IB+M-1,JB:JB+NRHS-1)
*
            CALL PCUNMLQ( 'Left', 'Conjugate transpose', N, NRHS, M, A,
     $                    IA, JA, DESCA, WORK, B, IB, JB, DESCB,
     $                    WORK( IPW ), LWORK-LTAU, INFO )
*
*           workspace at least NRHS, optimally NRHS*NB
...
*
            CALL PCUNMLQ( 'Left', 'No transpose', N, NRHS, M, A, IA, JA,
     $                    DESCA, WORK, B, IB, JB, DESCB, WORK( IPW ),
     $                    LWORK-LTAU, INFO )
*
*           workspace at least NRHS, optimally NRHS*NB
...
*                                        B(IB:IB+M-1,JB:JB+NRHS-1)
*
            CALL PCTRSM( 'Left', 'Lower', 'Conjugate transpose',
...
*
      IF( IASCL.EQ.1 ) THEN
         CALL PCLASCL( 'G', ANRM, SMLNUM, SCLLEN, NRHS, B, IB, JB,
     $                 DESCB, INFO )
      ELSE IF( IASCL.EQ.2 ) THEN
         CALL PCLASCL( 'G', ANRM, BIGNUM, SCLLEN, NRHS, B, IB, JB,
     $                 DESCB, INFO )
      END IF
      IF( IBSCL.EQ.1 ) THEN
         CALL PCLASCL( 'G', SMLNUM, BNRM, SCLLEN, NRHS, B, IB, JB,
     $                 DESCB, INFO )
      ELSE IF( IBSCL.EQ.2 ) THEN
         CALL PCLASCL( 'G', BIGNUM, BNRM, SCLLEN, NRHS, B, IB, JB,
     $                 DESCB, INFO )
      END IF
*
   10 CONTINUE
