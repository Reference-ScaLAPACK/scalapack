      SUBROUTINE PSORMQL( SIDE, TRANS, M, N, K, A, IA, JA, DESCA, TAU,
...
*     .. External Subroutines ..
     ...
     $                   PSLARFT, PSORM2L, PB_TOPGET, PB_TOPSET, PXERBLA
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PSORMQL', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
     ...
     $    ( .NOT.LEFT .AND. .NOT.NOTRAN ) ) THEN
         ...
         ELSE
            NI = N - K + JB
         END IF
         CALL PSORM2L( SIDE, TRANS, MI, NI, JB, A, IA, JA, DESCA, TAU,
...
*
         CALL PSLARFT( 'Backward', 'Columnwise', NQ-K+J+JB-JA, JB,
...
*        Apply H or H'
*
         CALL PSLARFB( SIDE, TRANS, 'Backward', 'Columnwise', MI, NI,
