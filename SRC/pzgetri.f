      SUBROUTINE PZGETRI( N, A, IA, JA, DESCA, IPIV, WORK, LWORK,
     $                    IWORK, LIWORK, INFO )
*
...
*     .. External Subroutines ..
     ...
     $                   PZGEMM, PZLACPY, PZLASET, PZLAPIV,
     $                   PZTRSM, PZTRTRI, PXERBLA
*     ..
*     .. External Functions ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PZGETRI', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
     ...
     $   RETURN
*
*     Form inv(U).  If INFO > 0 from PZTRTRI, then U is singular,
*     and the inverse is not computed.
*
      CALL PZTRTRI( 'Upper', 'Non-unit', N, A, IA, JA, DESCA, INFO )
      IF( INFO.GT.0 )
     $   RETURN
*
...
*
         CALL PZLACPY( 'Lower', JA+N-1-J, JB, A, I+1, J, DESCA,
     $                 WORK, IW+J-JA+1, 1, DESCW )
         CALL PZLASET( 'Lower', JA+N-1-J, JB, ZERO, ZERO, A, I+1, J,
     $                 DESCA )
*
*        Compute current block column of inv(A).
*
         IF( J+JB.LE.JA+N-1 )
     $      CALL PZGEMM( 'No transpose', 'No transpose', N, JB,
     $                   JA+N-J-JB, -ONE, A, IA, J+JB, DESCA, WORK,
     $                   IW+J+JB-JA, 1, DESCW, ONE, A, IA, J, DESCA )
         CALL PZTRSM( 'Right', 'Lower', 'No transpose', 'Unit', N, JB,
...
*     Copy current block column of L to WORK and replace with zeros.
*
      CALL PZLACPY( 'Lower', N-1, JB, A, IA+1, JA, DESCA, WORK, IW+1,
     $              1, DESCW )
      CALL PZLASET( 'Lower', N-1, JB, ZERO, ZERO, A, IA+1, JA, DESCA )
*
*     Compute current block column of inv(A).
*
      IF( JA+JB.LE.JA+N-1 )
...
*
     ...
     $              MP+DESCA( MB_ ) )
      CALL PZLAPIV( 'Backward', 'Columns', 'Column', N, N, A, IA,
