      SUBROUTINE PSGEHRD( N, ILO, IHI, A, IA, JA, DESCA, TAU, WORK,
     $                    LWORK, INFO )
*
...
*     .. External Subroutines ..
     ...
     $                   INFOG2L, PCHK1MAT, PSGEMM, PSGEHD2,
     $                   PSLAHRD, PSLARFB, PB_TOPGET, PB_TOPSET, PXERBLA
*     ..
*     .. External Functions ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PSGEHRD', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
         CALL PSLAHRD( IHI, K, IB, A, IA, J, DESCA, TAU, WORK( IPT ),
...
*
         CALL PSELSET2( EI, A, I+IB, J+IB-1, DESCA, ONE )
         CALL PSGEMM( 'No transpose', 'Transpose', IHI, IHI-K-IB+1, IB,
     $                -ONE, WORK( IPY ), 1, JY, DESCY, A, I+IB, J,
     $                DESCA, ONE, A, IA, J+IB, DESCA )
         CALL PSELSET( A, I+IB, J+IB-1, DESCA, EI )
*
*        Apply the block reflector H to A(i+1:ia+ihi-1,j+ib:ja+n-1) from
*        the left
*
         CALL PSLARFB( 'Left', 'Transpose', 'Forward', 'Columnwise',
...
*
      CALL PSGEHD2( N, K, IHI, A, IA, JA, DESCA, TAU, WORK, LWORK,
     $              IINFO )
*
