...
*  =======
*
*  PCPTTRF computes a Cholesky factorization
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PCPTTRF', -INFO )
         RETURN
      END IF
*
...
*
        CALL CPTTRF( ODD_SIZE, D( PART_OFFSET+1 ), E( PART_OFFSET+1 ),
     $                    INFO )
*
        IF( INFO.NE.0 ) THEN
           INFO = MYCOL+1
...
*
          CALL CPTTRSV( 'L', 'N', ODD_SIZE, INT_ONE, D( PART_OFFSET+1 ),
