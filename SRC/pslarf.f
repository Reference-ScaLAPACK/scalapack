      SUBROUTINE PSLARF( SIDE, M, N, V, IV, JV, DESCV, INCV, TAU,
     $                   C, IC, JC, DESCC, WORK )
*
...
*  =======
*
*  PSLARF applies a real elementary reflector Q (or Q**T) to a real
...
*     .. External Subroutines ..
     ...
     $                   SCOPY, SGEBR2D, SGEBS2D, SGEMV,
     $                   SGER, SGERV2D, SGESD2D, SGSUM2D,
     $                   SLASET
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
     ...
     $                              WORK( IPW ), 1 )
                     ELSE
                        CALL SLASET( 'All', NQ, 1, ZERO, ZERO,
     $                               WORK( IPW ), MAX( 1, NQ ) )
                     END IF
                     CALL SGSUM2D( ICTXT, 'Columnwise', ' ', NQ, 1,
...
*
                     IPW = MP+1
                     CALL SCOPY( MP, V( IOFFV ), 1, WORK, 1 )
                     WORK( IPW ) = TAU( JJV )
...
*
                        IF( NQ.GT.0 ) THEN
                           CALL SGEMV( 'No transpose', MP, NQ, ONE,
     $                                 C( IOFFC ), LDC, V( IOFFV ), LDV,
     $                                 ZERO, WORK, 1 )
                        ELSE
                           CALL SLASET( 'All', MP, 1, ZERO, ZERO,
     $                                  WORK, MAX( 1, MP ) )
                        END IF
                        CALL SGSUM2D( ICTXT, 'Rowwise', ' ', MP, 1,
...
*
                     ...
                     WORK(IPW) = TAU( IIV )
                     CALL SGESD2D( ICTXT, IPW, 1, WORK, IPW, ICROW,
     $                             MYCOL )
*
...
*
     ...
     $                           WORK( IPW ), 1 )
                  ELSE
                     CALL SLASET( 'All', MP, 1, ZERO, ZERO, WORK( IPW ),
     $                            MAX( 1, MP ) )
                  END IF
                  CALL SGSUM2D( ICTXT, 'Rowwise', ' ', MP, 1,
     $                          WORK( IPW ), MAX( 1, MP ), RDEST,
