...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PZDBTRF', -INFO )
         RETURN
      END IF
*
...
*
        CALL ZDBTRF( ODD_SIZE, ODD_SIZE, BWL, BWU, A( OFST + 1),
     $                    LLDA, INFO )
*
        IF( INFO.NE.0 ) THEN
           INFO = MYCOL+1
...
*
*
          CALL ZGEMM( 'C', 'N', MAX_BW, MAX_BW, MAX_BW, -CONE ,
...
*
          CALL ZTBTRS( 'L', 'N', 'U', ODD_SIZE, BWL, BWL,
...
*
          CALL ZLATCPY( 'L', UP_PREV_TRI_SIZE_N, UP_PREV_TRI_SIZE_M,
     $                  A( OFST+1 ), LLDA-1, AF( 1 ), ODD_SIZE )
*
          CALL ZTBTRS( 'U', 'C', 'N', ODD_SIZE, BWU, BWU,
     $                    A( OFST + 1 ), LLDA,
     $                    AF( 1 ), ODD_SIZE, INFO )
*
*
...
*
   ...
   30   CONTINUE
*
          CALL ZGEMM( 'C', 'N', BWU, BWL, ODD_SIZE,
     $                    -CONE, AF( 1 ), ODD_SIZE,
...
*
          CALL ZGESD2D( ICTXT, MAX_BW, MAX_BW,
...
*
        IF( MYCOL.LT. NPCOL-1 ) THEN
*
           CALL ZGERV2D( ICTXT, MAX_BW, MAX_BW,
...
*
          IF( MYCOL-LEVEL_DIST .GE. 0 ) THEN
            CALL ZGERV2D( ICTXT, MAX_BW, MAX_BW, WORK( 1 ),
...
*
       CALL ZDBTRF( MAX_BW, MAX_BW, MIN( MAX_BW-1, BWL ),
...
*
          CALL ZGESD2D( ICTXT, MAX_BW, MAX_BW, WORK( 1 ), MAX_BW,
     $                     0, MYCOL+LEVEL_DIST )
*
        ENDIF
...
*
          CALL ZLATCPY( 'N', MAX_BW, MAX_BW, AF( WORK_U+ODD_SIZE*BWL+
...
*         Since ZTBTRS has no "left-right" option, we must transpose
*
          CALL ZLATCPY( 'N', MAX_BW, MAX_BW, AF( ODD_SIZE*BWU+
     $      2*MBW2+1), MAX_BW, WORK( 1 ), MAX_BW )
*
          CALL ZTBTRS(
...
*
          CALL ZGEMM( 'N', 'C', MAX_BW, MAX_BW, MAX_BW, -CONE,
     $                AF( (ODD_SIZE)*BWU+2*MBW2+1 ), MAX_BW,
...
*
              CALL ZGEMM( 'N', 'N', MAX_BW, MAX_BW, MAX_BW, -CONE,
     $                    AF( WORK_U+ODD_SIZE*BWL+2*MBW2+1 ), MAX_BW,
