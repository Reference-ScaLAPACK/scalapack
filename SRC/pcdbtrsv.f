...
*  =======
*
*  PCDBTRSV solves a banded triangular system of linear equations
*
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PCDBTRSV', -INFO )
         RETURN
      END IF
*
...
*
     ...
     $                WORK( 1 ), MAX_BW )
*
            CALL CTRMM( 'L', 'U', 'N', 'N', BWL, NRHS, -CONE,
     $                  A(( OFST+(BWL+BWU+1)+(ODD_SIZE-BWL)*LLDA )),
     $                  LLDA-1, WORK( 1 ), MAX_BW )
*
            CALL CMATADD( BWL, NRHS, CONE, WORK( 1 ), MAX_BW,
...
*
            CALL CGEMM( 'C', 'N', BWU, NRHS, ODD_SIZE, -CONE, AF( 1 ),
...
*
        IF( MYCOL .GT. 0) THEN
*
          CALL CGESD2D( ICTXT, MAX_BW, NRHS,
     $                     WORK( 1 ), MAX_BW,
     $                     0, MYCOL - 1 )
*
        ENDIF
...
*
          CALL CGERV2D( ICTXT, MAX_BW, NRHS,
     $                     WORK( 1 ), MAX_BW,
     $                     0, MYCOL + 1 )
*
*         Combine contribution to locally stored right hand sides
*
          CALL CMATADD( MAX_BW, NRHS, CONE,
     $                WORK( 1 ), MAX_BW, CONE,
...
*
        CALL CTBTRS( 'L', 'N', 'U', MAX_BW, MIN( BWL, MAX_BW-1 ), NRHS,
...
*
          CALL CGEMM( 'C', 'N', MAX_BW, NRHS, MAX_BW, -CONE,
     $                     AF( (ODD_SIZE)*BWU+1 ),
     $                     MAX_BW,
...
*
          CALL CGEMM( 'N', 'N', MAX_BW, NRHS, MAX_BW, -CONE,
...
*       Solve with diagonal block
*
        CALL CTBTRS( 'L', 'C', 'U', MAX_BW, MIN( BWL, MAX_BW-1 ), NRHS,
     $               AF( ODD_SIZE*BWU+MBW2+1 ), MAX_BW+1,
     $               B( PART_OFFSET+ODD_SIZE+1 ), LLDB, INFO )
*
        IF( INFO.NE.0 ) THEN
...
*
          CALL CGEMM( 'N', 'N', ODD_SIZE, NRHS, BWU, -CONE, AF( 1 ),
...
*
          CALL CMATADD( BWL, NRHS, CONE, WORK( 1+MAX_BW-BWL ), MAX_BW,
...
*
     ...
     $                WORK( 1 ), MAX_BW )
*
            CALL CTRMM( 'L', 'L', 'C', 'N', BWU, NRHS, -CONE,
     $                  A(( OFST+1+ODD_SIZE*LLDA )), LLDA-1, WORK( 1 ),
     $                  MAX_BW )
*
            CALL CMATADD( BWU, NRHS, CONE, WORK( 1 ), MAX_BW,
     $                CONE, B( PART_OFFSET+ODD_SIZE+1 ), LLDB )
*
        ENDIF
*
...
*           processor's righthand-side.
*
            CALL CGEMM( 'C', 'N', BWL, NRHS, ODD_SIZE, -CONE,
...
*       Solve with diagonal block
*
        CALL CTBTRS( 'U', 'C', 'N', MAX_BW, MIN( BWU, MAX_BW-1 ), NRHS,
...
*       Solve with diagonal block
*
        CALL CTBTRS( 'U', 'N', 'N', MAX_BW, MIN( BWU, MAX_BW-1 ), NRHS,
     $               AF( ODD_SIZE*BWU+MBW2+1-MIN( BWU, MAX_BW-1 ) ),
     $               MAX_BW+1, B( PART_OFFSET+ODD_SIZE+1 ), LLDB,
     $               INFO )
*
...
*           processor's solution.
*
          CALL CGEMM( 'N', 'N', ODD_SIZE, NRHS, BWL, -CONE,
...
*
     ...
     $                WORK( 1+MAX_BW-BWU ), MAX_BW+BWL )
*
          CALL CMATADD( BWU, NRHS, CONE, WORK( 1+MAX_BW-BWU ),
     $                  MAX_BW+BWL, CONE,
     $                  B( PART_OFFSET+ODD_SIZE-BWU+1 ), LLDB )
*
        ENDIF
