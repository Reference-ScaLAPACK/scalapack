      SUBROUTINE PZUNM2R( SIDE, TRANS, M, N, K, A, IA, JA, DESCA, TAU,
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PZUNM2R', -INFO )
...
*           Apply H(j) or H(j)'
*
            CALL PZELSET2( AJJ, A, IA+J-JA, J, DESCA, ONE )
            IF( NOTRAN ) THEN
               CALL PZLARF( SIDE, MI, NI, A, IA+J-JA, J, DESCA, 1, TAU,
     $                      C, ICC, JCC, DESCC, WORK )
            ELSE
               CALL PZLARFC( SIDE, MI, NI, A, IA+J-JA, J, DESCA, 1, TAU,
     $                    C, ICC, JCC, DESCC, WORK )
            END IF
            CALL PZELSET( A, IA+J-JA, J, DESCA, AJJ )
*
   10    CONTINUE
*
