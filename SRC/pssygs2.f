*
*
      SUBROUTINE PSSYGS2( IBTYPE, UPLO, N, A, IA, JA, DESCA, B, IB, JB,
     $                    DESCB, INFO )
*
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PSSYGS2', -INFO )
...
*
               ...
               A( IOFFA-LDA ) = AKK
               IF( K.LT.N ) THEN
                  CALL SSCAL( N-K, ONE / BKK, A( IOFFA ), LDA )
                  CT = -HALF*AKK
                  CALL SAXPY( N-K, CT, B( IOFFB ), LDB, A( IOFFA ),
     $                        LDA )
                  CALL SSYR2( UPLO, N-K, -ONE, A( IOFFA ), LDA,
...
*
               IF( K.LT.N ) THEN
                  CALL SSCAL( N-K, ONE / BKK, A( IOFFA ), 1 )
                  CT = -HALF*AKK
                  CALL SAXPY( N-K, CT, B( IOFFB ), 1, A( IOFFA ), 1 )
                  CALL SSYR2( UPLO, N-K, -ONE, A( IOFFA ), 1,
...
*
               AKK = A( IOFFA+K-1 )
               BKK = B( IOFFB+K-1 )
               CALL STRMV( UPLO, 'No transpose', 'Non-unit', K-1,
     $                     B( IIB+( JJB-1 )*LDB ), LDB, A( IOFFA ), 1 )
               CT = HALF*AKK
               CALL SAXPY( K-1, CT, B( IOFFB ), 1, A( IOFFA ), 1 )
               CALL SSYR2( UPLO, K-1, ONE, A( IOFFA ), 1, B( IOFFB ), 1,
     $                     A( IIA+( JJA-1 )*LDA ), LDA )
               CALL SAXPY( K-1, CT, B( IOFFB ), 1, A( IOFFA ), 1 )
               CALL SSCAL( K-1, BKK, A( IOFFA ), 1 )
...
*
               AKK = A( IOFFA+( K-1 )*LDA )
               BKK = B( IOFFB+( K-1 )*LDB )
               CALL STRMV( UPLO, 'Transpose', 'Non-unit', K-1,
     $                     B( IIB+( JJB-1 )*LDB ), LDB, A( IOFFA ),
     $                     LDA )
               CT = HALF*AKK
               CALL SAXPY( K-1, CT, B( IOFFB ), LDB, A( IOFFA ), LDA )
               CALL SSYR2( UPLO, K-1, ONE, A( IOFFA ), LDA, B( IOFFB ),
     $                     LDB, A( IIA+( JJA-1 )*LDA ), LDA )
               CALL SAXPY( K-1, CT, B( IOFFB ), LDB, A( IOFFA ), LDA )
               CALL SSCAL( K-1, BKK, A( IOFFA ), LDA )
