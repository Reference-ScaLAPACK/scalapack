...
*
            ...
            IF( MYCOL.EQ.IVCOL ) THEN
               CALL DGEBS2D( ICTXT, 'Rowwise', ROWBTOP, MPC, K,
     $                       V( IOFFV ), LDV )
               IF( MYROW.EQ.IVROW )
...
*
   10          CONTINUE
               IF( K-ITOP .GT.0 ) THEN
                  CALL DLASET( 'Upper', IINXT-IIBEG+1, K-ITOP, ZERO,
...
*
   20          CONTINUE
               IF( JJ.LE.( JJV+K-1 ) ) THEN
                  HEIGHT = IBASE - ITOP
                  CALL DLASET( 'All', KP, ITOP-JJ+JJV, ZERO, ZERO,
     $                         WORK( IPV+II-IIV+(JJ-JJV)*LV ), LV )
                  CALL DLASET( 'Lower', KP, HEIGHT, ZERO, ONE,
...
*
            IF( MPC.GT.0 ) THEN
               CALL DGEMM( 'Transpose', 'No transpose', NQC, K, MPC,
     $                    ONE, C( IOFFC ), LDC, WORK( IPV ), LV, ZERO,
     $                    WORK( IPW ), LW )
            ELSE
               CALL DLASET( 'All', NQC, K, ZERO, ZERO, WORK( IPW ), LW )
            END IF
*
            CALL DGSUM2D( ICTXT, 'Columnwise', ' ', NQC, K, WORK( IPW ),
     $                    LW, IVROW, MYCOL )
*
...
*
               CALL DTRMM( 'Right', UPLO, TRANST, 'Non unit', NQC, K,
     $                     ONE, T, NBV, WORK( IPW ), LW )
               CALL DGEBS2D( ICTXT, 'Columnwise', ' ', NQC, K,
     $                       WORK( IPW ), LW )
            ELSE
               CALL DGEBR2D( ICTXT, 'Columnwise', ' ', NQC, K,
...
*
            CALL DGEMM( 'No transpose', 'Transpose', MPC, NQC, K, -ONE,
...
*
   40             CONTINUE
     ...
     $                            WORK( IPW1+II-IIV+ITOP*LW ), LW )
            ...
            END IF
*
            CALL PBDTRAN( ICTXT, 'Columnwise', 'Transpose', N+IROFFV, K,
...
*
            IF( NQC.GT.0 ) THEN
               CALL DGEMM( 'No transpose', 'Transpose', MPC, K, NQC,
     $                     ONE, C( IOFFC ), LDC, WORK( IPV ), LV, ZERO,
     $                     WORK( IPW ), LW )
            ELSE
               CALL DLASET( 'All', MPC, K, ZERO, ZERO, WORK( IPW ), LW )
            END IF
*
            CALL DGSUM2D( ICTXT, 'Rowwise', ' ', MPC, K, WORK( IPW ),
     $                    LW, MYROW, IVCOL )
*
*           WORK( IPW ) = WORK( IPW ) * T' or WORK( IPW ) * T
*
...
*
     ...
     $                          'Non unit', K, K, T, NBV, IVROW, MYCOL )
               END IF
               CALL DTRMM( 'Right', UPLO, TRANS, 'Non unit', MPC, K,
     $                     ONE, T, NBV, WORK( IPW ), LW )
*
               CALL DGEBS2D( ICTXT, 'Rowwise', ' ', MPC, K, WORK( IPW ),
     $                       LW )
            ELSE
               CALL DGEBR2D( ICTXT, 'Rowwise', ' ', MPC, K, WORK( IPW ),
     $                       LW, MYROW, IVCOL )
            END IF
*
*               C            C      -     W       *     V'
...
*                        MPC x NQC    MPC x K         K x NQC
*
            CALL DGEMM( 'No transpose', 'No transpose', MPC, NQC, K,
...
*
   50             CONTINUE
                  IF( ( K-ILEFT ).GT.0 ) THEN
                     CALL DLASET( 'Lower', K-ILEFT, JJNXT-JJBEG+1, ZERO,
     $                            ONE,
...
*
   60             CONTINUE
                  IF( II.LE.( IIV+K-1 ) ) THEN
                     WIDE = IRIGHT - ILEFT
                     CALL DLASET( 'All', ILEFT-II+IIV, KQ, ZERO, ZERO,
     $                            WORK( IPW1+II-IIV+(JJ-JJV)*LW ), LW )
                     CALL DLASET( 'Upper', WIDE, KQ, ZERO, ONE,
...
*
            ...
            IF( MYROW.EQ.IVROW ) THEN
               CALL DGEBS2D( ICTXT, 'Columnwise', COLBTOP, K, NQC,
     $                       V( IOFFV ), LDV )
               IF( MYCOL.EQ.IVCOL )
