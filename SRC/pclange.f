      REAL               FUNCTION PCLANGE( NORM, M, N, A, IA, JA, DESCA,
     $                                     WORK )
      IMPLICIT NONE
*
...
*          Specifies the value to be returned in PCLANGE as described
*          above.
*
*  M       (global input) INTEGER
*          The number of rows to be operated on i.e the number of rows
*          of the distributed submatrix sub( A ). When M = 0, PCLANGE
*          is set to zero. M >= 0.
*
*  N       (global input) INTEGER
*          The number of columns to be operated on i.e the number of
*          columns of the distributed submatrix sub( A ). When N = 0,
*          PCLANGE is set to zero. N >= 0.
*
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CLASSQ, INFOG2L, SCOMBSSQ,
     $                   SGEBR2D, SGEBS2D, SGAMX2D, SGSUM2D,
     $                   PSTREECOMB
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
         ...
         IF( NQ.GT.0 ) THEN
             DO 70 J = IOFFA, IOFFA + NQ*LDA - 1, LDA
                COLSSQ(1) = ZERO
                COLSSQ(2) = ONE
                CALL CLASSQ( MP, A( J ), 1, COLSSQ(1), COLSSQ(2) )
