      SUBROUTINE PZLARFT( DIRECT, STOREV, N, K, V, IV, JV, DESCV, TAU,
     $                    T, WORK )
*
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, INFOG2L, ZCOPY, ZGEMV,
     $                   ZGSUM2D, ZLACGV, ZLASET, ZTRMV
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
               ...
               IF( NP-II+IIV.GT.0 ) THEN
                  CALL ZGEMV( 'Conjugate transpose', NP-II+IIV, ITMP0,
     $                        -TAU( JJ ), V( II+(JJV-1)*LDV ), LDV,
     $                        V( II+(JJ-1)*LDV ), 1, ZERO,
     $                        WORK( IW ), 1 )
               ELSE
                  CALL ZLASET( 'All', ITMP0, 1, ZERO, ZERO, WORK( IW ),
     $                         ITMP0 )
               END IF
*
               IW = IW + ITMP0
...
*
   10       CONTINUE
*
            CALL ZGSUM2D( ICTXT, 'Columnwise', ' ', IW-1, 1, WORK, IW-1,
     $                    IVROW, MYCOL )
*
            IF( MYROW.EQ.IVROW ) THEN
*
...
*
                  ...
                  CALL ZCOPY( ITMP0, WORK( IW ), 1, T( ITMP1 ), 1 )
                  IW = IW + ITMP0
*
...
*
               ...
               IF( II-IIV+1.GT.0 ) THEN
                  CALL ZGEMV( 'Conjugate transpose', II-IIV+1, ITMP0,
...
*
               ...
               IF( NQ-JJ+JJV.GT.0 ) THEN
                  CALL ZLACGV( NQ-JJ+JJV, V( II+(JJ-1)*LDV ), LDV )
                  CALL ZGEMV( 'No transpose', ITMP0, NQ-JJ+JJV,
     $                        -TAU(II), V( IIV+(JJ-1)*LDV ), LDV,
     $                        V( II+(JJ-1)*LDV ), LDV, ZERO,
     $                        WORK( IW ), 1 )
                  CALL ZLACGV( NQ-JJ+JJV, V( II+(JJ-1)*LDV ), LDV )
               ELSE
                  CALL ZLASET( 'All', ITMP0, 1, ZERO, ZERO,
     $                         WORK( IW ), ITMP0 )
               END IF
*
               IW = IW + ITMP0
...
*
   50       CONTINUE
*
            CALL ZGSUM2D( ICTXT, 'Rowwise', ' ', IW-1, 1, WORK, IW-1,
     $                    MYROW, IVCOL )
*
            IF( MYCOL.EQ.IVCOL ) THEN
*
...
*
               ...
               IF( JJ-JJV+1.GT.0 ) THEN
                  CALL ZLACGV( JJ-JJV+1, V( II+(JJV-1)*LDV ), LDV )
                  CALL ZGEMV( 'No transpose', ITMP0, JJ-JJV+1,
