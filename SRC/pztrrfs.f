...
*     .. External Subroutines ..
     ...
     $                   INFOG2L, PCHK1MAT, PCHK2MAT, PXERBLA, PZATRMV,
     $                   PZAXPY, PZCOPY, PZLACON, PZTRMV,
...
*
         CALL PZCOPY( N, X, IX, JX+K, DESCX, 1, WORK( IPR ), IW, JW,
     $                DESCW, 1 )
         CALL PZTRMV( UPLO, TRANS, DIAG, N, A, IA, JA, DESCA,
     $                WORK( IPR ), IW, JW, DESCW, 1 )
         CALL PZAXPY( N, -ONE, B, IB, JB+K, DESCB, 1, WORK( IPR ), IW,
     $                JW, DESCW, 1 )
*
...
*
   ...
   20          CONTINUE
         ...
         END IF
*
         CALL PZATRMV( UPLO, TRANS, DIAG, N, RONE, A, IA, JA, DESCA, X,
...
*
               CALL PZTRSV( UPLO, TRANST, DIAG, N, A, IA, JA, DESCA,
     $                      WORK( IPR ), IW, JW, DESCW, 1 )
               IF( MYCOL.EQ.IXBCOL ) THEN
                  IF( NP.GT.0 ) THEN
...
*
   ...
   70                CONTINUE
                  END IF
               END IF
               CALL PZTRSV( UPLO, TRANSN, DIAG, N, A, IA, JA, DESCA,
     $                      WORK( IPR ), IW, JW, DESCW, 1 )
            END IF
            GO TO 50
         END IF
...
*           where op(A) = A or A', depending on TRANS.
*
            CALL PZCOPY( N, X, IX, J+K, DESCX, 1, WORK( IPR ), IW, JW,
     $                   DESCW, 1 )
            CALL PZTRMV( UPLO, TRANS, DIAG, N, A, IA, JA, DESCA,
     $                   WORK( IPR ), IW, JW, DESCW, 1 )
            CALL PZAXPY( N, -ONE, B, IB, J+K, DESCB, 1, WORK( IPR ),
     $                   IW, JW, DESCW, 1 )
*
*           Compute componentwise relative backward error from formula
*
...
*
  ...
  100             CONTINUE
            ...
            END IF
*
            CALL PZATRMV( UPLO, TRANS, DIAG, N, RONE, A, IA, JA, DESCA,
