...
*  =======
*
*  PCPOTRS solves a system of linear equations
*
*                      sub( A ) * X = sub( B )
...
*          rization sub( A ) = L*L**H or U**H*U, as computed by PCPOTRF.
*
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, CHK1MAT, PCHK2MAT, PCTRSM,
     $                   PXERBLA
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PCPOTRS', -INFO )
         RETURN
      END IF
*
...
*
         CALL PCTRSM( 'Left', 'Lower', 'No transpose', 'Non-unit', N,
     $              NRHS, ONE, A, IA, JA, DESCA, B, IB, JB, DESCB )
*
*        Solve L'*X = sub( B ), overwriting sub( B ) with X.
*
