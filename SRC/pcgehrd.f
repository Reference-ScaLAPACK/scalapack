      SUBROUTINE PCGEHRD( N, ILO, IHI, A, IA, JA, DESCA, TAU, WORK,
     $                    LWORK, INFO )
      IMPLICIT NONE
*
...
*     .. External Subroutines ..
     ...
     $                   INFOG2L, PCGEMM, PCGEHD2, PCHK1MAT,
     $                   PCLAHRD, PCLARFB, PB_TOPGET, PB_TOPSET, PXERBLA
*     ..
*     .. External Functions ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PCGEHRD', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
         CALL PCLAHRD( IHI, K, IB, A, IA, J, DESCA, TAU, WORK( IPT ),
...
*
         CALL PCELSET2( EI, A, I+IB, J+IB-1, DESCA, ONE )
         CALL PCGEMM( 'No transpose', 'Conjugate transpose', IHI,
     $                IHI-K-IB+1, IB, -ONE, WORK( IPY ), 1, JY, DESCY,
     $                A, I+IB, J, DESCA, ONE, A, IA, J+IB, DESCA )
         CALL PCELSET( A, I+IB, J+IB-1, DESCA, EI )
*
...
*
      CALL PCGEHD2( N, K, IHI, A, IA, JA, DESCA, TAU, WORK, LWORK,
     $              IINFO )
*
