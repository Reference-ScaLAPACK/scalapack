      REAL             FUNCTION PSLANSY( NORM, UPLO, N, A, IA, JA,
     $                                   DESCA, WORK )
*
...
*  =======
*
*  PSLANSY  returns the value of the one norm, or the Frobenius norm,
...
*          Specifies the value to be returned in PSLANSY as described
*          above.
*
...
*          sub( A ). When N = 0, PSLANSY is set to zero. N >= 0.
*
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, PSCOL2ROW, PSTREECOMB,
     $                   SAXPY, SCOMBSSQ, SGAMX2D, SGSUM2D,
     $                   SGEBR2D, SGEBS2D, SLASSQ
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
            IF( MYCOL.EQ.IACOL ) THEN
               DO 370 K = (JJ-1)*LDA, (JJ+IB-2)*LDA, LDA
                  CALL SLASSQ( II-IIA, A( IIA+K ), 1, SCALE, SUM )
                  IF( MYROW.EQ.IAROW )
     $               II = II + 1
                  CALL SLASSQ( II-IIA, A( IIA+K ), 1, SCALE, SUM )
  370          CONTINUE
...
*
            IF( MYCOL.EQ.IACOL ) THEN
               DO 400 K = (JJ-1)*LDA, (JJ+IB-2)*LDA, LDA
                  CALL SLASSQ( IIA+NP-II, A( II+K ), 1, SCALE, SUM )
                  IF( MYROW.EQ.IAROW )
     $               II = II + 1
                  CALL SLASSQ( IIA+NP-II, A( II+K ), 1, SCALE, SUM )
  400          CONTINUE
