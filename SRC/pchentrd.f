      SUBROUTINE PCHENTRD( UPLO, N, A, IA, JA, DESCA, D, E, TAU, WORK,
...
*
*
*  PCHENTRD reduces a complex Hermitian matrix sub( A ) to Hermitian
...
*
         CALL PCTRMR2D( UPLO, 'N', N, N, A, IA, JA, DESCA, WORK( INDB ),
     $                  1, 1, DESCB, ICTXT )
*
*
...
*           way that PCHENTRD expects it.
*
         CALL PSLAMR1D( N-1, RWORK( INDRE ), 1, 1, DESCB, E, 1, JA,
     $                  DESCA )
*
         CALL PSLAMR1D( N, RWORK( INDRD ), 1, 1, DESCB, D, 1, JA,
     $                  DESCA )
*
         CALL PCLAMR1D( N, WORK( INDTAU ), 1, 1, DESCB, TAU, 1, JA,
     $                  DESCA )
*
         CALL PCTRMR2D( UPLO, 'N', N, N, WORK( INDB ), 1, 1, DESCB, A,
     $                  IA, JA, DESCA, ICTXT )
*
         IF( MYROWB.GE.0 )
...
*
            CALL PCHETD2( UPLO, MIN( N, NB ), A, IA, JA, DESCA, D, E,
...
*
               CALL PCHER2K( UPLO, 'No transpose', N-K-NB+1, NB, -CONE,
...
*        Use unblocked code to reduce the last or only block
*
            CALL PCHETD2( UPLO, KK, A, IA+K-1, JA+K-1, DESCA, D, E, TAU,
     $                    WORK, LWORK, IINFO )
         END IF
*
