...
*  =======
*
*  PZDTTRS solves a system of linear equations
*
...
*
      IF( N .GT. NP*NB-MOD( JA-1, NB )) THEN
         INFO = -( 2 )
         CALL PXERBLA( ICTXT,
     $      'PZDTTRS, D&C alg.: only 1 block per proc',
     $      -INFO )
         RETURN
      ENDIF
*
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PZDTTRS', -INFO )
         RETURN
      END IF
*
...
*
      IF( LSAME( TRANS, 'N' ) ) THEN
*
         CALL PZDTTRSV( 'L', 'N', N, NRHS, DL( PART_OFFSET+1 ),
...
*
      ELSE
*
         CALL PZDTTRSV( 'U', 'C', N, NRHS, DL( PART_OFFSET+1 ),
     $                  D( PART_OFFSET+1 ), DU( PART_OFFSET+1 ), JA_NEW,
     $                  DESCA_1XP, B, IB, DESCB_PX1, AF, LAF, WORK,
     $                  LWORK, INFO )
*
...
*
         CALL PZDTTRSV( 'L', 'C', N, NRHS, DL( PART_OFFSET+1 ),
     $                  D( PART_OFFSET+1 ), DU( PART_OFFSET+1 ), JA_NEW,
     $                  DESCA_1XP, B, IB, DESCB_PX1, AF, LAF, WORK,
     $                  LWORK, INFO )
*
      ELSE
*
         CALL PZDTTRSV( 'U', 'N', N, NRHS, DL( PART_OFFSET+1 ),
     $                  D( PART_OFFSET+1 ), DU( PART_OFFSET+1 ), JA_NEW,
     $                  DESCA_1XP, B, IB, DESCB_PX1, AF, LAF, WORK,
     $                  LWORK, INFO )
*
