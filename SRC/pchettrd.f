...
*
     ...
     $                   PBMIN, PBSIZE, PNB, ROWSPERPROC
      ...
      COMPLEX            ALPHA, BETA, C, ONEOVERBETA, TOPH, TOPNV,
     $                   TOPTAU, TOPV, TTOPH, TTOPV
*     ..
*     .. Local Arrays ..
*
*
...
*
      ...
      ELSE
*
*     Here we set execution options for PCHETTRD
*
         PNB = PJLAENV( ICTXT, 2, 'PCHETTRD', 'L', 0, 0, 0, 0 )
         ANB = PJLAENV( ICTXT, 3, 'PCHETTRD', 'L', 0, 0, 0, 0 )
*
         INTERLEAVE = ( PJLAENV( ICTXT, 4, 'PCHETTRD', 'L', 1, 0, 0,
     $                0 ).EQ.1 )
         TWOGEMMS = ( PJLAENV( ICTXT, 4, 'PCHETTRD', 'L', 2, 0, 0,
     $              0 ).EQ.1 )
         BALANCED = ( PJLAENV( ICTXT, 4, 'PCHETTRD', 'L', 3, 0, 0,
     $              0 ).EQ.1 )
*
...
*
            IF( MYCOL.EQ.CURCOL ) THEN
               ...
               CALL CGEBS2D( ICTXT, 'R', ' ', NPM1+NPM1+1, 1,
     $                       WORK( INV+LIIP1-1+BINDEX*LDV ),
     $                       NPM1+NPM1+1 )
            ELSE
               CALL CGEBR2D( ICTXT, 'R', ' ', NPM1+NPM1+1, 1,
     $                       WORK( INV+LIIP1-1+BINDEX*LDV ),
...
*
   ...
   60          CONTINUE
            ELSE
               CALL CGESD2D( ICTXT, NPM1+NPM1, 1,
     $                       WORK( INV+LIIP1-1+BINDEX*LDV ), NPM1+NPM1,
     $                       MYCOL, MYROW )
               CALL CGERV2D( ICTXT, NQM1+NQM1, 1,
...
*           are not always involved in the calls to CTRMVT
*           for two reasons:
...
*
  ...
  100          CONTINUE
            ELSE
               IF( LTNM1.GT.0 )
     $            CALL CTRMVT( 'L', LTNM1, A( LTLIP1+( LIJP1-1 )*LDA ),
...
*     tmp = HT * v
*
               CALL CGEMV( 'C', NUMROWS, BINDEX+1, Z_ONE,
...
*
*
               CALL CGSUM2D( ICTXT, 'C', ' ', 2*( BINDEX+1 ), 1,
...
*
               CALL CGSUM2D( ICTXT, 'R', ' ', 2*( BINDEX+1 ), 1,
     $                       WORK( INTMP ), 2*( BINDEX+1 ), -1, -1 )
*
*
*     v = v + V * tmp
               IF( INDEX.GT.1. ) THEN
                  CALL CGEMV( 'N', NUMROWS, BINDEX+1, Z_NEGONE,
...
*
  ...
  120          CONTINUE
            ELSE
               CALL CGESD2D( ICTXT, NQM1, 1,
     $                       WORK( INVT+LIJP1-1+( BINDEX+1 )*LDV ),
     $                       NQM1, MYCOL, MYROW )
               CALL CGERV2D( ICTXT, NPM1, 1, WORK( INTMP ), NPM1, MYCOL,
     $                       MYROW )
*
            END IF
            DO 130 I = 0, NPM1 - 1
