      SUBROUTINE PZDTSV( N, NRHS, DL, D, DU, JA, DESCA, B, IB, DESCB,
     $                   WORK, LWORK, INFO )
      IMPLICIT NONE
*
*
...
*     .. External Subroutines ..
      EXTERNAL           PXERBLA, PZDTTRF, PZDTTRS
*     ..
...
*     Factor the matrix
*
      CALL PZDTTRF( N, DL, D, DU, JA, DESCA, WORK,
...
*
      IF( INFO.NE.0 ) THEN
         IF( INFO .LT. 0 ) THEN
            CALL PXERBLA( ICTXT, 'PZDTSV', -INFO )
         ENDIF
         RETURN
      END IF
*
...
*
      CALL PZDTTRS( 'N', N, NRHS, DL, D, DU, JA, DESCA, B, IB, DESCB,
