      SUBROUTINE PDLABRD( M, N, NB, A, IA, JA, DESCA, D, E, TAUQ, TAUP,
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, DESCSET, INFOG2L, PDCOPY,
     $                   PDELGET, PDELSET, PDGEMV, PDLARFG,
     $                   PDSCAL
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MIN, MOD
...
*
            IF( K.GT.1 ) THEN
               CALL PDGEMV( 'No transpose', M-K+1, K-1, -ONE, A, I, JA,
     $                      DESCA, Y, IY, JY+K-1, DESCY, 1, ONE, A, I,
     $                      J, DESCA, 1 )
               CALL PDGEMV( 'No transpose', M-K+1, K-1, -ONE, X, IX+K-1,
...
*
            CALL PDLARFG( M-K+1, ALPHA, I, J, A, I+1, J, DESCA, 1,
     $                    TAUQ )
...
*
            CALL PDGEMV( 'Transpose', M-K+1, N-K, ONE, A, I, J+1, DESCA,
     $                   A, I, J, DESCA, 1, ZERO, WORK( IPY ), 1, JWY,
     $                   DESCWY, DESCWY( M_ ) )
            CALL PDGEMV( 'Transpose', M-K+1, K-1, ONE, A, I, JA, DESCA,
     $                   A, I, J, DESCA, 1, ZERO, WORK, IW, 1, DESCW,
     $                   1 )
            CALL PDGEMV( 'Transpose', K-1, N-K, -ONE, Y, IY, JY+K,
     $                   DESCY, WORK, IW, 1, DESCW, 1, ONE, WORK( IPY ),
     $                   1, JWY, DESCWY, DESCWY( M_ ) )
            CALL PDGEMV( 'Transpose', M-K+1, K-1, ONE, X, IX+K-1, JX,
...
*
            CALL PDELGET( 'Rowwise', ' ', TAU, TAUQ, 1, J, DESCTQ )
            CALL PDSCAL( N-K, TAU, WORK( IPY ), 1, JWY, DESCWY,
     $                   DESCWY( M_ ) )
            CALL PDCOPY( N-K, WORK( IPY ), 1, JWY, DESCWY, DESCWY( M_ ),
     $                   Y, IY+K-1, JY+K, DESCY, DESCY( M_ ) )
*
*           Update A(i,j+1:ja+n-1)
*
            CALL PDGEMV( 'Transpose', K, N-K, -ONE, Y, IY, JY+K, DESCY,
...
*
            CALL PDGEMV( 'No transpose', M-K, N-K, ONE, A, I+1, J+1,
     $                   DESCA, A, I, J+1, DESCA, DESCA( M_ ), ZERO, X,
     $                   IX+K, JX+K-1, DESCX, 1 )
            CALL PDGEMV( 'No transpose', K, N-K, ONE, Y, IY, JY+K,
     $                   DESCY, A, I, J+1, DESCA, DESCA( M_ ), ZERO,
     $                   WORK, IW, 1, DESCW, 1 )
            CALL PDGEMV( 'No transpose', M-K, K, -ONE, A, I+1, JA,
     $                   DESCA, WORK, IW, 1, DESCW, 1, ONE, X, IX+K,
     $                   JX+K-1, DESCX, 1 )
            CALL PDGEMV( 'No transpose', K-1, N-K, ONE, A, IA, J+1,
     $                   DESCA, A, I, J+1, DESCA, DESCA( M_ ), ZERO,
     $                   WORK, IW, 1, DESCW, 1 )
            CALL PDGEMV( 'No transpose', M-K, K-1, -ONE, X, IX+K, JX,
     $                   DESCX, WORK, IW, 1, DESCW, 1, ONE, X, IX+K,
     $                   JX+K-1, DESCX, 1 )
*
            CALL PDELGET( 'Columnwise', ' ', TAU, TAUP, I, 1, DESCTP )
            CALL PDSCAL( M-K, TAU, X, IX+K, JX+K-1, DESCX, 1 )
   10    CONTINUE
*
      ELSE
*
...
*
            IF( K.GT.1 ) THEN
               CALL PDGEMV( 'Transpose', K-1, N-K+1, -ONE, Y, IY,
     $                      JY+K-1, DESCY, A, I, JA, DESCA, DESCA( M_ ),
     $                      ONE, A, I, J, DESCA, DESCA( M_ ) )
               CALL PDGEMV( 'Transpose', K-1, N-K+1, -ONE, A, IA, J,
...
*
            CALL PDLARFG( N-K+1, ALPHA, I, J, A, I, J+1, DESCA,
     $                    DESCA( M_ ), TAUP )
...
*           Compute X(i+1:ia+m-1,j)
*
            CALL PDGEMV( 'No transpose', M-K, N-K+1, ONE, A, I+1, J,
     $                   DESCA, A, I, J, DESCA, DESCA( M_ ), ZERO, X,
     $                   IX+K, JX+K-1, DESCX, 1 )
            CALL PDGEMV( 'No transpose', K-1, N-K+1, ONE, Y, IY, JY+K-1,
     $                   DESCY, A, I, J, DESCA, DESCA( M_ ), ZERO,
     $                   WORK, IW, 1, DESCW, 1 )
            CALL PDGEMV( 'No transpose', M-K, K-1, -ONE, A, I+1, JA,
     $                   DESCA, WORK, IW, 1, DESCW, 1, ONE, X, IX+K,
     $                   JX+K-1, DESCX, 1 )
            CALL PDGEMV( 'No transpose', K-1, N-K+1, ONE, A, IA, J,
...
*
     ...
     $                   DESCA, 1 )
            CALL PDGEMV( 'No transpose', M-K, K, -ONE, X, IX+K, JX,
...
*
            CALL PDLARFG( M-K, ALPHA, I+1, J, A, MIN( I+2, M+IA-1 ),
     $                    J, DESCA, 1, TAUQ )
...
*           Compute Y(ia+i:ia+n-1,j)
*
            CALL PDGEMV( 'Transpose', M-K, N-K, ONE, A, I+1, J+1, DESCA,
     $                   A, I+1, J, DESCA, 1, ZERO, WORK( IPY ), 1,
     $                   JWY, DESCWY, DESCWY( M_ ) )
            CALL PDGEMV( 'Transpose', M-K, K-1, ONE, A, I+1, JA, DESCA,
     $                   A, I+1, J, DESCA, 1, ZERO, WORK, IW, 1, DESCW,
     $                   1 )
            CALL PDGEMV( 'Transpose', K-1, N-K, -ONE, Y, IY, JY+K,
     $                   DESCY, WORK, IW, 1, DESCW, 1, ONE, WORK( IPY ),
     $                   1, JWY, DESCWY, DESCWY( M_ ) )
            CALL PDGEMV( 'Transpose', M-K, K, ONE, X, IX+K, JX, DESCX,
     $                   A, I+1, J, DESCA, 1, ZERO, WORK, IW, 1, DESCW,
     $                   1 )
            CALL PDGEMV( 'Transpose', K, N-K, -ONE, A, IA, J+1, DESCA,
     $                   WORK, IW, 1, DESCW, 1, ONE, WORK( IPY ), 1,
     $                   JWY, DESCWY, DESCWY( M_ ) )
*
            CALL PDELGET( 'Rowwise', ' ', TAU, TAUQ, 1, J, DESCTQ )
