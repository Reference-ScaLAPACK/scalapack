      SUBROUTINE PDORM2L( SIDE, TRANS, M, N, K, A, IA, JA, DESCA, TAU,
...
*
      ...
      END IF
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PDORM2L', -INFO )
...
*           Apply H(j) or H(j)'
*
            CALL PDELSET2( AJJ, A, IA+NQ-K+J-JA, J, DESCA, ONE )
            CALL PDLARF( SIDE, MI, NI, A, IA, J, DESCA, 1, TAU, C, IC,
     $                   JC, DESCC, WORK )
            CALL PDELSET( A, IA+NQ-K+J-JA, J, DESCA, AJJ )
*
   10    CONTINUE
*
