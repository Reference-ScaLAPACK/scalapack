      SUBROUTINE PZGESVX( FACT, TRANS, N, NRHS, A, IA, JA, DESCA, AF,
...
*  =======
*
*  PZGESVX uses the LU factorization to compute the solution to a
...
*     .. External Subroutines ..
     ...
     $                   DGEBR2D, DGEBS2D, DGAMN2D,
     $                   DGAMX2D, INFOG2L, PDCOPY, PXERBLA,
     $                   PZGECON, PZGEEQU, PZGERFS,
     $                   PZGETRF, PZGETRS, PZLACPY, PZLAQGE
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      INTEGER            ICEIL, ILCM, INDXG2P, NUMROC
      DOUBLE PRECISION   PDLAMCH, PZLANGE
      EXTERNAL           ICEIL, ILCM, INDXG2P, LSAME, NUMROC, PZLANGE,
     $                   PDLAMCH
*     ..
*     .. Intrinsic Functions ..
            ...
            ELSE
               IF( ROWEQU ) THEN
                  ...
                  DO 10 J = IIA, IIA + NP - 1
                     ...
                     RCMAX = MAX( RCMAX, R( J ) )
   10             CONTINUE
                  CALL DGAMN2D( ICTXT, 'Columnwise', ' ', 1, 1, RCMIN,
     $                          1, IDUMM, IDUMM, -1, -1, MYCOL )
                  CALL DGAMX2D( ICTXT, 'Columnwise', ' ', 1, 1, RCMAX,
     $                          1, IDUMM, IDUMM, -1, -1, MYCOL )
     ...
     $                        MIN( RCMAX, BIGNUM )
               ...
               IF( COLEQU .AND. INFO.EQ.0 ) THEN
                  ...
                  DO 20 J = JJA, JJA+NQ-1
                     ...
                     RCMAX = MAX( RCMAX, C( J ) )
   20             CONTINUE
                  CALL DGAMN2D( ICTXT, 'Rowwise', ' ', 1, 1, RCMIN,
     $                          1, IDUMM, IDUMM, -1, -1, MYCOL )
                  CALL DGAMX2D( ICTXT, 'Rowwise', ' ', 1, 1, RCMAX,
     $                          1, IDUMM, IDUMM, -1, -1, MYCOL )
                  IF( RCMIN.LE.ZERO ) THEN
                     INFO = -15
                  ELSE IF( N.GT.0 ) THEN
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PZGESVX', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
         CALL PZGEEQU( N, N, A, IA, JA, DESCA, R, C, ROWCND, COLCND,
     $                 AMAX, INFEQU )
         IF( INFEQU.EQ.0 ) THEN
*
*           Equilibrate the matrix.
*
            CALL PZLAQGE( N, N, A, IA, JA, DESCA, R, C, ROWCND, COLCND,
     $                    AMAX, EQUED )
            ROWEQU = LSAME( EQUED, 'R' ) .OR. LSAME( EQUED, 'B' )
            COLEQU = LSAME( EQUED, 'C' ) .OR. LSAME( EQUED, 'B' )
         END IF
...
*
     ...
     $                 IACOL, ICTXT, 1 )
         CALL PDCOPY( N, C, 1, JA, CDESC, CDESC( LLD_ ), RWORK, IB, JB,
     $                DESCB, 1 )
         IF( MYCOL.EQ.IBCOL ) THEN
            CALL DGEBS2D( ICTXT, 'Rowwise', ' ', NP, 1, RWORK( IIB ),
     $                    DESCB( LLD_ ) )
         ELSE
            CALL DGEBR2D( ICTXT, 'Rowwise', ' ', NP, 1, RWORK( IIB ),
...
*
     ...
     $                 DESCAF )
         CALL PZGETRF( N, N, AF, IAF, JAF, DESCAF, IPIV, INFO )
*
...
*
      CALL PZGECON( NORM, N, AF, IAF, JAF, DESCAF, ANORM, RCOND, WORK,
...
*
     ...
     $              DESCX )
      CALL PZGETRS( TRANS, N, NRHS, AF, IAF, JAF, DESCAF, IPIV, X, IX,
     $              JX, DESCX, INFO )
*
*     Use iterative refinement to improve the computed solution and
*     compute error bounds and backward error estimates for it.
*
      CALL PZGERFS( TRANS, N, NRHS, A, IA, JA, DESCA, AF, IAF, JAF,
...
*
            CALL DESCSET( CDESC, 1, N+ICOFFA, 1, DESCA( NB_ ), MYROW,
     $                    IACOL, ICTXT, 1 )
            CALL PDCOPY( N, C, 1, JA, CDESC, CDESC( LLD_ ), RWORK, IX,
     $                   JX, DESCX, 1 )
            IF( MYCOL.EQ.IBCOL ) THEN
