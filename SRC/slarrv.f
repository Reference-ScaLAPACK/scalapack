      SUBROUTINE SLARRV_CV( N, VL, VU, D, L, PIVMIN,
...
*     .. External Subroutines ..
      EXTERNAL           SAXPY, SCOPY, SLAR1V_CV, SLARRB_CV, SLARRF_CV,
     $                   SLASET, SSCAL
*     ..
*     .. Intrinsic Functions ..
...
C                    SLARRF needs LWORK = 2*N
                     CALL SLARRF_CV( IN, D( IBEGIN ), L( IBEGIN ),
     $                         WORK(INDLD+IBEGIN-1), 
...
*                    Given LAMBDA, compute the eigenvector.
                     CALL SLAR1V_CV( IN, 1, IN, LAMBDA, D( IBEGIN ),
