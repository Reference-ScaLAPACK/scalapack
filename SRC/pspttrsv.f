      SUBROUTINE PSPTTRSV( UPLO, N, NRHS, D, E, JA, DESCA, B, IB, DESCB,
...
*     .. External Subroutines ..
     ...
     $                   GLOBCHK, PXERBLA, RESHAPE, SAXPY, SGEMM,
     $                   SGERV2D, SGESD2D, SMATADD, SPTTRSV, STRTRS
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      INTEGER            NUMROC
...
*
      IF( ( JA+N-1.GT.NB ) .AND. ( NB.LT.2*INT_ONE ) ) THEN
         INFO = -( 7*100+4 )
         CALL PXERBLA( ICTXT, 'PSPTTRSV, D&C alg.: NB too small',
     $                 -INFO )
         RETURN
      END IF
*
...
*
      IF( LWORK.LT.WORK_SIZE_MIN ) THEN
         IF( LWORK.NE.-1 ) THEN
            INFO = -14
            CALL PXERBLA( ICTXT, 'PSPTTRSV: worksize error', -INFO )
         END IF
         RETURN
      END IF
*
...
*
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( ICTXT, 'PSPTTRSV', -INFO )
         RETURN
      END IF
*
...
*
         CALL SPTTRSV( 'N', ODD_SIZE, NRHS, D( PART_OFFSET+1 ),
...
*
            CALL SAXPY( NRHS, -E( PART_OFFSET+ODD_SIZE ),
...
*
            CALL SGEMM( 'T', 'N', 1, NRHS, ODD_SIZE, -ONE, AF( 1 ),
...
*
         IF( MYCOL.GT.0 ) THEN
*
            CALL SGESD2D( ICTXT, INT_ONE, NRHS, WORK( 1 ), INT_ONE, 0,
     $                    MYCOL-1 )
*
         END IF
*
...
*
            CALL SGERV2D( ICTXT, INT_ONE, NRHS, WORK( 1 ), INT_ONE, 0,
     $                    MYCOL+1 )
*
*         Combine contribution to locally stored right hand sides
*
            CALL SMATADD( INT_ONE, NRHS, ONE, WORK( 1 ), INT_ONE, ONE,
     $                    B( PART_OFFSET+ODD_SIZE+1 ), LLDB )
*
         END IF
*
...
*
         CALL STRTRS( 'L', 'N', 'U', INT_ONE, NRHS, AF( ODD_SIZE+2 ),
...
*       Solve with diagonal block
*
         CALL STRTRS( 'L', 'T', 'U', INT_ONE, NRHS, AF( ODD_SIZE+2 ),
     $                INT_ONE, B( PART_OFFSET+ODD_SIZE+1 ), LLDB, INFO )
*
         IF( INFO.NE.0 ) THEN
            GO TO 90
...
*
            CALL SGEMM( 'N', 'N', ODD_SIZE, NRHS, 1, -ONE, AF( 1 ),
...
*       Use main partition in each processor to solve locally
*
         CALL SPTTRSV( 'T', ODD_SIZE, NRHS, D( PART_OFFSET+1 ),
     $                 E( PART_OFFSET+1 ), B( PART_OFFSET+1 ), LLDB,
     $                 INFO )
*
*
