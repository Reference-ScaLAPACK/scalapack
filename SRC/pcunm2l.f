      SUBROUTINE PCUNM2L( SIDE, TRANS, M, N, K, A, IA, JA, DESCA, TAU,
...
*
     ...
     $                        DESCC( LLD_ ) )
               ELSE
                  CALL CGEBR2D( ICTXT, 'Rowwise', ' ', 1, 1, AJJ, 1,
     $                          IAROW, IACOL )
                  CALL CSCAL( NQ-JCC+1, AJJ,
     $                        C( ICC+(JCC-1)*DESCC( LLD_ ) ),
     $                        DESCC( LLD_ ) )
...
*
            IF( IACOL.NE.ICCOL ) THEN
               IF( MYCOL.EQ.IACOL )
     $            CALL CGESD2D( ICTXT, 1, 1, AJJ, 1, MYROW, ICCOL )
               IF( MYCOL.EQ.ICCOL )
     $            CALL CGERV2D( ICTXT, 1, 1, AJJ, 1, MYROW, IACOL )
            END IF
*
            IF( MYCOL.EQ.ICCOL ) THEN
               MP = NUMROC( IC+M-1, DESCC( MB_ ), MYROW, DESCC( RSRC_ ),
     $                      NPROW )
               CALL CSCAL( MP-ICC+1, AJJ, C( ICC+(JCC-1)*
     $                     DESCC( LLD_ ) ), 1 )
            END IF
*
         END IF
