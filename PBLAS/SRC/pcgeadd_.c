...
*  =======
*
*  PCGEADD  adds a matrix to another
*
...
*/
   ...
   if( !( info = ( ( nprow == -1 ) ? -( 801 + CTXT_ ) : 0 ) ) )
   {
      ...
      {
         PB_Cwarn( ctxt, __LINE__, "PCGEADD", "Illegal TRANS = %c\n", TrA );
         info = -1;
      }
      if( notran )
         PB_Cchkmat( ctxt, "PCGEADD", "A", *M, 2, *N, 3, Ai, Aj, Ad,  8,
                     &info );
      else
         PB_Cchkmat( ctxt, "PCGEADD", "A", *N, 3, *M, 2, Ai, Aj, Ad,  8,
                     &info );
      PB_Cchkmat( ctxt, "PCGEADD", "C", *M, 2, *N, 3, Ci, Cj, Cd, 13, &info );
   }
   if( info ) { PB_Cabort( ctxt, "PCGEADD", info ); return; }
#endif
/*
...
*/
   if( ( ALPHA[REAL_PART] == ZERO ) && ( ALPHA[IMAG_PART] == ZERO ) )
   {
      ...
      {
         PB_Cplapad( PB_Cctypeset(), ALL, NOCONJG, *M, *N, ((char *)BETA),
                     ((char *)BETA), ((char *) C), Ci, Cj, Cd );
      }
      else
      {
         PB_Cplascal( PB_Cctypeset(), ALL, NOCONJG, *M, *N, ((char *)BETA),
                      ((char * )C), Ci, Cj, Cd );
      }
      return;
   }
...
*/
   if( notran )
   {
      ...
      PB_Cpgeadd( PB_Cctypeset(), &DirA, &DirC, NOCONJG, *M, *N,
                  ((char *) ALPHA), ((char *) A), Ai, Aj, Ad,
                  ((char *)  BETA), ((char *) C), Ci, Cj, Cd );
   }
   else if( TrA == CTRAN )
   {
      PB_Cptran( PB_Cctypeset(), NOCONJG, *M, *N, ((char *) ALPHA),
                 ((char *) A), Ai, Aj, Ad, ((char *)  BETA), ((char *) C),
                 Ci, Cj, Cd );
   }
   else
   {
      PB_Cptran( PB_Cctypeset(), CONJG,   *M, *N, ((char *) ALPHA),
                 ((char *) A), Ai, Aj, Ad, ((char *)  BETA), ((char *) C),
                 Ci, Cj, Cd );
   }
/*
