...
*  =======
*
*  PDZASUM  returns  the sum of absolute values of the entries of a sub-
*  vector sub( X ),
*
*  where
*
...
*/
   ...
   if( !( info = ( ( nprow == -1 ) ? -( 601 + CTXT_ ) : 0 ) ) )
      PB_Cchkvec( ctxt, "PDZASUM", "X", *N, 1, Xi, Xj, Xd, *INCX, 6, &info );
   if( info ) { PB_Cabort( ctxt, "PDZASUM", info ); return; }
#endif
/*
*  Initialize ASUM
*/
...
*/
          ...
          ( ( mycol == Xcol ) || ( Xcol < 0 ) ) )
      {
         ...
         *ASUM = ABS( ((double *) Xptr)[REAL_PART] ) +
                 ABS( ((double *) Xptr)[IMAG_PART] );
      }
      return;
   }
   else if( *INCX == Xd[M_] )
...
*/
         ...
         if( Xnq > 0 )
         {
            Xld = Xd[LLD_];
            type = PB_Cztypeset();
            dzvasum_( &Xnq, ((char *) ASUM), Mptr( ((char *) X), Xii,
