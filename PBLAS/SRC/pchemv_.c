...
*  =======
*
*  PCHEMV  performs the matrix-vector operation
*
...
*/
   if( !( info = ( ( nprow == -1 ) ? -( 701 + CTXT_ ) : 0 ) ) )
   {
      ...
      PB_Cchkmat( ctxt, "PCHEMV", "A", *N, 2, *N, 2, Ai, Aj, Ad,  7, &info );
      PB_Cchkvec( ctxt, "PCHEMV", "X", *N, 2, Xi, Xj, Xd, *INCX, 11, &info );
      PB_Cchkvec( ctxt, "PCHEMV", "Y", *N, 2, Yi, Yj, Yd, *INCY, 17, &info );
   }
   if( info ) { PB_Cabort( ctxt, "PCHEMV", info ); return; }
#endif
/*
...
*/
            ...
            if( Ynq > 0 )
            {
               ...
               {
                  cset_( &Ynq, ((char *) BETA), Mptr( ((char *) Y), Yii,
                         Yjj, Yld, type->size ), &Yld );
               }
               else
               {
                  cscal_( &Ynq, ((char *) BETA), Mptr( ((char *) Y), Yii,
                          Yjj, Yld, type->size ), &Yld );
               }
            }
         }
...
*/
               if( Anq > 0 )
               {
                  ...
                  {
                     cset_( &Anq, ((char *) tbeta), YR, &YRld );
                  }
                  else
                  {
                     cscal_( &Anq, ((char *) tbeta), YR, &YRld );
                  }
               }
            }
         }
...
*/
               if( Amp > 0 )
               {
                  if( ( tbeta[REAL_PART] == ZERO ) &&
                      ( tbeta[IMAG_PART] == ZERO ) )
                  {
                     cset_( &Amp, ((char *) tbeta), YC, &ione );
                  }
                  else
                  {
                     cscal_( &Amp, ((char *) tbeta), YC, &ione );
                  }
               }
            }
         }
...
*/
      ...
      if( upper )
      {
         for( k = 0; k < *N; k += nb )
         {
            ...
            if( Akp > 0 && Anq0 > 0 )
            {
               cgemv_( C2F_CHAR( NOTRAN ), &Akp, &Anq0, ((char *)ALPHA),
                       Mptr( Aptr, 0, Akq, Ald, size ), &Ald, Mptr( XR, 0, Akq,
                       XRld, size ), &XRld, one, YC, &ione );
               cgemv_( C2F_CHAR( COTRAN ), &Akp, &Anq0, ((char *)ALPHA),
      ...
      else
      {
         for( k = 0; k < *N; k += nb )
         {
            ...
            if( Amp0 > 0 && Anq0 > 0 )
            {
               cgemv_( C2F_CHAR( NOTRAN ), &Amp0, &Anq0, ((char *) ALPHA),
                       Mptr( Aptr, Akp, Akq, Ald, size ), &Ald, Mptr( XR, 0,
                       Akq, XRld, size ), &XRld, one, Mptr( YC, Akp, 0, YCld,
                       size ), &ione );
               cgemv_( C2F_CHAR( COTRAN ), &Amp0, &Anq0, ((char *) ALPHA),
...
*/
      if( YCsum )
      {
         ...
         if( Amp > 0 )
         {
            ...
            Ccgsum2d( ctxt, ROW, &top, Amp, 1, YC, YCd[LLD_], myrow, 0 );
         }
      }
/*
...
*/
      if( YRsum && ( Anq > 0 ) )
      {
         ...
         Ccgsum2d( ctxt, COLUMN, &top, 1, Anq, YR, YRd[LLD_], YRd[RSRC_],
                   mycol );
      }
...
*/
      if( YRsum )
      {
         ...
         {
            top = *PB_Ctop( &ctxt, COMBINE, COLUMN, TOP_GET );
            Ccgsum2d( ctxt, COLUMN, &top, 1, Anq, YR, YRd[LLD_], 0,
                      mycol );
         }
      }
/*
...
*/
      ...
      {
         top = *PB_Ctop( &ctxt, COMBINE, ROW, TOP_GET );
         Ccgsum2d( ctxt, ROW, &top, Amp, 1, YC, YCd[LLD_], myrow,
                   YCd[CSRC_] );
      }
/*
*  YC := YR + YC
