...
*  =======
*
*  PZSCAL  multiplies  an  n  element  subvector  sub( X ) by the scalar
*  alpha,
*
*  where
*
...
*/
   ...
   if( !( info = ( ( nprow == -1 ) ? -( 601 + CTXT_ ) : 0 ) ) )
      PB_Cchkvec( ctxt, "PZSCAL", "X", *N, 1, Xi, Xj, Xd, *INCX, 6, &info );
   if( info ) { PB_Cabort( ctxt, "PZSCAL", info ); return; }
#endif
/*
...
*/
         ...
         if( Xnq > 0 )
         {
            ...
            {
               zset_( &Xnq, ((char *) ALPHA), Mptr( ((char *) X), Xii, Xjj,
                      Xld, type->size ), &Xld );
            }
            else
            {
               zscal_( &Xnq, ((char *) ALPHA), Mptr( ((char *) X), Xii,
