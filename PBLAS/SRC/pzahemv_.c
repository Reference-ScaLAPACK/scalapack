...
*  =======
*
*  PZAHEMV  performs the matrix-vector operation
*
...
*/
   ...
   if( !( info = ( ( nprow == -1 ) ? -( 701 + CTXT_ ) : 0 ) ) )
   {
      ...
      PB_Cchkmat( ctxt, "PZAHEMV", "A", *N, 2, *N, 2, Ai, Aj, Ad,  7, &info );
      PB_Cchkvec( ctxt, "PZAHEMV", "X", *N, 2, Xi, Xj, Xd, *INCX, 11, &info );
      PB_Cchkvec( ctxt, "PZAHEMV", "Y", *N, 2, Yi, Yj, Yd, *INCY, 17, &info );
   }
   if( info ) { PB_Cabort( ctxt, "PZAHEMV", info ); return; }
#endif
/*
...
*/
      ...
      if( upper )
      {
         for( k = 0; k < *N; k += nb )
         {
            ...
            if( Akp > 0 && Anq0 > 0 )
            {
               zagemv_( C2F_CHAR( NOTRAN ), &Akp, &Anq0, ((char *)ALPHA),
                        Mptr( Aptr, 0, Akq, Ald, size ), &Ald, Mptr( XR, 0, Akq,
                        XRld, size ), &XRld, one, YC, &ione );
               zagemv_( C2F_CHAR( COTRAN ), &Akp, &Anq0, ((char *)ALPHA),
      ...
      else
      {
         for( k = 0; k < *N; k += nb )
         {
            ...
            if( Amp0 > 0 && Anq0 > 0 )
            {
               zagemv_( C2F_CHAR( NOTRAN ), &Amp0, &Anq0, ((char *) ALPHA),
                        Mptr( Aptr, Akp, Akq, Ald, size ), &Ald, Mptr( XR, 0,
                        Akq, XRld, size ), &XRld, one, Mptr( YC, Akp, 0, YCld,
                        usiz ), &ione );
               zagemv_( C2F_CHAR( COTRAN ), &Amp0, &Anq0, ((char *) ALPHA),
