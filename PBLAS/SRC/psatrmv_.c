...
*  =======
*
*  PSATRMV  performs one of the matrix-vector operations
*
...
*/
   ...
   if( !( info = ( ( nprow == -1 ) ? -( 801 + CTXT_ ) : 0 ) ) )
   {
      ...
      {
         PB_Cwarn( ctxt, __LINE__, "PSATRMV", "Illegal UPLO = %c\n", UploA );
         info = -1;
      }
      else if( ( !notran ) && ( TranOp != CTRAN ) && ( TranOp != CCOTRAN ) )
      {
         PB_Cwarn( ctxt, __LINE__, "PSATRMV", "Illegal TRANS = %c\n", TranOp );
         info = -2;
      }
      else if( ( !nounit ) && ( DiagA != CUNIT ) )
      {
         PB_Cwarn( ctxt, __LINE__, "PSATRMV", "Illegal DIAG = %c\n", DiagA );
         info = -3;
      }
      PB_Cchkmat( ctxt, "PSATRMV", "A", *N, 4, *N, 4, Ai, Aj, Ad,  9, &info );
      PB_Cchkvec( ctxt, "PSATRMV", "X", *N, 4, Xi, Xj, Xd, *INCX, 13, &info );
      PB_Cchkvec( ctxt, "PSATRMV", "Y", *N, 4, Yi, Yj, Yd, *INCY, 19, &info );
   }
   if( info ) { PB_Cabort( ctxt, "PSATRMV", info ); return; }
#endif
/*
      ...
      if( upper )
      {
         if( notran )
         {
            for( k = 0; k < *N; k += nb )
            {
               ...
               if( Akp > 0 && Anq0 > 0 )
               {
                  sagemv_( TRANS, &Akp, &Anq0, ((char *) ALPHA),
      ...
      else
      {
         if( notran )
         {
            for( k = 0; k < *N; k += nb )
            {
               ...
               if( Amp0 > 0 && Anq0 > 0 )
               {
                  sagemv_( TRANS, &Amp0, &Anq0, ((char *) ALPHA),
