      SUBROUTINE PBZMATADD( ICONTXT, MODE, M, N, ALPHA, A, LDA, BETA, B,
     $                      LDB )
*
...
*  =======
*
*  PBZMATADD performs the matrix add operation B := alpha*A + beta*B,
...
*
            ELSE
               IF( M.EQ.LDB ) THEN
                  CALL ZSCAL( M*N, BETA, B( 1, 1 ), 1 )
               ELSE IF( LSAME( MODE, 'V' ) ) THEN
                  DO 670 J = 1, N
                     CALL ZSCAL( M, BETA, B( 1, J ), 1 )
  670             CONTINUE
               ELSE
                  DO 690 J = 1, N
                     DO 680 I = 1, M
         ...
         ELSE IF( ALPHA.EQ.ONE ) THEN
            IF( BETA.EQ.ZERO ) THEN
               IF( M.EQ.LDA .AND. M.EQ.LDB ) THEN
                  CALL ZCOPY( M*N, A( 1, 1 ), 1, B( 1, 1 ), 1 )
               ELSE IF( LSAME( MODE, 'V' ) ) THEN
                  DO 700 J = 1, N
                     CALL ZCOPY( M, A( 1, J ), 1, B( 1, J ), 1 )
  700             CONTINUE
               ELSE
                  DO 720 J = 1, N
                     DO 710 I = 1, M
...
*
            ELSE IF( BETA.EQ.ONE ) THEN
               IF( M.EQ.LDA .AND. M.EQ.LDB ) THEN
                  CALL ZAXPY( M*N, ALPHA, A( 1, 1 ), 1, B( 1, 1 ), 1 )
               ELSE IF( LSAME( MODE, 'V' ) ) THEN
                  DO 790 J = 1, N
                     CALL ZAXPY( M, ALPHA, A( 1, J ), 1, B( 1, J ), 1 )
  790             CONTINUE
               ELSE
                  DO 810 J = 1, N
                     DO 800 I = 1, M
