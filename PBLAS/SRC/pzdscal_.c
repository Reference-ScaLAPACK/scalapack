...
*  =======
*
*  PZDSCAL multiplies an n element subvector sub( X ) by the real scalar
*  alpha,
*
*  where
*
...
*/
   ...
   if( !( info = ( ( nprow == -1 ) ? -( 601 + CTXT_ ) : 0 ) ) )
      PB_Cchkvec( ctxt, "PZDSCAL", "X", *N, 1, Xi, Xj, Xd, *INCX, 6, &info );
   if( info ) { PB_Cabort( ctxt, "PZDSCAL", info ); return; }
#endif
/*
...
*/
         ...
         if( Xnq > 0 )
         {
            ...
            {
               zset_( &Xnq, type->zero, Mptr( ((char *) X), Xii, Xjj, Xld,
                      type->size ), &Xld );
            }
            else
            {
               zdscal_( &Xnq, ((char *) ALPHA), Mptr( ((char *) X),
