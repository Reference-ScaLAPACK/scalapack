...
*  =======
*
*  PZGEADD  adds a matrix to another
*
...
*/
   ...
   if( !( info = ( ( nprow == -1 ) ? -( 801 + CTXT_ ) : 0 ) ) )
   {
      ...
      {
         PB_Cwarn( ctxt, __LINE__, "PZGEADD", "Illegal TRANS = %c\n", TrA );
         info = -1;
      }
      if( notran )
         PB_Cchkmat( ctxt, "PZGEADD", "A", *M, 2, *N, 3, Ai, Aj, Ad,  8,
                     &info );
      else
         PB_Cchkmat( ctxt, "PZGEADD", "A", *N, 3, *M, 2, Ai, Aj, Ad,  8,
                     &info );
      PB_Cchkmat( ctxt, "PZGEADD", "C", *M, 2, *N, 3, Ci, Cj, Cd, 13, &info );
   }
   if( info ) { PB_Cabort( ctxt, "PZGEADD", info ); return; }
#endif
/*
...
*/
   if( notran )
   {
      ...
      PB_Cpgeadd( PB_Cztypeset(), &DirA, &DirC, NOCONJG, *M, *N,
                  ((char *) ALPHA), ((char *) A), Ai, Aj, Ad,
                  ((char *)  BETA), ((char *) C), Ci, Cj, Cd );
   }
   else if( TrA == CTRAN )
   {
      PB_Cptran( PB_Cztypeset(), NOCONJG, *M, *N, ((char *) ALPHA),
